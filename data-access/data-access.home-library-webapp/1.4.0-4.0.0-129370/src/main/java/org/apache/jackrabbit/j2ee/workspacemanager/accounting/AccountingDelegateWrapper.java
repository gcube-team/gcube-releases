...
//			logger.debug("Accountin node type " + nodeType.getNodeTypeDefinition());
	...
	private void setCustomProperties(Node entryNode, AccountingDelegate item) throws ValueFormatException, VersionException, LockException, ConstraintViolationException, RepositoryException {
		...
		case REMOVAL:
			try{
				FolderItemType folderItemType = (FolderItemType) new XStream().fromXML(properties.get(AccountingProperty.FOLDER_ITEM_TYPE));
				entryNode.setProperty(AccountingProperty.FOLDER_ITEM_TYPE.toString(), folderItemType.toString());
...
//				logger.info("FOLDER_ITEM_TYPE and MIME_TYPE not in " + entryNode.getPath() );
			...
			WorkspaceItemType itemType = (WorkspaceItemType) new XStream().fromXML(properties.get(AccountingProperty.ITEM_TYPE));
			entryNode.setProperty(AccountingProperty.ITEM_TYPE.toString(), itemType.toString());
			entryNode.setProperty(AccountingProperty.ITEM_NAME.toString(), (String) new XStream().fromXML(properties.get(AccountingProperty.ITEM_NAME)));
		...
		case ADD:
			try{
				FolderItemType folderItemType1 = (FolderItemType) new XStream().fromXML(properties.get(AccountingProperty.FOLDER_ITEM_TYPE));
				entryNode.setProperty(AccountingProperty.FOLDER_ITEM_TYPE.toString(), folderItemType1.toString());
				entryNode.setProperty(AccountingProperty.MIME_TYPE.toString(), (String) new XStream().fromXML(properties.get(AccountingProperty.MIME_TYPE)));		
			}catch (Exception e) {
//				logger.info("FOLDER_ITEM_TYPE and MIME_TYPE not in " + entryNode.getPath() );
			}

			WorkspaceItemType itemType1 = (WorkspaceItemType) new XStream().fromXML(properties.get(AccountingProperty.ITEM_TYPE));
			entryNode.setProperty(AccountingProperty.ITEM_TYPE.toString(), itemType1.toString());
			entryNode.setProperty(AccountingProperty.ITEM_NAME.toString(), (String) new XStream().fromXML(properties.get(AccountingProperty.ITEM_NAME)));
			break;
		case PASTE:
