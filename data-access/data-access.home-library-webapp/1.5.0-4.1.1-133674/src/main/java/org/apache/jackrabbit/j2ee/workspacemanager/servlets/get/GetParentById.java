		...
		try {
			...
			if (exist){				
				...
				try{
					...
					ItemDelegate parent = getParent(session, id, sessionManager.getLogin(request));
					xmlConfig = xstream.toXML(parent);
//					response.setContentLength(xmlConfig.length());
					out.println(xmlConfig);
				} catch (Exception e) {
					logger.error("Error getting parent by id: " + id, e);
...
//			out.flush();
	 ...
	 * @throws Exception
	 */
	private ItemDelegate getParent(Session session,
