...
public class Copy extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		Boolean isSourceFolderShared = request.getParameter(ServletParameter.IS_FOLDER_SHARED)==null?false:
			Boolean.parseBoolean(request.getParameter(ServletParameter.IS_FOLDER_SHARED));
		...
		try {
			...
			if (subgraph)
				...
				new_item = copyItem(session, srcAbsPath, destAbsPath, sessionManager.getLogin(request), isSourceFolderShared);
	...
	private ItemDelegate copyItem(Session session, String srcAbsPath, String destAbsPath, String login, boolean isSourceShared) throws RepositoryException{
		ItemDelegate item = null;
		try{
			...
			Node destnode;
			if (isSourceShared){
				destnode = session.getNode(destAbsPath);
				...
				while(children.hasNext()){
					...
					if (child.getName().startsWith("hl:"))
						continue;
					try{
						session.getWorkspace().copy(srcAbsPath+"/"+child.getName(), destAbsPath+"/"+child.getName());
					} catch (Exception e) {
						logger.error("Error copying node " + child.getPath(), e);
					}
				}
				session.save();
			} else {
			...
			NodeManager wrap = new NodeManager(destnode, login);
			item = wrap.getItemDelegate();
			

		} catch (Exception e) {
			logger.error("Error getting copy of node {}",srcAbsPath,e);
			throw new RepositoryException("Error getting copy of node " + srcAbsPath ,e);
		}
		return item;
	}
