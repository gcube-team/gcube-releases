...
//		String uuid = request.getParameter(ServletParameter.UUID);	
		...
		try {
			...
			try{
				...
				Lock lock = lockManager.lock(session.getNodeByIdentifier(id).getPath(), true, true, 0, sessionId);
				logger.info("LOCK on Node: " + lock.getNode().getPath() + " by session with ID " + sessionId);
				session.save();
				flag = true;
			
			} catch (Exception e) {
				logger.error("Item with id: " + id + " cannot be locked in session " + sessionId, e);
			}
