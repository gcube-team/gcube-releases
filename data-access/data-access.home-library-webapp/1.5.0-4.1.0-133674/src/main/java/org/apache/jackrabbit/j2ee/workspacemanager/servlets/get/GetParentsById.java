...
public class GetParentsById extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			try{
				...
				List<ItemDelegate> children = getParentsById(session, id, sessionManager.getLogin(request));
	 ...
	 * @throws Exception
	 */
	private List<ItemDelegate> getParentsById(Session session, String identifier,
			String login) throws Exception {

		Validate.notNull(identifier, "Item id must be not null");
		Validate.notNull(login, "Login must be not null");

		logger.info("Getting parents by id " + identifier + " - user: " + login);
		...
		try {
			while(!isRoot(node) && (node!=null)) {
				...
				if ((item.getPrimaryType().equals(PrimaryNodeType.NT_WORKSPACE_SHARED_FOLDER) && (!login.equals("guest")))){
