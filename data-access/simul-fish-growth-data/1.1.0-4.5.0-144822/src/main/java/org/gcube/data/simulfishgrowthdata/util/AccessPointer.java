...
public class AccessPointer {
	...
	public AccessPoint getIt() throws Exception {
		...
		try {
			...
			query.addCondition(String.format("$resource/Profile/Name/text() eq '%s'", mName)).setResult(
					"<accesspoint><id>{$resource/ID/text()}</id>{$resource/Profile/AccessPoint}</accesspoint>");
