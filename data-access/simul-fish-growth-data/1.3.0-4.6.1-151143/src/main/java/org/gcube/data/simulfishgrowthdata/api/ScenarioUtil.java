...
public class ScenarioUtil extends BaseUtil {
	...
	public Scenario executeScenario(@PathParam("id") Long id) throws Exception {
		try {
			return new org.gcube.data.simulfishgrowthdata.api.base.ScenarioUtil().executeScenario(id);
		} catch (Exception e) {
	...
	public String executeConsumptionScenario(@PathParam("from") String from, @PathParam("to") String to,
			...
			throws Exception {
		return new org.gcube.data.simulfishgrowthdata.api.base.ScenarioUtil().executeConsumptionScenario(from, to,
				weight, count, modelId);
	}
