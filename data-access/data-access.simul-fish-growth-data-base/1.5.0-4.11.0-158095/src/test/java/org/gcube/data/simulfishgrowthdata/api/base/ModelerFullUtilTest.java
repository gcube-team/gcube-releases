...
public class ModelerFullUtilTest extends TestCase {
	...
	public void testModeler() throws Exception {
		...
		try {
			...
			if (myHeroModelId != null) {
				...
				mutil.update(session, pong, true);
	...
	static List<Site> prepareSitesInfrastructureFromScratch(final Session session, Integer forceTemp) throws Exception {
		...
		Set<Long> similars = su.findSitesSimilarToMe(session, site1, 1, 1);
		assertTrue("site1 similars contains site1", similars.contains(site1Id));
		assertTrue("site1 similars contains site2", similars.contains(site2Id));

		similars = su.findSitesSimilarToMe(session, site2, 1, 1);
		assertTrue("site2 similars contains site1", similars.contains(site1Id));
		assertTrue("site2 similars contains site2", similars.contains(site2Id));
	...
	static List<Modeler> prepareModelsInfrastructureFromScratch(final Session session, List<Site> sites)
		...
		{ // modelers
			...
			{ // modeler 1 1
				...
				mu.update(session, modeler, true);
			}
