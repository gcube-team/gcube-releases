...
public class GlobalModelWrapper {
	...
	public GlobalModelWrapper(final Session session, final String latitude, final String longitude,
			...
			final OxygenRetriever oxygenRetriever) throws Exception {
		this(session, latitude, longitude, speciesId, temperatureRetriever, oxygenRetriever, 1, 1);
	}

	public GlobalModelWrapper(final Session session, final String latitude, final String longitude,
			final Long speciesId, final TemperatureRetriever temperatureRetriever,
			final OxygenRetriever oxygenRetriever, int acceptableSiteCount, int upToGrade) throws Exception {
		this(session);
	...
	private Site discoverVirtualSite(String latitude, String longitude) throws Exception {
		...
		Set<Long> similars = new SiteUtil().setAdditionalSimilarityConstraint(additionalSimilarityConstraint)
