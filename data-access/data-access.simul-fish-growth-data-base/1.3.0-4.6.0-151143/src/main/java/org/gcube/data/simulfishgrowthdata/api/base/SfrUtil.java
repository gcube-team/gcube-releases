...
public class SfrUtil extends BaseUtil {
	...
	public List<Sfr> getSfrs(Long modelId) throws Exception {
		...
		try {
			...
			List<Sfr> list = getSfrs(session, modelId);

			session.getTransaction().commit();

			return list;
		} catch (Exception e) {
			logger.error(String.format("Could not retrieve sfr for modelid [%s]", modelId), e);
	...
	public List<Sfr> getSfrs(final Session session, final Long modelId) throws Exception {
		Query q = session.createQuery(_GET_ALL_ON_MODELER).setParameter("modelerId", modelId);
		List<Sfr> list = q.list();
		return list;
	}
