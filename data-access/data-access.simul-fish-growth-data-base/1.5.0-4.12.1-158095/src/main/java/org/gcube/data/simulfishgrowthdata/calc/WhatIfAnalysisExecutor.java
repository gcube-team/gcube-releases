...
public class WhatIfAnalysisExecutor extends ScenarioExecutor {
	...
	protected String processResults() {
		...
		Scenario globalScenario = new Scenario(mScenario);
		try {
			...
			globalScenario.setId(-1);
			GlobalModelScenarioExecutor globalExecutor = new GlobalModelScenarioExecutor(mSession, globalScenario,
					globalModel);
		...
		String fullResults = "[";
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "Average Weight", 
				mScenario.getResultsWeight(), globalScenario.getResultsWeight());
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "LTD Growth", 
				mScenario.getResultsGrowth(), globalScenario.getResultsGrowth());
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "LTD SGR", 
				mScenario.getResultsSGR(), globalScenario.getResultsSGR());
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "LTD Biological FCR", 
				mScenario.getResultsBiolFCR(), globalScenario.getResultsBiolFCR());
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "LTD Economical FCR", 
				mScenario.getResultsEconFCR(), globalScenario.getResultsEconFCR());
		fullResults += String.format("{\"label\":\"%s\", \"value\": \"%.2f\", \"global\": \"%.2f\"},", "LTD Mortality %", 
				mScenario.getResultsMortality(), globalScenario.getResultsMortality());
		fullResults += "]";
