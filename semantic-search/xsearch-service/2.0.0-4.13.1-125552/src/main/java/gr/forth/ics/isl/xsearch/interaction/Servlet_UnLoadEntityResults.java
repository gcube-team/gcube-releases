 ...
 */
public class Servlet_UnLoadEntityResults extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //System.out.println("# Initial docs:" + docs);

            String docids_string = request.getParameter("docids");
            //System.out.println("# Docs to Unload:" + docids_string);
            ...
            String[] docids = docs.split(",");
            ArrayList<Integer> docids_in_list = new ArrayList<Integer>();

            for (int i = 0; i < docids.length; i++) {
                int id = Integer.parseInt(docids[i].trim());
                docids_in_list.add(id);
            }

            String[] docids_to_rm = docids_string.split(",");

            for (int i = 0; i < docids_to_rm.length; i++) {
                int id = Integer.parseInt(docids_to_rm[i].trim());
                docids_in_list.remove(new Integer(id));
            }

            Collections.sort(docids_in_list);
            //System.out.println("# Final Docs:" + docids_in_list);

            String new_docs_list = docids_in_list.toString();
            new_docs_list = new_docs_list.replace("]", "").replace("[", "").replace(" ", "");
            session.setAttribute("docs", new_docs_list);
            ...
            if (docids_list_to_show.isEmpty()) {

                session.setAttribute("docs", "");
                results = (Bean_Search) session.getAttribute("entities");


                out.print(results.getResults_first_page());
    ...
    public void updateLog(HttpServletRequest request, String query, String category, String entity, String docs) {
        ...
        String line = "\n" + Resources.SYSTEMNAME + "\t" + date + "\t" + ip + "\t" + query + "\tUNLOAD ENTITY '" + entity + "' FROM CATEGORY '" + category + "' WITH DOCS '" + docs + "'";
