...
public class Servlet_ShowAll extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String what = "";
        if (id.toLowerCase().contains("descr")) {
            what = "DESCRIPTION";
        } else {
            what = "TITLE";
        }

        String doc = id.substring(id.length() - 1);


        updateLog(request, query, what, doc);

    }

    public void updateLog(HttpServletRequest request, String query, String what, String doc) {

        String ip = request.getRemoteAddr();
        String date = IOSLog.getCurrentDate();

        String line = "\n" + Resources.SYSTEMNAME + "\t" + date + "\t" + ip + "\t" + query + "\tSHOW ALL " + what + " " + doc;
