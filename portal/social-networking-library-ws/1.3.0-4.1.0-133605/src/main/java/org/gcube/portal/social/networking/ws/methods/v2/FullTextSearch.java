...
@Api(tags={"/2/full-text-search"}, description="Endpoint for full-text-search services over social data", protocols="https", authorizations={@Authorization(value=RequestsAuthFilter.AUTH_TOKEN)})
		...
		try{
			...
			if (groupManager.isRootVO(currentGroupId)) {
				for (GCubeGroup group : groupManager.listGroupsByUser(currUser.getUserId())) {		
					if (groupManager.isVRE(group.getGroupId())) {
			...
			List<EnhancedFeed> enhancedFeeds = ElasticSearchConnection.getElasticSearchConnection().searchInEnhancedFeeds(query, vres, from, quantity);
			DatabookStore store = CassandraConnection.getDatabookStore();
