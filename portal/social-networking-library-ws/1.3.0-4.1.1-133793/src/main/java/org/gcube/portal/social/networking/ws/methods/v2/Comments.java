 ...
 */
@Path("2/comments")
@Api(protocols="https", authorizations={@Authorization(value=RequestsAuthFilter.AUTH_TOKEN)}, tags={"/2/comments"})
/**
...
public class Comments {
	...
	public Response getCommentsUser() {
		...
		try{
			...
			comments = CassandraConnection.getDatabookStore().getRecentCommentsByUserAndDate(username, 0);
			Utils.filterCommentsPerContext(comments, context);
	...
	public Response getCommentsUserByTime(
		...
		try{
			...
			logger.info("Retrieving comments for user id " + username);
			comments = CassandraConnection.getDatabookStore().getRecentCommentsByUserAndDate(username, timeInMillis);
			Utils.filterCommentsPerContext(comments, context);
			responseBean.setResult(comments);
