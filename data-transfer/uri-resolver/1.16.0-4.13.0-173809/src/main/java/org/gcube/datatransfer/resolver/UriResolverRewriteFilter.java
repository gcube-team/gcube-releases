...
import org.gcube.datatransfer.resolver.storagehub.StorageHubResolver;
...
public class UriResolverRewriteFilter implements Filter{
	...
	public static final String SERVLET_STORAGE_HUB = "/shub";
	...
	public void doFilter(ServletRequest req, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		try{
			...
			}else if(servletPath.startsWith(SERVLET_STORAGE_HUB) || servletPath.startsWith(SERVLET_URI_RESOLVER+SERVLET_STORAGE_HUB)){
				logger.debug("It is a request to StorageHub");
				int lastSlash = requestURI.lastIndexOf(PATH_SEPARATOR);
				String toStorageHUBID = requestURI.substring(lastSlash + 1, requestURI.length());
				String toQueryString = queryString==null?"":queryString; //it can contain 'version' to download
				String newURI = SERVLET_STORAGE_HUB + "?" +  StorageHubResolver.ID+"=" + toStorageHUBID+ toQueryString;
