...
public class DecompressPluginFactory extends AbstractPluginFactory<DecompressPlugin> {
	...
	@Override
	public void checkInvocation(PluginInvocation invocation) throws ParameterException {
		try{
			Map<String,String> params=invocation.getParameters();
			if(params == null || params.isEmpty()) throw new ParameterException("Invocation parameter map is empty");
			if(!params.containsKey(SOURCE_PARAMETER)) throw new ParameterException("Missing mandatory parameter "+SOURCE_PARAMETER);
