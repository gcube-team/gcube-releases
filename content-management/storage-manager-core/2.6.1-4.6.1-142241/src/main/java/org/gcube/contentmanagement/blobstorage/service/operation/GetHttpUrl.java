...
public class GetHttpUrl extends Operation {
	...
	public Object doIt(MyFile myFile) throws RemoteBackendException {
		...
		String urlParam="";
		try {
			String id=getId(myFile.getAbsoluteRemotePath(), myFile.isForceCreation(), myFile.getGcubeMemoryType());
...
//			String urlEncoded=URLEncoder.encode(urlParam, "UTF-8");
		...
		String smpUrl = smpUrl=urlBase+urlParam;
		...
		if(myFile.getGcubeMemoryType().equals(MemoryType.VOLATILE)){
			return httpUrl.toString()+VOLATILE_URL_IDENTIFICATOR;
		}
	...
	private String getId(String path, boolean forceCreation, MemoryType memoryType){
		...
		if(tm ==null){
			...
			tm=tmf.getTransport(backendType, memoryType);
		}
		try {
