...
//	the shared scope is a private scope. The change is in the id variable 		
			this.accessType="private";
		}
...
//		delete initial / from variable environment
	...
	public LocalResource get(String backendType){
		...
		this.service=new OperationManager(server, user, password, getCurrentOperation(), file, backendType);
	...
	public String getTotalUserVolume(){
		...
		try {
			if(((file.getInputStream() != null) || (file.getOutputStream()!=null)) || ((file.getLocalPath() != null) || (file.getRemotePath() != null)))
				info=(String)service.startOperation(file,file.getRemotePath(), owner, server, DEFAULT_CHUNK_OPTION, getContext(), isReplaceOption());
			else{
	...
	public LocalResource put(boolean replace){
		...
		this.service=new OperationManager(server, user, password,  getCurrentOperation(), getMyFile(), backendType);
	...
	public RemoteResource remove(String backendType){
		...
		file.setGcubeMemoryType(this.getGcubeMemoryType());
...
//		put(true);
	...
	public void close(){
		...
		try {
			if(((file.getInputStream() != null) || (file.getOutputStream()!=null)) || ((file.getLocalPath() != null) || (file.getRemotePath() != null)))
				service.startOperation(file,file.getRemotePath(), owner, server, DEFAULT_CHUNK_OPTION, getContext(), isReplaceOption());
			else{
				logger.error("parameters incompatible ");
			}
