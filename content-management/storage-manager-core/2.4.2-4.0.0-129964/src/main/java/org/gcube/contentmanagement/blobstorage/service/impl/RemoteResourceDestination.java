	...
	public String to(String path) throws RemoteBackendException{
		...
		try {
			if((file.getLocalPath() != null) && ((file.getRemotePath() != null))){
				bucketName=(String)engine.service.startOperation(file, file.getRemotePath(), engine.owner, engine.server, ServiceEngine.DEFAULT_CHUNK_OPTION, engine.getContext(), engine.isReplaceOption());
			}else{
