	...
	public long RFile(String path) throws RemoteBackendException{
		...
		try {
			if(((file.getInputStream() != null) || (file.getOutputStream()!=null)) || ((file.getLocalPath() != null) || (file.getRemotePath() != null)))
				info=(String)engine.service.startOperation(file,file.getRemotePath(), engine.owner, engine.server, ServiceEngine.DEFAULT_CHUNK_OPTION, engine.getContext(), engine.isReplaceOption());
			else{
	...
	public long RFileById(String id) throws RemoteBackendException{
		...
		try {
			if(engine.getCurrentOperation().equalsIgnoreCase("download"))
				info=engine.service.startOperation(getMyFile(), id, engine.owner, engine.server, ServiceEngine.DEFAULT_CHUNK_OPTION, "", engine.isReplaceOption());
			else
