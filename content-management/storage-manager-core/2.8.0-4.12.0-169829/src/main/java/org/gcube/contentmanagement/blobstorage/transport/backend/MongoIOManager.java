...
public class MongoIOManager {
	...
	private static final boolean DEFAULT_READWRITE_PREFERENCE= true;
...
// if contains the field link it means that is a link hence I follow ne or more links		
		while((f !=null ) && (f.containsField(Operation.LINK_IDENTIFIER)) && (f.get(Operation.LINK_IDENTIFIER) != null)){
			BasicDBObject query = new BasicDBObject();
			query.put( "_id" , new ObjectId((String)f.get(Operation.LINK_IDENTIFIER)) );
//			query.put( "_id" , f.get(Operation.LINK_IDENTIFIER) );
			f=gfs.findOne( query );
		}
...
//			        if((oldir !=null) &&(oldir.equalsIgnoreCase(((MyFile)resource).getRemoteDir()))){
			  // if the file contains a link the replace is not allowed
			         	  if((!replace)){
			         		  return oldId;
			         	  }else if((fold.containsField(Operation.COUNT_IDENTIFIER)) && (fold.get(Operation.COUNT_IDENTIFIER)!=null)){
			         		  close();
...
// check if the file is linked		
		if((f!=null) && (f.containsField(Operation.COUNT_IDENTIFIER)) && (f.get(Operation.COUNT_IDENTIFIER) != null)){
...
// remove metadata: dir, filename, name 	
	...
	// check if the file is a link 
		}else if((f.containsField(Operation.LINK_IDENTIFIER)) && (f.get(Operation.LINK_IDENTIFIER) != null )){
			while((f!=null) && (f.containsField(Operation.LINK_IDENTIFIER)) && (f.get(Operation.LINK_IDENTIFIER) != null )){
				// remove f and decrement linkCount field on linked object
				String id=(String)f.get(Operation.LINK_IDENTIFIER);
				GridFSDBFile fLink=findGFSCollectionObject(new ObjectId(id));
			    int linkCount=(Integer)fLink.get(Operation.COUNT_IDENTIFIER);
			    linkCount--;
			    if(linkCount == 0){
		// if the name the filename and dir are null, then I delete also the link object	    	
		    ...
		    // this field is an advice for oplog collection reader			 
						 removeGFSFile(f, idF);
			    		if((fLink.containsField(Operation.LINK_IDENTIFIER)) && (fLink.get(Operation.LINK_IDENTIFIER) != null )){
			    	//the link is another link		
							id=(String)fLink.get(Operation.LINK_IDENTIFIER);
			    	...
			    	// the link is not another link		
			    		...
			    		fLink.put(Operation.COUNT_IDENTIFIER, null);
			    		fLink.save();
			    	...
			    	fLink.put(Operation.COUNT_IDENTIFIER, linkCount);
			    	fLink.save();
					ObjectId oId=(ObjectId) f.getId();
					idFile=oId;
     				removeGFSFile(f, oId);
...
//	 */
//	public ObjectId createNewDuplicatesMap(Object resource, String bucket, String dir,String name, ObjectId oldId) throws UnknownHostException {
//			ObjectId id;
//		// create new dir
//			if((dir !=null && !dir.isEmpty()) && (bucket !=null && !bucket.isEmpty())){
//				buildDirTree(getMetaDataCollection(null), dir);
...
//	}
//	
//	public ObjectId createNewObjectMap() throws UnknownHostException {
//		ObjectId id;
...
//}
//	
//	public ObjectId addToDuplicateMap(ObjectId mapId, GridFSDBFile f) throws UnknownHostException {
//		return mapId;
//	}
//
//	public ObjectId getDuplicatesMap(String md5){
...
//		clean();
	...
	protected void replaceObjectIDOnMetaCollection(ObjectId oldId, ObjectId newId)
		...
		String collectionName= MongoIOManager.DEFAULT_META_COLLECTION;
		DBCollection dbc=getCollection(null, collectionName);
		DBObject obj=findCollectionObject(dbc, oldIdQuery);// or multiple objects?
		obj.put("_id", newId);
//		dbc.dropIndex("_id");
		if (!(memoryType== MemoryType.VOLATILE)){
			dbc.remove(oldIdQuery, MongoIOManager.DEFAULT_WRITE_TYPE);
			dbc.insert(obj, MongoIOManager.DEFAULT_WRITE_TYPE);
		}else{
...
//		searchQuery.put("files_id", oldId);
		...
		String chunksCollectionName=MongoIOManager.DEFAULT_CHUNKS_COLLECTION;
...
// END PATCHED METHODS
	...
	public GridFSDBFile retrieveLinkPayload(GridFSDBFile f) throws UnknownHostException {
		while((f.containsField(Operation.LINK_IDENTIFIER)) && (f.get(Operation.LINK_IDENTIFIER) != null )){
			String id=(String)f.get(Operation.LINK_IDENTIFIER);
	...
	public BasicDBObject findMetaCollectionObject(String source) throws UnknownHostException {
		DBCollection fileCollection=getConnectionDB(dbName, false).getCollection(DEFAULT_META_COLLECTION);
		BasicDBObject query = new BasicDBObject();
...
//		GridFS gfs = new GridFS(getDB());
	...
	public GridFS getGfs(){
		return getGfs(DEFAULT_READWRITE_PREFERENCE);
	}
