...
//	public static MongoIO getInstance(String[] server, String user, String password){
//		if(instance==null){
//			instance= new MongoIO(server, user, password);
//		}
//		return instance;
//	}
	
	protected MongoIO(String[] server, String user, String password){
		setServer(server);
		setUser(user);
	...
	protected DB getDB(String writeConcern, String readConcern){
		if(db==null){
			...
			if(mongo==null){		
					...
					MongoClientOptions options=MongoClientOptions.builder().connectionsPerHost(30).connectTimeout(30000).readPreference(DEFAULT_READ_PREFERENCE).build();
	...
	protected String readByPath(MyFile resource, GridFSDBFile f, boolean isLock, int count)
		...
		String key=null;
		try{
//			MongoInputStream is=new MongoInputStream(mongo, f.getInputStream());
