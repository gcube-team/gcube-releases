...
public class DefaultMongoClient extends TransportManager{
	...
	public DefaultMongoClient(String[] server, String user, String password){
		try {
			io=new MongoIO(server, user, password);//MongoIO.getInstance(server, user, password);
			io.clean();
	...
	@Override
	public void initBackend(String[] server, String user, String pass) {
		try {
			io=new MongoIO(server, user, pass);//MongoIO.getInstance(server, user, password);
			io.clean();
			DBCollection coll =io.getMetaDataCollection();// io.getDB().getCollection("fs.files");
			coll.createIndex(new BasicDBObject("filename", 1));  // create index on "filename", ascending
			coll.createIndex(new BasicDBObject("dir", 1));  // create index on "filename", ascending
...
// create new file		
	...
	private void replaceObjectIDOnMetaCollection(ObjectId oldId, ObjectId newId)
		...
		String collectionName= io.DEFAULT_META_COLLECTION;
...
//			 db=null;
		...
		String dir=((MyFile)resource).getRemoteDir();
		String name=((MyFile)resource).getName();
		String path=bucket; //((MyFile)resource).getPathServer();
		if(logger.isDebugEnabled())
	...
	public void removeDir(String remoteDir, MyFile resource){
		...
		dirs.add(remoteDir);
	// patch for incompatibility v 1-2	
		if((remoteDir.contains(io.ROOT_PATH_PATCH_V1)) || (remoteDir.contains(io.ROOT_PATH_PATCH_V2))){
			if(remoteDir.contains(io.ROOT_PATH_PATCH_V1)){
				String remoteDirV1=remoteDir.replace(io.ROOT_PATH_PATCH_V1, io.ROOT_PATH_PATCH_V2);
				dirs.add(remoteDirV1);
			}else{
				String remoteDirV2= remoteDir.replace(io.ROOT_PATH_PATCH_V2, io.ROOT_PATH_PATCH_V1);
...
//				updateCommonFields(sourcePathMetaCollection, resource);
				...
				DBCollection metaCollectionInstance=io.getMetaDataCollection(io.getDB(resource.getWriteConcern(), resource.getReadPreference()));
	...
	// retrieve original object		
	   ...
	   //update common fields		
				...
				metaCollectionInstance.update(querySourcePath, updateQuery, false, true, io.DEFAULT_WRITE_TYPE);
...
//					sourcePathMetaCollection.put("filename", destinationPath);  
//					sourcePathMetaCollection.put("name", name);
//					sourcePathMetaCollection.put("owner", ((MyFile)resource).getOwner());
					...
					io.printObject(sourcePathMetaCollection);
			//applies the update		
					metaCollectionInstance.update(querySourcePath, sourcePathMetaCollection, false, true, io.DEFAULT_WRITE_TYPE);
...
// then if it is a new folder						
					...
					//update common fields		
						...
						updateQuery.put("$set", sourcePathMetaCollection);
						metaCollectionInstance.update(queryDestPath, updateQuery, true, true, io.DEFAULT_WRITE_TYPE);
					}
				}
				close();
	...
	private ObjectId removeFile(Object resource, String key, boolean replace,
		...
		//	         			  idFile=checkAndRemove(fold, (MyFile)resource);
			         			 oldId=checkAndRemove(fold, (MyFile)resource);
			         			  //	         	  		  gfs.remove(oldId);  
			         		  }
			         	  }
			        }else if(oldir == null){
			        	String filename=(String)fold.get("filename");
	...
	private ObjectId removeOldMetadataFile(ObjectId oldId) throws IllegalAccessError,
			UnknownHostException {
		//remove old metadata object
		DBCollection files=io.getMetaDataCollection(null);
		BasicDBObject query=new BasicDBObject();
		query.put("_id", oldId);	
		files.remove(query);
		return oldId;
...
//		f.put("$set", new BasicDBObject().append("filename", filename).append("type", "file").append("name", name).append("dir", dir));
...
//// create query for dir field
			...
			if(folder!=null){
				idList=new ArrayList(folder.size());
			...
			if((folderCursor !=null)){
				...
				while(folderCursor.hasNext()){//GridFSDBFile f : folder){
					...
					if(f.get("type").equals("file")){
						...
						meta.update(query, f, true, false, io.DEFAULT_WRITE_TYPE);
//						meta.update(query, f, true, true);
	...
	public String setFileProperty(String remotePath, String propertyField, String propertyValue){
		...
		if(!propertyField.equalsIgnoreCase("mimetype")){
			...
			try {
				...
				if(remoteMetaCollectionObject!=null){
					...
					metaCollectionInstance.update(querySourceObject, updateQuery, false, true, io.DEFAULT_WRITE_TYPE);
	...
	// if the file doesn't exist. An empty file will be created
			if(!ObjectId.isValid(path)){
				byte[] data=new byte[1];
				GridFSInputFile f2 = io.createGFSFileObject(data, null, null);//gfs.createFile(data);
	...
	private BasicDBObject setCommonFields(BasicDBObject f, MyFile resource, OPERATION op) {
		BasicDBObject updateFieldObject=null;
