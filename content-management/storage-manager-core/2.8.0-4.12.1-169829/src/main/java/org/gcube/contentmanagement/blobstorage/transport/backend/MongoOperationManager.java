...
//		DB db=mongoPrimaryInstance.getConnectionDB(resource.getWriteConcern(), resource.getReadPreference(),getPrimaryCollectionName(), true);
	...
	private void patchCompatibilityOldLibraryVersion(String remoteDir, ArrayList<String> dirs) {
		if((remoteDir.contains(MongoIOManager.ROOT_PATH_PATCH_V1)) || (remoteDir.contains(MongoIOManager.ROOT_PATH_PATCH_V2))){
			if(remoteDir.contains(MongoIOManager.ROOT_PATH_PATCH_V1)){
				String remoteDirV1=remoteDir.replace(MongoIOManager.ROOT_PATH_PATCH_V1, MongoIOManager.ROOT_PATH_PATCH_V2);
				dirs.add(remoteDirV1);
			}else{
				String remoteDirV2= remoteDir.replace(MongoIOManager.ROOT_PATH_PATCH_V2, MongoIOManager.ROOT_PATH_PATCH_V1);
	...
	private void updateMetaObject(String remoteIdentifier, String propertyField, String propertyValue)
		...
		if(remoteMetaCollectionObject!=null){
			...
			if (!(memoryType== MemoryType.VOLATILE))
				metaCollectionInstance.update(querySourceObject, updateQuery, false, true, MongoIOManager.DEFAULT_WRITE_TYPE);
			else
