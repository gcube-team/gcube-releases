...
public class DistributionGeneratorAsynchronousWriting{
	...
	//QUEUES TO BE WRITTEN TO DB
	List<StringBuffer> probabilitiesQueue;
	List<StringBuffer> speciesCriteria1Queue;
	List<StringBuffer> speciesCriteria2Queue;
	List<StringBuffer> speciesCriteria3Queue;
...
//	static String selectSpeciesObservationQuery = "SELECT DISTINCT Max(hcaf_s.CenterLat) AS maxCLat, Min(hcaf_s.CenterLat) AS minCLat FROM occurrencecells INNER JOIN hcaf_s ON occurrencecells.CsquareCode = hcaf_s.CsquareCode WHERE (((hcaf_s.oceanarea > 0))) AND occurrencecells.SpeciesID = '%1$s' AND occurrencecells.GoodCell <> 0;";
	...
	public String getSpeciesLoad(){
		...
		if (isRemoteGeneration){
			...
			ResourceLoad rs = new ResourceLoad(rhoo.metrics.timestamp,rhoo.metrics.processedspecies);
	...
	private void initBuffers(){
		...
		speciesCriteria1Queue = new ArrayList<StringBuffer>();
		speciesCriteria2Queue = new ArrayList<StringBuffer>();
		speciesCriteria3Queue = new ArrayList<StringBuffer>();
		probabilitiesQueue = new ArrayList<StringBuffer>();
	}
	
	
	//initializes write buffers
	private void initWritingThread(){
		ThreadInserter inserter = new ThreadInserter();
	...
	//inserts string buffers from the queues
	private void insertCriteriaQueue(){
		
		for (int i=0;i<speciesCriteria1Queue.size();i++){
			bulkInsert(speciesCriteria1Queue.get(i));
			speciesCriteria1Queue.remove(i);
			i--;
		}
		for (int i=0;i<speciesCriteria2Queue.size();i++){
			bulkInsert(speciesCriteria2Queue.get(i));
			speciesCriteria2Queue.remove(i);
			i--;
		}
		for (int i=0;i<speciesCriteria3Queue.size();i++){
			bulkInsert(speciesCriteria3Queue.get(i));
			speciesCriteria3Queue.remove(i);
			i--;
		}
		for (int i=0;i<probabilitiesQueue.size();i++){
			bulkInsert(probabilitiesQueue.get(i));
			probabilitiesQueue.remove(i);
			i--;
		}
	}
	...
	//checks out the insertion type to perform in the queue
	private void insertCriteriaintoQueue(){
		
		StringBuffer criteria = new StringBuffer();
		if (speciesCriteria1.length()>0){
				criteria = criteria.append(speciesCriteria1);
				speciesCriteria1Queue.add(criteria);
			}
		else if (speciesCriteria2.length()>0){
			criteria = criteria.append(speciesCriteria2);
			speciesCriteria2Queue.add(criteria);
		}
		else if (speciesCriteria3.length()>0){
			criteria = criteria.append(speciesCriteria3);
			speciesCriteria3Queue.add(criteria);
		}
		else if (probsClause.length()>0){
			criteria = criteria.append(probsClause);
			probabilitiesQueue.add(criteria);
		}
	}
...
//		AnalysisLogger.getLogger().trace("elapsed on complete calculation: "+t1c+" ms");
	...
	public DistributionGeneratorAsynchronousWriting(EngineConfiguration engine) throws Exception 
	{
		if (engine!=null){
			useDB = engine.useDB();
			//init DB session
			...
			if (engine.createTable()){
				try{
					if (useDB)
						DatabaseFactory.executeSQLUpdate(String.format(createTableStatement,default_hspec_suitable_table,engine.getDatabaseUserName()), vreConnection);
				}catch(Exception e){
...
//		AnalysisLogger.getLogger().warn("Distribution Generator ->Engine Initialized");
	...
	public void generateHSPEC() throws Exception{
			...
			if ((""+generatorAlgorithm).startsWith("REMOTE")){
				...
				AnalysisLogger.getLogger().warn("generateHSPEC -> Required Algorithm is of Remote Type ");
				isRemoteGeneration = true;
				generateRemoteHSPEC();
			}
			else{
				AnalysisLogger.getLogger().warn("generateHSPEC -> Required Algorithm is of Standalone Type ");
	...
	public void generateRemoteHSPEC() throws Exception{
		...
		rhio.hspecDestinationTableName = default_hspec_suitable_table;
	...
	public void generateStandaloneHSPEC() throws Exception {
		...
		//INITIALIZATION
		AnalysisLogger.getLogger().warn("Distribution Generator->populating species");
		populateSelectedSpecies();
		AnalysisLogger.getLogger().warn("Distribution Generator->getting squares");
		//get all csquares
		getCsquares();
		AnalysisLogger.getLogger().warn("Distribution Generator->getting all species information");
...
//			System.exit(0);
	...
	private class ThreadCalculator implements Callable<Integer> {
		...
		String species;
		DistributionGeneratorAsynchronousWriting aag;
		public ThreadCalculator(DistributionGeneratorAsynchronousWriting aag,int index, String species,int start) {
			this.index = index;
	...
	public static void main(String[] args) throws Exception {
		...
		e.setConfigPath("./cfg");
		DistributionGeneratorAsynchronousWriting aag = new DistributionGeneratorAsynchronousWriting(null);
		aag.generateHSPEC();
	}
	...
	//THREAD SECTION
	//definition of the Thread
	private class ThreadInserter implements Runnable {
		public ThreadInserter() {
		}

		public void run(){
			AnalysisLogger.getLogger().warn("ThreadInserter - Insertion Thread Initialized");
			try{
			while ((globalstatus<100)&&(isrunning)){
				insertCriteriaQueue();
				Thread.sleep(500);
			}
			insertCriteriaQueue();
				...
				e.printStackTrace();
				AnalysisLogger.getLogger().warn("ThreadInserter - ERROR IN WRITER THREAD - EXITING");
			}
			
			//shutdown connection
			shutdownConnection();
			
			AnalysisLogger.getLogger().warn("ThreadInserter - INSERTION WRITING HAS FINISHED");
		}
	}
}
