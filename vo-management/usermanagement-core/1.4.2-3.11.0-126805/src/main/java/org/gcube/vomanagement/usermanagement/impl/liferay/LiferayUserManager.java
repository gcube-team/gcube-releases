...
public class LiferayUserManager implements UserManager {
	...
	public boolean userExistsByEmail(String emailAddress) {
		...
		try {
			long companyId = gm.getCompanyId();
			if(UserLocalServiceUtil.getUserByEmailAddress(companyId, emailAddress) != null)
				return true;
		} catch (Exception e){
	...
	private GCubeUser createUserBody(boolean autoScreenName, String username,
		...
		try {
			_log.debug("Trying create user " + email);
	...
	private static GCubeUser mapLRUser(User u) throws PortalException, SystemException {
		if (u != null) {
			...
			emails.add(new Email(email, null, true));
	...
	public boolean isPasswordChanged(String emailAddress) {
		...
		LiferayGroupManager gm = new LiferayGroupManager();
		try {
			long companyId = gm.getCompany().getCompanyId();
	...
	public void deleteUserByEMail(String email) throws UserManagementSystemException, UserManagementPortalException, PortalException, SystemException {
		...
		long companyId = gm.getCompanyId();
		User user;
		if((user = UserLocalServiceUtil.getUserByEmailAddress(companyId, email))!= null){
	...
	public byte[] getUserAvatarBytes(String screenName) {
		try {
			...
			return ImageServiceUtil.getImage(user.getPortraitId()).getTextObj();
		}catch(Exception e){
			_log.error("Unable to retrieve user's avatar", e);
		}
		return null;
	}
	...
	public String getUserOpenId(String screenName) {
		try {
			LiferayGroupManager gm = new LiferayGroupManager();
			long companyId = gm.getCompanyId();
			return UserLocalServiceUtil.getUserByScreenName(companyId, screenName).getOpenId();
		}catch(Exception e){
			_log.error("Unable to retrieve user's OpenId", e);
		}
		return null;
	}
