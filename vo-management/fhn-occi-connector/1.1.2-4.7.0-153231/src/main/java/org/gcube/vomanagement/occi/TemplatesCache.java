...
public class TemplatesCache {
  ...
  private static final int CACHE_TTL = 864000; // 10 days
  ...
  static {
    ...
    cacheManager = CacheManagerBuilder.newCacheManagerBuilder()
          //.with(CacheManagerBuilder.persistence(FHN_CONNECTOR_CACHE_LOCATION))
          .build(true);

    ResourcePoolsBuilder osTemplateCachePoolBuilder =
        ResourcePoolsBuilder.newResourcePoolsBuilder()
        .heap(CACHE_SIZE);


    osTemplatesCache = cacheManager.createCache("OS_TEMPLATES",
        CacheConfigurationBuilder.newCacheConfigurationBuilder(String.class, OSTemplate.class, osTemplateCachePoolBuilder)
            .withExpiry(Expirations.timeToLiveExpiration(Duration.of(CACHE_TTL, TimeUnit.SECONDS)))
            .build());


    ResourcePoolsBuilder resourceTemplateCachePoolBuilder =
        ResourcePoolsBuilder.newResourcePoolsBuilder()
            .heap(CACHE_SIZE);

    resourceTemplatesCache = cacheManager.createCache("RESOURCE_TEMPLATES",
        CacheConfigurationBuilder.newCacheConfigurationBuilder(String.class, ResourceTemplate.class, resourceTemplateCachePoolBuilder)
            .withExpiry(Expirations.timeToLiveExpiration(Duration.of(CACHE_TTL, TimeUnit.SECONDS)))
            .build());
  }
  ...
  public void cache(String providerId, OSTemplate template) {
    this.osTemplatesCache.put(providerId + template.getId(), template);
  }

  public void cache(String providerId, ResourceTemplate template) {
    this.resourceTemplatesCache.put(providerId + template.getId(), template);
  }

  public OSTemplate getOSTemplate(String providerId, String templateId) {
    return this.osTemplatesCache.get(providerId + templateId);
  }

  public ResourceTemplate getResourceTemplate(String providerId, String tid) {
    return this.resourceTemplatesCache.get(providerId + tid);
  }
