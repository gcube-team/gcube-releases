...
public class UsernamePasswordAuthenticationService extends AuthenticationInternalServiceImpl implements AuthenticationInternalService 
{
	...
	protected AccessControlBean askToSOA3(String id, long currentTime) 
	{
		...
		if (response != null) 
		{
			...
			removeInvalidRoles(roles);
			
			if (roles.size()>0) 
			{
				log.debug("trying to load roles from response");
				bean.setRolesLoaded(true);
				bean.getRoles().addAll(response.getRoles());
				log.debug("Roles loaded");
			}
			else log.debug("No roles found in the response");
	...
	private void removeInvalidRoles (List<String> roles)
	{
		if (roles!=null)
		{
			log.debug("Checking for invalid roles");
			
			List<String> internalRoles = new ArrayList<String>(roles);
			
			for (String ir : internalRoles)
			{
				if (ir.trim().length() == 0)
				{
					log.debug("Found invalid role");
					roles.remove(ir);
				}
			}
