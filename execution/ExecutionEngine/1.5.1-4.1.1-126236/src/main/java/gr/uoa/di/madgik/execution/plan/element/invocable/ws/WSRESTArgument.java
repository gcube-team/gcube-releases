...
public class WSRESTArgument extends ArgumentBase
{
...
//				this.ArgumentNamespace.getLocalPart().trim().length()==0)) throw new ExecutionValidationException("Needed invocation argument namespace info not provided");
	...
	public String ToXML() throws ExecutionSerializationException
	{
		...
		if(this.ArgumentName==null || this.ArgumentName.trim().length()==0) 
			buf.append("<argument type=\""+this.GetArgumentType().toString()+"\" order=\"" + this.Order + "\">");
		else 
	...
	public void FromXML(Node XML) throws ExecutionSerializationException
	{
		try
		{
			...
			if(!XMLUtils.AttributeExists((Element)XML, "name")) this.ArgumentName=null;
			else this.ArgumentName = XMLUtils.GetAttribute((Element) XML, "name");
