...
public class SearchService extends RRElement
{
	private static final Logger logger = LoggerFactory
			.getLogger(SearchService.class);
	
	
	private SearchServiceDao item = new SearchServiceDao();
	...
	public SearchService() throws ResourceRegistryException
	{
	...
	@Override
	public SearchServiceDao getItem()
	{
		return this.item;
	}
	...
	{
		if(!(target instanceof SearchService)) throw new ResourceRegistryException("cannot apply to target of "+target);
		if(this.isEqual(target,applyDetails)) return;
		this.item.setID(((SearchService)target).item.getID());
		this.item.setFunctionality(((SearchService)target).item.getFunctionality());
		this.item.setHostingNode(((SearchService)target).item.getHostingNode());
		this.item.setEndpoint(((SearchService)target).item.getEndpoint());
		this.item.setScopes(((SearchService)target).item.getScopes());
	...
	public boolean load(boolean loadDetails, RRContext.DatastoreType persistencyType, boolean overridePrefetched) throws ResourceRegistryException
	{
		...
		try
		{
			if(!overridePrefetched && persistencyType.equals(DatastoreType.LOCAL) && context.isTargetInMemory(this.getClass().getName()))
			{
				...
				{
					SearchService retrieved = (SearchService)InMemoryStore.getItem(this.getClass(), this.getID());
			...
			this.item=pm.detachCopy(pm.getObjectById(SearchServiceDao.class, this.item.getID()));
	...
	public void delete(boolean deleteDetails, RRContext.DatastoreType persistencyType) throws ResourceRegistryException
	{
		...
		SearchService f=new SearchService();
		f.setID(this.getID());
	...
	private void store(boolean storeDetails, RRContext.DatastoreType persistencyType, boolean writeThrough) throws ResourceRegistryException
	{
		...
		{
			SearchService item=new SearchService();
	...
	{
		if(!(target instanceof SearchService)) throw new ResourceRegistryException("cannot apply to target of "+target);
		if(this.item.getID() == null && ((SearchService)target).item.getID()!=null) return false;
		if(this.item.getID() != null && ((SearchService)target).item.getID()==null) return false;
		if(this.item.getID() != null && ((SearchService)target).item.getID()!=null && !this.item.getID().equals(((SearchService)target).item.getID())) return false;
		if(this.item.getFunctionality() == null && ((SearchService)target).item.getFunctionality()!=null) return false;
		if(this.item.getFunctionality() != null && ((SearchService)target).item.getFunctionality()==null) return false;
		if(this.item.getFunctionality() != null && ((SearchService)target).item.getFunctionality()!=null && !this.item.getFunctionality().equals(((SearchService)target).item.getFunctionality())) return false;
		if(this.item.getEndpoint() == null && ((SearchService)target).item.getEndpoint()!=null) return false;
		if(this.item.getEndpoint() != null && ((SearchService)target).item.getEndpoint()==null) return false;
		if(this.item.getEndpoint() != null && ((SearchService)target).item.getEndpoint()!=null && !this.item.getEndpoint().equals(((SearchService)target).item.getEndpoint())) return false;
		if(this.item.getHostingNode() == null && ((SearchService)target).item.getHostingNode()!=null) return false;
		if(this.item.getHostingNode() != null && ((SearchService)target).item.getHostingNode()==null) return false;
		if(this.item.getHostingNode() != null && ((SearchService)target).item.getHostingNode()!=null && !this.item.getID().equals(((SearchService)target).item.getHostingNode())) return false;
		if(this.item.getScopes().size()!=((SearchService)target).item.getScopes().size()) return false;
		if(!((SearchService)target).item.getScopes().containsAll(this.item.getScopes())) return false;
		return true;
	}
	...
	public boolean exists(RRContext.DatastoreType persistencyType) throws ResourceRegistryException
	{
		...
		try
		{
			...
			query=pm.newNamedQuery(SearchServiceDao.class, "exists");
			query.compile();
	...
	public static List<SearchService> getAll(boolean loadDetails) throws ResourceRegistryException
	{
		Lock sharedLock = ResourceRegistry.getContext().getSharedLock();
		boolean locked = false;
		PersistenceManager pm = null;
		Query query = null;
		List<SearchService> col=new ArrayList<SearchService>();
		try {
			if(ResourceRegistry.getContext().isTargetInMemory(SearchService.class.getName()))
			{
				sharedLock.lock(); locked=true;
				Set<IRRElement> retrieved = InMemoryStore.getItems(SearchService.class);
				sharedLock.unlock(); locked=false;
				if(retrieved != null)
				{
					for(IRRElement item : retrieved)
						col.add((SearchService)item);
					return col;
				}
			}
			pm = ResourceRegistry.getContext().getManagerForRead(RRContext.DatastoreType.LOCAL);
			query=pm.newNamedQuery(SearchServiceDao.class, "all");
			query.compile();
			sharedLock.lock(); locked = true;
