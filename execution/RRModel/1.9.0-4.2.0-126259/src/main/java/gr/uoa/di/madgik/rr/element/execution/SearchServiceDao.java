...
@PersistenceCapable(table="SEARCHSERVICE", detachable="true")
@Queries(
		{@Query(
				name="all", 
				language="JDOQL", 
				value="SELECT this.ID FROM gr.uoa.di.madgik.rr.element.execution.SearchServiceDao"
				),
		@Query(
				name="exists", 
				language="JDOQL", 
				value="SELECT this.ID FROM gr.uoa.di.madgik.rr.element.execution.SearchServiceDao WHERE this.ID == :id"
				),
		}
		
)
public class SearchServiceDao implements IDaoElement 
{
	@PrimaryKey
	...
	{
		if(!(target instanceof SearchServiceDao)) throw new ResourceRegistryException("cannot apply to target of "+target);
		this.setID(((SearchServiceDao)target).getID());
		this.setFunctionality(((SearchServiceDao)target).getFunctionality());
		this.setHostingNode(((SearchServiceDao)target).getHostingNode());
		this.setEndpoint(((SearchServiceDao)target).getEndpoint());
		this.setScopes(((SearchServiceDao)target).getScopes());
	}
	...
	public String toXML()
	{
		...
		buf.append("<searchService id=\""+this.getID()+"\">");
	...
	{
		StringBuilder buf=new StringBuilder();
		buf.append("SearchService endpoint : "+this.endpoint);
		return buf.toString();
	}
}
