 ...
 * true or false is returned. True is returned in case the input is {@link JobStatusExternalFilter.JobStatus#Ready}.
 * 
 * @author gpapanikos
 */
public class JobStatusSuccessFilter implements IExternalFilter
{
	
	/** The logger. */
	private static Logger logger=LoggerFactory.getLogger(JobStatusSuccessFilter.class);
	...
	public Object Process(ExecutionHandle Handle) throws ExecutionRunTimeException
	{
		try
		{
			...
			switch(s)
			{
				...
				case Other:
				{
					logger.debug("returning true, status=" + s.toString());
					return true;
				}
				case Ready:
				case Running:
				...
				case Waiting:
				{
					logger.debug("returning false, status=" + s.toString());
					return false;
				}
				default:
				{
