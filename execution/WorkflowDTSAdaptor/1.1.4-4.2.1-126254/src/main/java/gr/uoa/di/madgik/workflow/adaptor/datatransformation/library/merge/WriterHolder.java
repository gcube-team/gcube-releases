...
public class WriterHolder {
	...
	public WriterHolder() throws Exception {
		...
		writer = new RecordWriter<GenericRecord>(new TCPWriterProxy(), defs);
	}
...
//			throw new Exception("Merger's resultSet has closed unexpectendly.");
		...
		try {
			...
			log.warn("Could not add " + uri + " to merger: " + getLocator(), e);
		}

		log.info("Subplan Succesfully added " + uri + " to merger" + getLocator());
	}
