package gr.uoa.di.madgik.workflow.adaptor.hive.test;
...
import gr.uoa.di.madgik.workflow.adaptor.hive.WorkflowHiveQLAdaptor;
 ...
 * 
 */
public class WorkflowHiveQLAdaptorTester {
	private static Logger log = LoggerFactory.getLogger(WorkflowHiveQLAdaptorTester.class.getName());
	...
	public static void init() throws Exception {
		// List<PortRange> ports=new ArrayList<PortRange>(); //The ports that
		// the TCPConnection manager should use
		// ports.add(new PortRange(6050, 6100));
		// TCPConnectionManager.Init(
		// new
		// TCPConnectionManagerConfig(InetAddress.getLocalHost().getHostName(),
		// //The hostname by which the machine is reachable
		// ports, //The ports that can be used by the connection manager
		// true //If no port ranges were provided, or none of them could be
		// used, use a random available port
		// ));
	...
	public static void test(String dist, String file, String script, long waitTime) throws Exception {
		WorkflowHiveQLAdaptor adaptor = new WorkflowHiveQLAdaptor();

		String line = HiveParserTest.getLine(dist, file, script);
		HiveQLPlanner planner = new HiveQLPlanner();
		planner .processLine(line);
		PlanNode plan = OperatorAnalyzer.optimizePlan(planner.getCreatedPlan());
		
//		PlanNode plan = HiveParserTest.createPlan();

		log.info("Operator plan:\n"+plan.toString());
...
//		GRS2Printer.print(new URI(output), 6);
//		GRS2Printer.compute();
		System.out.println("Waiting...");
...
//		deleteFolder(new File(output));
	...
	public static void test() throws Exception {
		...
		String line = HiveParserTest.getSimpleLine();
		HiveQLPlanner planner = new HiveQLPlanner();
		planner .processLine(line);
		PlanNode plan = planner.getCreatedPlan();
...
//		System.out.println("Enough waiting...");
	...
	//
	// WorkflowHiveAdaptor adaptor = new WorkflowHiveAdaptor();
	// TransformationDescription desc = new TransformationDescription(input,
	// output, false);
	//
	// adaptor.setPlanDesc(desc);
	...
	// adaptor.ExecutePlan();
	//
	// ContentType tiffCT = new ContentType("image/tiff", new
	// ArrayList<Parameter>());
	// ContentType jpegCT = new ContentType("image/jpeg", new
	// ArrayList<Parameter>());
	// ContentType gifCT = new ContentType("image/gif", new
	// ArrayList<Parameter>());
	// ContentType pngCT = new ContentType("image/png", new
	// ArrayList<Parameter>());
	// {
	// ArrayList<TransformationUnit> tus = new ArrayList<TransformationUnit>();
	// ArrayList<ContentType> cts = new ArrayList<ContentType>();
	//
	// TransformationsGraphImpl graph = null;
	// tus.add(graph.findApplicableTransformationUnits(tiffCT, pngCT,
	// true).get(0));
	// cts.add(pngCT);
	//
	// tus.add(graph.findApplicableTransformationUnits(pngCT, gifCT,
	// true).get(0));
	// cts.add(gifCT);
	//
	...
	// }
	// // {
	// // ArrayList<TransformationUnit> tus = new
	// ArrayList<TransformationUnit>();
	// // ArrayList<ContentType> cts = new ArrayList<ContentType>();
	// //
	// // tus.add(graph.findApplicableTransformationUnits(jpegCT, pngCT,
	// true).get(0));
	// // cts.add(pngCT);
	// //
	// //// TransformationUnitConverter conv = new
	// TransformationUnitConverter();
	// //// System.out.println(conv.Convert(tus.get(0)));
	// // adaptor.addPlan(tus, cts);
	// // }
	// // {
	...
	// // ArrayList<ContentType> cts = new ArrayList<ContentType>();
	// //
	// // tus.add(graph.findApplicableTransformationUnits(gifCT, pngCT,
	// true).get(0));
	// // cts.add(pngCT);
	// //
	// //// TransformationUnitConverter conv = new
...
//			 System.out.println("Initializing");init();
			for (int i = 1; i <= 1; i++)
				for (int j = 1; j <= 1; j++) {
//				System.out.println("Testing with " + i + " nodes. round: " + j);
//				test("1", "/home/jgerbe/Desktop/datasets/wiki/multi/input.dat.10", "/home/jgerbe/Desktop/datasets/wiki/multi/counter.py", 200);
