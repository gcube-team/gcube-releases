...
public class TextToGeometrySQLHandler extends TypeTransitionSQLHandler {

	public TextToGeometrySQLHandler(SQLExpressionEvaluatorFactory evaluator) {
		super(evaluator);
	}
	...
	private String generateTypedColumnNameSnippet(Table newTable, Table targetTable, Column targetColumn, ValueFormat format) throws MalformedExpressionException {
		...
		for (Column column : newTable.getColumns()) {
			if (column.getName().equals(targetColumn.getName())) {
				sb.append(String.format("ST_GeomFromText(%s)", this.convert(format, targetTable.getId(), targetColumn)));
			} else
