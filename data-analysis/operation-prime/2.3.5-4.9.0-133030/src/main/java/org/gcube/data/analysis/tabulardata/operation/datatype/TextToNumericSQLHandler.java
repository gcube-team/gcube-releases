...
public class TextToNumericSQLHandler extends TypeTransitionSQLHandler {

	public TextToNumericSQLHandler(SQLExpressionEvaluatorFactory evaluator) {
		super(evaluator);
	}
	...
	private String generateTypedColumnNameSnippet(Table newTable, Table targetTable, Column targetColumn, ValueFormat format) throws MalformedExpressionException {
		...
		for (Column column : newTable.getColumns()) {
			if (column.getName().equals(targetColumn.getName())) {
				sb.append(String.format("%s::numeric", this.convert(format, targetTable.getId(), targetColumn)));
			} else
