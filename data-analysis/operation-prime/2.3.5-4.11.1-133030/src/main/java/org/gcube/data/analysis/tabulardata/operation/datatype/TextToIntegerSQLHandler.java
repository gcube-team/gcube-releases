...
public class TextToIntegerSQLHandler extends TypeTransitionSQLHandler {
	...
	public TextToIntegerSQLHandler(SQLExpressionEvaluatorFactory evaluator) {
		super(evaluator);
	}
	...
	private String generateTypedColumnNameSnippet(Table newTable, Table targetTable, Column targetColumn,  ValueFormat format) throws MalformedExpressionException{
		...
		for (Column column : newTable.getColumns()) {
			if (column.getName().equals(targetColumn.getName())) {
				sb.append(String.format("%s::integer", this.convert(format, targetTable.getId(), targetColumn)));
			} else
