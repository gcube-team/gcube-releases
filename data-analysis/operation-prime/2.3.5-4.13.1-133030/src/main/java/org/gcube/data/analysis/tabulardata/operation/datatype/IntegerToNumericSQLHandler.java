...
public class IntegerToNumericSQLHandler extends TypeTransitionSQLHandler {

	public IntegerToNumericSQLHandler(SQLExpressionEvaluatorFactory evaluator) {
		super(evaluator);
	}
	...
	private String generateTypedColumnNameSnippet(Table newTable, Column targetColumn) {
		...
		for (Column column : newTable.getColumns()) {
			if (column.getName().equals(targetColumn.getName())) {
				sb.append(String.format("%s::numeric", column.getName()));
			} else
