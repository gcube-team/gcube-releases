...
public class RuntimeResourceManager {
	...
	public static AccessPointDescriptor getDatabaseProfile(DatabaseType type) throws StatisticalManagerException,Exception{
		logger.debug("Get Access point for "+type+" under scope"+ScopeProvider.instance.get());
		...
		if (resources == null || resources.isEmpty())
			throw new StatisticalManagerException("Runtime resource "+ rrName + " not found");
		...
		if (resource.profile().accessPoints() == null|| resource.profile().accessPoints().isEmpty())
			throw new StatisticalManagerException("No AccessPoint defined in "+ rrName + " not found");
		...
		throw new StatisticalManagerException("AccessPoint not found for "+type);
	}


	public static HashMap<String, String> getServiceEndpointAsMap(String runtimeResource)
			throws Exception {
