...
public class DataMinerUpdater {
	...
	public static void main(String args[]) throws Exception {
		...
		Update(algorithmName, implementation, category, configPath, libPath, scope, description, atype, skipJava, url);
	}
	...
	public static void Update(String algorithmName, String implementation, String category, String configPath, String applicationlibs, String scope, String description, String atype, String skipJava, String url) throws Exception {
		BufferedReader br;
		String line;
		...
		indexOnIS(algorithmName, description, scope);
		...
		System.out.println("| "+ algorithmName+" | "+
				"system" +" | "+ category +"| Prod | <notextile>./addAlgorithm.sh "+ 
				algorithmName+ " " +category+" "+implementation+" "+
				scope+" "+atype+" "+(skipJava!=null?"N":"Y")+" "+
				((url!=null && url.length()>1)?url:"k")+" \""+description+"\""+" </notextile> | none |");
	...
	private static void compileJava(String javaFileName) throws Exception{
		...
		if (!success) throw new Exception("error compiling generated class");
	...
	public static void indexOnIS(String algorithmName, String algorithmDescription, String scope) throws Exception {
		System.out.println("setting scope to "+scope);

		InfrastructureDialoguer id = new InfrastructureDialoguer(scope);
		ScopeProvider.instance.set(scope);
		List<String> algorithms = id.getAlgorithmsInScope();
		boolean found = false;
		...
		if (found){
			System.out.println("Algorithm already present in the scope");
			return;
		}
		/*
		if (algorithms.indexOf(algorithmName) >= 0) {
			System.out.println("Algorithm already present in the scope");
			return;
		}
		 */
		System.out.println("Algorithm is not present in the scope-Adding");

		String xml = FileTools.loadString("algorithmTemplate", "UTF-8");


		String secondaryType = "StatisticalManagerAlgorithm";
