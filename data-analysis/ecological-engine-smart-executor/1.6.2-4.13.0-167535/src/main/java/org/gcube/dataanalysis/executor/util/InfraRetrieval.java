...
//		ScopeProvider.instance.set(scope);
	...
	public static String findEmailingSystemAddress(String scope) throws Exception {
		...
		List<String> endpoints  = retrieveService(serviceName,scope);
		...
		if (endpoints == null || endpoints.isEmpty()) throw new Exception("Cannot retrieve the GCoreEndpoint serviceName: "+serviceName +", in scope: "+scope);
		
		if(resourceEntyName==null)
			throw new Exception("Endpoint:"+serviceName+", is null for serviceName: "+serviceName +", in scope: "+scope);
