...
public class DBObjectTranslator {
	...
	public BigInteger calculateTotalEntries(SessionFactory dbSession, String timeSeriesName, String timeSeriesColumn) {
		...
		for (Object result : resultSet) {
			...
			try {
				...
				count = resultcount;
				LexicalLogger.getLogger().trace("DBObjectTranslator->calculateTotalEntries: Time Series " + timeSeriesName + " total " + totalEntries);
			} catch (Exception e) {
			}
		}
	...
	public ArrayList<String> retrieveTimeSeriesEntries(SessionFactory dbSession, String timeSeriesName, String timeSeriesColumn, BigInteger min, int numberOfElements) {
		...
		LexicalLogger.getLogger().trace("DBObjectTranslator->query: " + query);
		...
		for (Object result : resultSet) {
			try {
				...
				e.printStackTrace();
				LexicalLogger.getLogger().trace("DBObjectTranslator->retrieveTimeSeriesEntries: Error in adding entry :" + e.getLocalizedMessage());
			}
		}

		LexicalLogger.getLogger().trace("DBObjectTranslator->retrieveColumnRange: Column " + column.toString());
	...
	public ArrayList<Entry> retrieveEntries(SessionFactory dbSession, String timeSeriesName, BigInteger min, int numberOfElements) {
		...
		if (descriptions.size() > 0) {
			...
			LexicalLogger.getLogger().trace("DBObjectTranslator->retrieveEntries: query " + query);
			...
			for (Object result : resultSet) {
				...
				try {
					...
					// e.printStackTrace();
					LexicalLogger.getLogger().trace("DBObjectTranslator->retrieveEntries: Error in adding entry :" + e.getLocalizedMessage());
				}
			}
		}
...
//		AnalysisLogger.getLogger().trace("DBObjectTranslator->retrieveEntries: Entries " + currentEntries);
	...
	public void buildRelationsEdges(SessionFactory dbSession) {
		...
		for (Object result : resultSet) {
			...
			if (re != null) {
				relations.add(re);
				LexicalLogger.getLogger().trace("DBObjectTranslator->buildRelationsEdges: add relation " + re.toString());
			}
		}
	}
	...
	public void buildCategories(SessionFactory dbSession, String referenceTable, String referenceColumn, String idColumn, String nameHuman, String description) {
		...
		if (resultSet != null) {
			for (Object result : resultSet) {
				...
				if (cat != null) {
					categories.add(cat);
					LexicalLogger.getLogger().trace("DBObjectTranslator->buildCategories: add category " + cat.toString());
				}
			}
		}
	}
	...
	public void populateRelationWithCategories() {
		...
		for (RelationEdge re : relations) {
			...
			LexicalLogger.getLogger().trace("DBObjectTranslator->populateRelationWithCategories: modified Relation " + re.toString());
		}
	}
		...
		for (RelationEdge re : relations) {
			...
			for (Object result : resultSet) {
				...
				try {
					...
					LexicalLogger.getLogger().trace("DBObjectTranslator->calculateRelationWeights: Relation " + re.getName() + " weight " + re.getWeigth());
				} catch (Exception e) {
				}
			}
		}
		...
		for (Category cat : categories) {
			...
			for (Object result : resultSet) {
				...
				try {
					...
					LexicalLogger.getLogger().trace("DBObjectTranslator->calculateCategoriesWeights: Category " + cat.getName() + " weight " + cat.getNumberOfElements() + " total " + totalCatElements);
				} catch (Exception e) {
				}
			}
		}
	...
	public void buildCategoriesStructure(SessionFactory dbSession, String referenceTable, String referenceColumn, String idColumn, String nameHuman, String description) {
		...
		LexicalLogger.getLogger().trace("DBObjectTranslator->buildWholeStructure: Total Categories Elements " + totalCatElements + " Total Relation Elements " + totalRelationElements);
	}
