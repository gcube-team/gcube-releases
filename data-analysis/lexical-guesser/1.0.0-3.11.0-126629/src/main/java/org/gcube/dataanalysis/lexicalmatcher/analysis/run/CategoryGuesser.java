...
import org.gcube.dataanalysis.lexicalmatcher.analysis.guesser.data.CategoryOrderedList;
...
public class CategoryGuesser {
	...
	public static void showResults(ArrayList<SingleResult> results) {
		...
		LexicalLogger.getLogger().warn("CLASSIFICATION RESULT:\n");
		int i = 1;
		for (SingleResult result : results) {
			if (result.getColumn() != null)
				LexicalLogger.getLogger().warn(i + ": " + result.getCategory() + " - " + result.getColumn() + " ; SCORE: " + result.getStringScore() + "%");
			else
				LexicalLogger.getLogger().warn(i + ": " + result.getCategory() + " ; SCORE: " + result.getStringScore() + "%");
	...
	public static void AccuracyCalc(LexicalEngineConfiguration externalcfg, CategoryGuesser guesser, String configPath, String seriesName, String column, int attempts, String correctFamily, String correctColumn) throws Exception {
		...
		for (int i = 0; i < attempts; i++) {
			...
			LexicalLogger.getLogger().info("CLASSIFICATION RESULT " + result + " " + CategoryGuesser.resultString(result, correctFamily, correctColumn));
		...
		LexicalLogger.getLogger().info("->ACCURACY ON FAMILY " + correctFamily + ":" + percFamily + " ACCURACY ON COLUMN " + correctColumn + ":" + percColumn);
	}
...
//		AnalysisLogger.setLogger(LogFile);
		...
		LexicalLogger.getLogger().trace("******************INITIALIZING******************");
		...
		if (col == null) {
			LexicalLogger.getLogger().trace("******************Order Category******************");
			if (externalConfig == null)
			...
			LexicalLogger.getLogger().trace("***************End Ordering********************");
...
//		AnalysisLogger.setLogger(LogFile);
		...
		LexicalLogger.getLogger().debug("Guessing Table " + seriesName + " column " + columnName);
		if (externalConfig != null) {
			config = new LexicalEngineConfiguration();
			config.configureByStream(cfgFileCompletePath);
			config.mergeConfig(externalConfig);
...
//		if (col == null) {
		...
		LexicalLogger.getLogger().warn("Starting Calculation...wait...");
		...
		while ((checkingResults == null || checkingResults.size() == 0) && (triesCounter < maxTriesClassification)) {
			LexicalLogger.getLogger().warn("..another processing pass is required. Attempt number " + (triesCounter + 1));
			triesCounter++;
			...
			LexicalLogger.getLogger().trace("Performing next processing pass");
			runGuesser(seriesName, columnName, externalConfig, CategoryFilter, ColumnFilter, SingletonString);
			LexicalLogger.getLogger().debug("End processing pass");
		...
		LexicalLogger.getLogger().warn("...End Calculation in " + t1 + "ms");
...
*/
		for (int i = 0; i < size; i++) {
			...
			LexicalLogger.getLogger().warn("WEIGHT FOR CATEGORY " + processor.bestCategories.get(i) + "-" + processor.bestColumns.get(i) + " : " + weight + " SCORE " + score);
