...
public class EndpointProvider {
	...
	public EndpointProvider(String scope) {
		if(scheduler != null)
			try { logger.info("Shutting down scheduler"); scheduler.shutdown();	} catch (SchedulerException e) {e.printStackTrace();}
		else{
			this.scope = scope;
			...
			try {
				...
				logger.error("Index client could not enable its smart update feature of the service endpoints of scope: "+scope, e);
			}
			endpoints.addAll(EndpointsHelper.getEndpointsOfScope(scope));
		}
	}
	...
	public String getAnEndpoint() throws IndexDiscoverException {
		if(endpoints.isEmpty())
			throw new IndexDiscoverException("There's no available service endpoint on scope "+scope);
