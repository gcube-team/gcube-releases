...
Workspace {
	...
	WrongDestinationException, WorkspaceFolderNotFoundException, ItemAlreadyExistException, InternalErrorException {
		...
		try{
			...
			if (!lm.isLocked(itemId) && !lm.isLocked(destinationFolderId)){
				...
				//						logger.debug("Found common path " + commonPath + ". It has been locked");
				//					else
				//						logger.debug("item " + commonPath + " cannot be locked");
				//				}else{
				...
				//						logger.debug("destinationFolder " + destinpath + " has been locked");
				//					else
				//						logger.debug("destinationFolder " + destinpath + " cannot be locked");
	...
	public List<WorkspaceItem> searchByProperties(SearchQuery queryString)
		...
		for (ItemDelegate node: itemDelegateList) {
			try {
				JCRWorkspaceItem item = getWorkspaceItem(node);
				if (!JCRPrivilegesInfo.canReadNode(getOwner().getPortalLogin(), getOwner().getPortalLogin(), item.getAbsolutePath())) 
						continue;
				list.add(item);		
			} catch (RepositoryException e) {
				try {
