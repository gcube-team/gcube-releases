...
public class JCRGroup implements GCubeGroup{
	...
	public List<String> getMembers() throws InternalErrorException {
		...
		try {
			...
			getMethod =  new GetMethod(url + "/GroupMembershipServlet?" + JCRRepository.getCredentials() + "&groupName=" + name);
...
//				logger.debug("URL: " + url + "/UpdateGroupServlet?groupName=" + name + listMembers);
				getMethod =  new GetMethod(url + "/UpdateGroupServlet?" + JCRRepository.getCredentials() + "&groupName=" + name + listMembers);
			}
...
//				logger.debug("URL: " + url + "/UpdateGroupServlet?groupName=" + name + listMembersToDelete);
				getMethod =  new GetMethod(url + "/UpdateGroupServlet?" + JCRRepository.getCredentials() + "&groupName=" + name + listMembersToDelete);
			}
	...
	public String getDisplayName() throws InternalErrorException {
		...
		try {
			...
			HttpClient httpClient = new HttpClient();            
			getMethod =  new GetMethod(url + "/GetDisplayNameServlet?" + JCRRepository.getCredentials() + "&groupName=" + name);
			httpClient.executeMethod(getMethod);
	...
	public boolean setDisplayName(String displayName) throws InternalErrorException {
		...
		try {
			...
			HttpClient httpClient = new HttpClient();            
			getMethod =  new GetMethod(url + "/SetDisplayNameServlet?" + JCRRepository.getCredentials() + "&groupName=" + name + "&displayName=" + displayName);
			httpClient.executeMethod(getMethod);
