...
public class JCRWorkspaceMessage implements WorkspaceMessage {
	...
	public JCRWorkspaceMessage(JCRWorkspace workspace, ItemDelegate messageItem, WorkspaceMessageType type) throws RepositoryException {
		...
		this.type = type;
		this.isRead = (Boolean) new XStream().fromXML(messageItem.getProperties().get(NodeProperty.READ));
		this.isOpened = (Boolean) new XStream().fromXML(messageItem.getProperties().get(NodeProperty.OPEN));
	}
	...
	public JCRWorkspaceMessage(JCRWorkspace workspace, ItemDelegate messageItem, WorkspaceMessageType type, String messageId, String subject, String body,
			User sender, List<String> attachmentIds, List<String> addresses, String user) throws  RepositoryException,
			...
			for(String attachmentId : attachmentIds) {
				...
				if(item.getType().equals(WorkspaceItemType.FOLDER_ITEM)){
					try {
						...
						ItemDelegate newNode = ((JCRWorkspaceItem)item).internalCopy(servlets, servlets.getItemById(rootAttachmentsId), nodeItem.getName(), false);
						logger.info("Set owner of attachment " + nodeItem.getName() + " to user " + user +  " - " + type.toString() );
