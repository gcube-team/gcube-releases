...
Workspace {
	...
	private ItemDelegate createItemDelegate(JCRSession session, ItemDelegate parent, String name, String nodeType) throws ItemAlreadyExistException,
	WorkspaceFolderNotFoundException, InternalErrorException, WrongDestinationException, InsufficientPrivilegesException {
		...
		//check ACL
		if(parent.isShared()){
			if (!JCRPrivilegesInfo.canAddChildren(parent.getOwner(), getOwner().getPortalLogin(), parent.getPath()))
				throw new InsufficientPrivilegesException("Insufficient Privileges to add the node");
		}
	...
	public WorkspaceFolder getRoot() {
		...
		try {
			...
			logger.info("Root WorkspaceFolder not found. It will be created");
	...
	public WorkspaceItem moveItem(String itemId, String destinationFolderId)
		...
		try{
			...
			if(nodeItem.isShared()){
				...
				if (!JCRPrivilegesInfo.canModifyProperties(nodeItem.getOwner(), getOwner().getPortalLogin(), nodeItem.getPath(), false)) 
			...
			if(nodeDestination.isShared()){
				...
				if (!JCRPrivilegesInfo.canAddChildren(nodeDestination.getOwner(), getOwner().getPortalLogin(), nodeDestination.getPath()))
					throw new InsufficientPrivilegesException("Insufficient Privileges to add the node");
			}
			...
			try {
				...
				if (!lm.isLocked(itemId) && !lm.isLocked(destinationFolderId)){
					...
					try{
						...
						logger.error("Error setting CUT accounting entry to folder parent item ID " + parentItem.getId());
					}
					...
					try{
						...
						logger.error("Error setting ADD accounting entry to item ID " + destinationItem.getId());
					}
					...
					try{
						...
						logger.error("Error setting PASTE accounting entry to item ID "+ item.getId());
					}
				}
				else
	...
	public void renameItem(String itemId, String newName)
		...
		try{
			...
			if(itemDelegate.isShared()){
				...
				if (!JCRPrivilegesInfo.canModifyProperties(itemDelegate.getOwner(), getOwner().getPortalLogin(), itemDelegate.getPath(), false)) 
	...
	public WorkspaceItem getItem(String itemId) throws ItemNotFoundException {
		...
		try {
			...
			if (!JCRPrivilegesInfo.canReadNode(item.getOwner().getPortalLogin(), getOwner().getPortalLogin(), item.getAbsolutePath())) 
				throw new ItemNotFoundException("Insufficient Privileges to READ the node " + item.getAbsolutePath() + " for user " + getOwner().getPortalLogin());
	...
	//						}	
	//					}
	//					//					sharedFoldere.setACL(users, ACLType.WRITE_OWNER);
	//				}else{
	...
	//					sharedFoldere.save();
	//					try {
	//						session.move(nodeToShare.getPath(), repository.getSharedRoot().getPath() + getPathSeparator() +  itemId + getPathSeparator() + nodeToShare.getName());
	//					} catch (HttpException e) {
	//						throw new InternalErrorException(e);
	//					} catch (IOException e) {
	//						throw new InternalErrorException(e);
		...
		try{
			...
			if (!nodeToShare.isLocked()){
				...
				if (itemToShare.getType() == WorkspaceItemType.FOLDER){
					...
					try {
						logger.info("MOVE FROM: " + nodeToShare.getPath() + " - TO: " + shareRoot.getPath()+ getPathSeparator() + itemId) ;
						...
						//						System.out.println("MOVE FROM " + oldPath + " TO " + itemMoved.getPath());


						//move remote folder and not single files
						//						getStorage().moveRemoteFolder(oldPath, itemMoved.getPath());
	...
	public void updateItem(String itemId, InputStream fileData)
		...
		try{
			...
			if (!itemDelegate.isLocked()){
				...
				updateProperties(session, itemDelegate, lastupdate, info);
	...
	public void init(String portalLogin) throws PathNotFoundException, RepositoryException, InternalErrorException, ItemNotFoundException{
		...
		logger.info(portalLogin + " --> USER VERSION: " + userVersion + " - HL VERSION: " + JCRRepository.HL_VERSION);
		...
		if(!JCRRepository.HL_VERSION.equals(userVersion)){
			...
			try{
				...
				//				session.save();
				um.setVersionByUser(portalLogin, JCRRepository.HL_VERSION);
	...
	public WorkspaceSharedFolder getVREFolderByScope(String scope) throws ItemNotFoundException, InternalErrorException {
		...
		WorkspaceSharedFolder folder = (WorkspaceSharedFolder) getItemByPath(mySpecialFoldersPath + JCRRepository.PATH_SEPARATOR + VRE);
		return folder;
	}
	...
	public ExternalFile createExternalFile(String name, String description,
		...
		try {
			...
			//			System.out.println("get METADATA INFO");
	...
	public JCRWorkspaceCatalogue getCatalogue() throws InternalErrorException {
		logger.info("get Catalogue for user: " + getOwner().getPortalLogin());
