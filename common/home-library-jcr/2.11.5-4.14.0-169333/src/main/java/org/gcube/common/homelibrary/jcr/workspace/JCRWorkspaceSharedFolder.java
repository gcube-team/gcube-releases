...
public class JCRWorkspaceSharedFolder extends JCRAbstractWorkspaceFolder implements WorkspaceSharedFolder {
	...
	public JCRWorkspaceSharedFolder(JCRWorkspace workspace, ItemDelegate delegate,
			String name, String description, String originalDestinationFolderId, List<String> users, JCRSession session) throws RepositoryException, InternalErrorException {		

		super(workspace, delegate, name, description, session);
	...
	private void shareWithUsers(List<String> users) throws RepositoryException {
		try {
	...
	private List<String> listUsers(List<String> usersList) throws InternalErrorException {
		...
		boolean sessionCreated = this.getSession() == null;
		JCRSession servlets = null;
		try {
			servlets = sessionCreated ? new JCRSession(workspace.getOwner().getPortalLogin(), false) : this.getSession();
	...
	private void addUser(String user, String destinationFolderId) throws InternalErrorException, RepositoryException {
		...
		JCRSession servlets = null;
		try {
			servlets = sessionCreated ? new JCRSession(workspace.getOwner().getPortalLogin(), true) : this.getSession();
			HomeManager homeManager = workspace.getHome().getHomeManager();
			...
			if(sessionCreated && servlets != null)
				servlets.releaseSession();
		}
	...
	public WorkspaceFolder unShare(String user) throws InternalErrorException {
		...
		try {
			...
			//			thread.start();
			//			thread.join();
			itemUnshared = this.unShareNode(user);
			logger.info("*********************** After unshare = " + itemUnshared);
		} catch (Exception e) {
