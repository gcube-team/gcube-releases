...
//	}
	...
	InternalErrorException, ItemNotFoundException {
		...
		logger.info("Get user Node of " + user + " in node " + delegate.getPath());
		ItemDelegate userNode = null;
		JCRServlets servlets = null;
		try{
			servlets = new JCRServlets(workspace.getOwner().getPortalLogin(), true);
			@SuppressWarnings("unchecked")
...
//			servlets = new JCRServlets(workspace.getOwner().getPortalLogin());
			...
			//			path = ((JCRWorkspaceFolder)getParent(userNode)).getPath(servlets.getItemById(userNode.getParentId())) + workspace.getPathSeparator() + getNodeName(userNode);	
...
//		System.out.println("share with users: " + users);
