...
public class GXInboundResponse {
	...
	//the content cannot be read more than one time from the response or input stream
	 ...
	 * @param source the original response
	 */
	...
	/**
	 * @param connection the connection from which to parse the information
	 * @throws IOException 
	 */
	public GXInboundResponse(HttpURLConnection connection) throws IOException {
		...
		if (Objects.nonNull(eTag) 
				&& eTag.replaceAll("^\"|\"$", "").equals(EntityTag.gxError)) {
	 ...
	 * Builds a new inbound response.
	 * @param source the original response
	 * @param expectedMediaTypes the expected media type(s) in the response
	 */
	public GXInboundResponse(Response response, MediaType[] expectedMediaTypes) {
		...
		for (MediaType media : expectedMediaTypes) {
			if (response.getMediaType().isCompatible(media))
				compatible = true;
		}
		if (!compatible) 
	 ...
	 * @throws ClassNotFoundException if the exception's class is not available on the classpath
	 */
	 ...
	 * @return the content
	 * @throws IOException if unable to read the content
	 */
	public String getStreamedContentAsString() throws IOException {
		if (this.body.isEmpty()) {
			if (this.fromConnection && !this.contentAlreadyConsumed) {
				this.body = ContentUtils.toString(ContentUtils.toByteArray(connection.getInputStream()));
			} else if (this.fromResponse && !this.contentAlreadyConsumed) {
				this.body = source.readEntity(String.class);
			}
		}
		return this.body;
	}
	...
	public byte[] getStreamedContent() throws IOException {
		if (!this.body.isEmpty()) {
			...
			if (this.fromConnection && !this.contentAlreadyConsumed) 
				...
				this.streamedContent = ContentUtils.toByteArray((InputStream) source.getEntity());
			}
		}
	...
	public <T> T tryConvertStreamedContentFromJson(Class<T> raw) throws Exception {
		return ContentUtils.fromJson(this.getStreamedContentAsString(), raw);
	}
