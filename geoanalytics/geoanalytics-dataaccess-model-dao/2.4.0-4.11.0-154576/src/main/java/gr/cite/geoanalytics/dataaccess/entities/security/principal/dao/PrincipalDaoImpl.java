...
public class PrincipalDaoImpl extends JpaDao<Principal, UUID> implements PrincipalDao {
	...
	public List<PrincipalProjectInfoDao> findByClassReturnsPrincipalProjectInfo(
			PrincipalClass  classType, ActiveStatus activeStatus, Tenant tenant){
		...
		queryB.append(" and p.tenant = :tenant");
...
//		query.setParameter("active", activeStatus.code());
	...
	@Override
	public List<Principal> findByClassAndCreatorAndTenantAndStatus(
			PrincipalClass classType, ActiveStatus activeStatus,
			UUID tenantID, Principal principal) {
		
		StringBuilder queryB = new StringBuilder("FROM Principal p");
		queryB.append(" WHERE p.classId = :classId");
		queryB.append(" AND p.tenant.id = :tenantID");
	...
	public Principal findByVreIdAndTenant(UUID vreId, Tenant tenant) throws Exception {
		StringBuilder queryB = new StringBuilder("FROM Principal p WHERE p.principalData.vreUsrId=:vreId AND p.tenant = :tenant");
		
		TypedQuery<Principal> query = entityManager.createQuery(queryB.toString(), Principal.class);
		query.setParameter("vreId", vreId);
		query.setParameter("tenant", tenant);
	...
	public Principal findByVreIdAndTenantID(UUID vreId, UUID tenantId) {
		...
		StringBuilder queryB = new StringBuilder("FROM Principal p WHERE p.principalData.vreUsrId = :vreId AND p.tenant.id = :tenantId");
		...
		try {
			...
			log.info("Principal with vreUsrID: " + vreId + " and tenantID: " + tenantId + "not found");
			e.printStackTrace();
		}
