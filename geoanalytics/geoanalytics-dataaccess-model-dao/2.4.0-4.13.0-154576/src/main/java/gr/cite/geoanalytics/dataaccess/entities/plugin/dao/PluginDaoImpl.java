...
TypedQuery<Plugin> query = entityManager.createQuery("FROM Plugin pl WHERE pl.tenant = :tenant", Plugin.class);
	...
	public List<PluginInfoDaoUtil> listPluginsByTenantOrNullTenant(Tenant tenant) {
		...
		queryStr.append("SELECT new gr.cite.geoanalytics.dataaccess.entities.plugin.dao.PluginInfoDaoUtil(p.id, p.name, p.descrtiption, p.lastUpdate, p.creationDate, p.type)");
		queryStr.append(" FROM Plugin p WHERE p.tenant = :tenant OR p.tenant IS NULL");
		queryStr.append(" GROUP BY p.id, p.name, p.descrtiption, p.lastUpdate, p.creationDate, p.type");
	...
	public String getPluginNameByID(UUID pluginID) {
		log.debug("Retrieving plugin name by ID: " + pluginID);
		StringBuilder queryStrB = new StringBuilder("");
		queryStrB.append("SELECT p.name FROM Plugin p WHERE p.id = :pluginID");
		
		String result = null;
		
		try {
			Query query = entityManager.createQuery(queryStrB.toString(), String.class);
			query.setParameter("pluginID", pluginID);
	...
	public void deletePluginByPluginID(UUID pluginID) {
		...
		str.append("DELETE FROM Plugin p WHERE p.id = :pluginID");
