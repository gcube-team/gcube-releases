...
private static Logger log = LoggerFactory.getLogger(GeocodeDaoImpl.class);
	...
	public List<Geocode> findByName(String name) {
		...
		{
			for (Geocode t : (List<Geocode>) result) 
	...
	public List<Geocode> findByGeocodeSystem(GeocodeSystem geocodeSystem) {
		...
		log.debug((result != null ? result.size() : 0) + " results");
		if(log.isDebugEnabled() && result != null){
			for (Geocode tt : (List<Geocode>) result) 
	...
	public List<Geocode> getLinked(Geocode t) {
		...
		TypedQuery<Geocode> query = entityManager.createQuery(
				"select l.sourceTerm from TaxonomyTermLink l where l.destinationTerm = :tt", Geocode.class);
		query.setParameter("tt", t);
		
		result = query.getResultList();
		
		query = entityManager.createQuery(
				"select l.destTerm from TaxonomyTermLink l where l.sourceTerm = :tt", Geocode.class);
		query.setParameter("tt", t);
	...
	public List<Geocode> getLinkedTerms(GeocodeSystem sourceGeocodeSystem, Geocode destTerm, TaxonomyTermLink.Verb verb) {
		TypedQuery<Geocode> query = entityManager.createQuery(
				"select distinct stt from TaxonomyTermLink ttl, Geocode stt, Geocode dtt where stt.geocodeSystem = :t and ttl.sourceTerm = stt and ttl.destinationTerm = :tt and ttl.verb = :v", 
				Geocode.class);
		query.setParameter("t", sourceGeocodeSystem);
	...
	public List<Geocode> getLinkedTerms(Geocode destTerm, TaxonomyTermLink.Verb verb) {
		TypedQuery<Geocode> query = entityManager.createQuery(
				"select distinct stt from TaxonomyTermLink ttl, Geocode dtt where ttl.destinationTerm = :tt and ttl.verb = :v", 
				Geocode.class);
		query.setParameter("tt", destTerm);
		query.setParameter("v", verb.verbCode());
	...
	public List<Geocode> getActiveLinkedTerms(GeocodeSystem sourceGeocodeSystem, Geocode destTerm, TaxonomyTermLink.Verb verb) {
		TypedQuery<Geocode> query = entityManager.createQuery(
				"select distinct stt from TaxonomyTermLink ttl, Geocode stt, Geocode dtt where stt.isActive = 1 and stt.geocodeSystem = :t and ttl.sourceTerm = stt and ttl.destinationTerm = :tt and dtt.isActive = 1 and ttl.verb = :v", 
				Geocode.class);
		query.setParameter("t", sourceGeocodeSystem);
		query.setParameter("tt", destTerm);
		query.setParameter("v", verb.verbCode());
	...
	public List<Geocode> getActiveLinkedTerms(Geocode destTerm, TaxonomyTermLink.Verb verb) {
		TypedQuery<Geocode> query = entityManager.createQuery(
				"select distinct stt from TaxonomyTermLink ttl, Geocode stt, Geocode dtt where stt.isActive = 1 and ttl.sourceTerm = stt and ttl.destinationTerm = :tt and dtt.isActive = 1 and ttl.verb = :v", 
				Geocode.class);
		query.setParameter("tt", destTerm);
		query.setParameter("v", verb.verbCode());
