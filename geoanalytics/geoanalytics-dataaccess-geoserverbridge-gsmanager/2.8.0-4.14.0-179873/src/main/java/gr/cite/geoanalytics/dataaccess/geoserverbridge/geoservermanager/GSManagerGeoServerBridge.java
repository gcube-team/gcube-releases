...
import gr.cite.geoanalytics.dataaccess.geoserverbridge.geoservermanager.CustomPublishers.NetCDFPublisher;
...
public class GSManagerGeoServerBridge implements GeoServerBridge {
	...
	private NetCDFPublisher netCDFPublisher;
	...
	private void initialize(){
		...
		this.netCDFPublisher = new NetCDFPublisher(
				geoServerBridgeConfig.getGeoServerBridgeUrl(),
				geoServerBridgeConfig.getGeoServerBridgeUser(),
				geoServerBridgeConfig.getGeoServerBridgePassword());
	...
	@Override
	public Boolean coverageStoreOfCoverageByIdExists(String workspaceName, String coverageId) throws GeoServerBridgeException {
		logger.debug("Checking if Coveragestore with name: "+ coverageId +" in workspace "+workspaceName+" exists");
		logger.info("Checking if Coveragestore with name: "+ coverageId +" in workspace "+workspaceName+" exists");
		System.out.println("Checking if Coveragestore with name: "+ coverageId +" in workspace "+workspaceName+" exists");
		Boolean exists = false;
		try {
			logger.info("Attempting to retrieve layer: " + coverageId);
			RESTLayer rL = reader.getLayer(workspaceName, coverageId);
			logger.info("Retrieved layer successfully");
			RESTCoverage rC = reader.getCoverage(rL);
			logger.info("Retrieved coverage successfully");

			logger.info("attempting to retrieve coveragestore");

			String coverageStoreName = rC.getStoreName();
			int indexOfColon = coverageStoreName.indexOf(':') + 1;

			exists = (reader.getCoverageStore(workspaceName, coverageStoreName.substring(indexOfColon))  == null) ? false : true;

			logger.info("retrieved coveragestore successfully");
		} catch(Exception e){
			e.printStackTrace();
			logger.debug("Error while retrieveing coverage store", e);
			throw new GeoServerBridgeException("Error while retrieveing coverage store", e);
		}
		logger.debug("Coveragestore with name: "+ coverageId +" in workspace "+workspaceName+" exists: "+ exists);
		return exists;
	}

	@Override
	public Boolean deleteCoverageStoreByCoverageId(String workspaceName, String coverageId, boolean recursive) throws GeoServerBridgeException {
		logger.debug("Deleting coverageStore name: " + coverageId + " in workspace: " + workspaceName );

		if(recursive)
			logger.debug("All containing layers will be permanently deleted!!!");
		...
		GeoServerRESTPublisher.Purge purgeParamValue = recursive ? GeoServerRESTPublisher.Purge.ALL : GeoServerRESTPublisher.Purge.NONE;
		Boolean deletedSuccesfully =  publisher.removeCoverageStore(workspaceName, coverageStoreName.substring(indexOfColon), recursive, purgeParamValue);

		if(!deletedSuccesfully)
			throw new GeoServerBridgeException("An error occurred while deleting coverage store with name:" + coverageId + " in workspace " + workspaceName);
	...
	public GeoserverLayer getGeoserverLayer(String id) throws GeoServerBridgeException
	{
		...
		try
		{
			...
			RESTResource ft = reader.getResource(layer);
	...
	@Override
	public void deleteLayer(String id,String layerName, DataSource dataSource) throws GeoServerBridgeException {
		logger.debug("Deleting layer with name: " + layerName + "...");
		
		boolean removed = false;
		boolean removeAll = false;
		logger.info("Deleting layer with name: " + layerName + " for datasource:" + dataSource);
		try {
			if(DataSource.isVector(dataSource)){
				...
				if(DataSource.isGeoTIFF(dataSource)) {
					removed = publisher.unpublishCoverage(
							geoServerBridgeConfig.getGeoServerBridgeWorkspace(),
							layerName,
							URLEncoder.encode(id, "UTF-8"));
					//previous Data store: geoServerBridgeConfig.getPostgisDataStoreConfig().getDataStoreName()
					removeAll = publisher.removeCoverageStore(geoServerBridgeConfig.getGeoServerBridgeWorkspace(),layerName,false, GeoServerRESTPublisher.Purge.ALL);
					removed &= removeAll;
				} else if(DataSource.isNetCDF(dataSource)) {
					RESTLayer layer = reader.getLayer(geoServerBridgeConfig.getGeoServerBridgeWorkspace(), id);
					RESTCoverage rc = reader.getCoverage(layer);


					String coverageStoreName = rc.getStoreName().substring(rc.getStoreName().lastIndexOf(":") + 1);
					logger.info("Purging coverage store: " + coverageStoreName);

					removed = publisher.removeCoverageStore(geoServerBridgeConfig.getGeoServerBridgeWorkspace(),coverageStoreName,true, GeoServerRESTPublisher.Purge.ALL);
				}
				else {
					removed = publisher.unpublishCoverage(
							geoServerBridgeConfig.getGeoServerBridgeWorkspace(),
		...
		if (removed == false) {
			logger.error("An error originating from the underlying bridge provider occurred while deleting layer " + layerName);
			throw new GeoServerBridgeException("An error originating from the underlying bridge provider occurred while deleting layer " + layerName);
		}
		logger.debug("Layer with name: " + layerName + " has been deleted");
	}
...
//			RESTCoverage rc = reader.getCoverage(workspace, store, name);
			...
			Bounds latLonBounds = new Bounds();
			latLonBounds.setMaxx(rc.getLatLonBoundingBox().getMaxX());
			latLonBounds.setMaxy(rc.getLatLonBoundingBox().getMaxY());
			latLonBounds.setMinx(rc.getLatLonBoundingBox().getMinX());
			latLonBounds.setMiny(rc.getLatLonBoundingBox().getMinY());
			latLonBounds.setCrs(rc.getLatLonBoundingBox().getCRS());

			ct.setLatLonBoundingBox(latLonBounds);

			Bounds nativeBounds = new Bounds();
			nativeBounds.setMaxx(rc.getLatLonBoundingBox().getMaxX());
			nativeBounds.setMaxy(rc.getLatLonBoundingBox().getMaxY());
			nativeBounds.setMinx(rc.getLatLonBoundingBox().getMinX());
			nativeBounds.setMiny(rc.getLatLonBoundingBox().getMinY());
			nativeBounds.setCrs(rc.getLatLonBoundingBox().getCRS());

			ct.setNativeBoundingBox(nativeBounds);
			
			Map<String, String> metaData = new HashMap<String, String>();
			if(rc != null && rc.getEncodedDimensionsInfoList() != null) {
				rc.getEncodedDimensionsInfoList().forEach(d -> {
					metaData.put(d.getName(), d.getDimensionTypeName());

				});
			}
//			while(rc.getMetadataList().iterator().hasNext()) {
//				RESTMetadataList.RESTMetadataElement elem = rc.getMetadataList().iterator().next();
//				metaData.put(elem.getKey(), elem.getMetadataElem().getValue());
//			}
	...
	public List<String> getAllStyles() throws GeoServerBridgeException
	{
		...
		System.out.println("Get all styles");
	...
	@Override
	public void addStyleIcon(String filename, File icon)  {
		logger.debug("Adding icons for style: " +filename+".");

		String result = HTTPUtils.put(this.geoServerBridgeConfig.getGeoServerBridgeUrl()+"/rest/resource/styles/"+filename, icon, "image/png", this.geoServerBridgeConfig.getGeoServerBridgeUser(), this.geoServerBridgeConfig.getGeoServerBridgePassword());
		logger.debug("For Style: " +filename+" and icon file: "+icon.getName()+" has been added");
	...
	public void publishGeoTIFF(PublishConfig publishConfig, Coverage geotiff) throws Exception {	
		...
		File geotiffImage = File.createTempFile(geotiff.getName(), fileExtension);
		...
		try {			
			...
			coverageEncoder.addSupportedFormats("PNG");
			coverageEncoder.addSupportedFormats("GIF");
			coverageEncoder.addSupportedFormats("JPEG");
			coverageEncoder.addSupportedFormats("TIFF");
	...
	public void publishGeoTIFF(PublishConfig publishConfig, Coverage coverage, InputStream geotiff) throws Exception {
		String workspace = geoServerBridgeConfig.getGeoServerBridgeWorkspace();
//		String storeName = geoServerBridgeConfig.getGeotiffDataStoreConfig().getDataStoreName();
		String layerName = publishConfig.getLayerId();
		String layerTitle = publishConfig.getLayerName();

		int dotIndex = coverage.getName().lastIndexOf(".");

		String fileName = coverage.getName().substring(0, dotIndex);

        String fileExtension = coverage.getName().substring(dotIndex, coverage.getName().length());//FilenameUtils.getExtension(coverage.getName());

		File geotiffImage = File.createTempFile(fileName , fileExtension);
		FileUtils.copyInputStreamToFile(geotiff, geotiffImage);
		geotiff.close();
		System.out.println("Import stream size:"+ geotiffImage.getTotalSpace());

        System.out.println("The after file name is:" + geotiffImage.getName() + "and layer title is:" +layerTitle+ " geotif path:" + geotiffImage.getPath());
		String storeName = layerTitle;
		try {
			final GSCoverageEncoder coverageEncoder = new GSCoverageEncoder();
			coverageEncoder.setTitle(layerTitle);
			coverageEncoder.setProjectionPolicy(ProjectionPolicy.REPROJECT_TO_DECLARED);
			/***** working projection for rd_new "EPSG:2018" *****/
			coverageEncoder.setSRS("EPSG:4326");

//			Assert.isTrue(publisher.publishGeoTIFF(workspace, storeName, layerName, geotiffImage), "");
			double [] bounds = { publishConfig.getBounds().getMinX(), publishConfig.getBounds().getMinY(), publishConfig.getBounds().getMaxX(), publishConfig.getBounds().getMaxY()};
            Assert.isTrue(publisher.publishGeoTIFF(workspace, storeName, layerName, geotiffImage,"EPSG:4326", ProjectionPolicy.REPROJECT_TO_DECLARED,publishConfig.getStyleName(),bounds), "");
	...
	@Override
	public void publishNetCDF(PublishConfig publishConfig, Coverage netCDF) throws Exception {

	}

	@Override
	public void publishNetCDF(NetCDFPublishConfig publishConfig, Coverage coverage, InputStream netCDF, GeoServerBridgeConfig configuration) throws Exception {
		String workspace = geoServerBridgeConfig.getGeoServerBridgeWorkspace();
		String layerName = publishConfig.getLayerId();
		String layerTitle = publishConfig.getLayerName();
		...
		String fileExtension = coverage.getName().substring(dotIndex, coverage.getName().length());

		// The netCDF file(stream) is zipped. Unzip it and get the zipentry name
		String netcdfFileName = fileName+".nc";

		File netCDFFile = File.createTempFile(fileName , fileExtension);
		FileUtils.copyInputStreamToFile(netCDF, netCDFFile);
		netCDF.close();

//		System.out.println("The after file name is:" + netCDFFile.getName() + "and layer title is:" +layerTitle+ " netCDF path:" + netCDFFile.getPath());
		logger.debug("The after file name is:" + netCDFFile.getName() + "and layer title is:" +layerTitle+ " netCDF path:" + netCDFFile.getPath());
//		String storeName = layerTitle + UUID.randomUUID().toString();
		try {
			final GSCoverageEncoder coverageEncoder = new GSCoverageEncoder();
			coverageEncoder.setTitle(layerTitle);
			coverageEncoder.setProjectionPolicy(ProjectionPolicy.REPROJECT_TO_DECLARED);
			...
			double [] bounds = { publishConfig.getBounds().getMinX(), publishConfig.getBounds().getMinY(), publishConfig.getBounds().getMaxX(), publishConfig.getBounds().getMaxY()};
//			Assert.isTrue(publisher.publishGeoTIFF(workspace, storeName, layerName, netCDFFile,"EPSG:4326", ProjectionPolicy.REPROJECT_TO_DECLARED,publishConfig.getStyleName(),bounds), "");

//			Assert.isTrue(this.netCDFPublisher.publishNetCDF(workspace, storeName, layerName, netCDFFile,"EPSG:4326", ProjectionPolicy.REPROJECT_TO_DECLARED,publishConfig.getStyleName(),bounds), "");

			NetCDFCoverage nc = new NetCDFCoverage();
			nc.setEnabled(true);
			nc.setName(publishConfig.getStoreName());
			nc.setWorkspace(workspace);
			nc.setUrl("file:" + workspace + "\\/" + publishConfig.getStoreName() + "\\/" + netcdfFileName);
			nc.setType("NetCDF");
			String xmlBody = this.marshall(nc);
			logger.info("XML to be send to Geoserver as part of the request: " + xmlBody);

			Assert.isTrue(this.netCDFPublisher.createNetCDFCoverageStore(workspace, publishConfig.getStoreName(), xmlBody), "");
			Assert.isTrue(this.netCDFPublisher.createNetCDFLayerFromFile(workspace, publishConfig.getStoreName(), netCDFFile),"");
			Assert.isTrue(this.configureNetCDFLayers(publishConfig,configuration, coverageEncoder), "");
//			Assert.isTrue(publisher.configureCoverage(coverageEncoder, workspace, publishConfig.getStoreName(), layerName), "");
		} catch (Exception e) {
			throw new GeoServerBridgeException("Could not publish NetCDF " + layerName + " in GeoServer", e);
		} finally {
			if(netCDF != null)
				netCDFFile.delete();
		}

		logger.info("NetCDF " + layerName + " has been published successfully");
	}

	private boolean configureNetCDFLayers(NetCDFPublishConfig publishConfig, GeoServerBridgeConfig configuration, GSCoverageEncoder coverageEncoder) {
		boolean result = true;
		...
		while( entryIt.hasNext() ) {
			...
			//default publisher.configureCoverage
			publisher.configureCoverage(coverageEncoder, workspace, publishConfig.getStoreName(), idName.getKey().toString());
	...
	private boolean createNetCDFCoverageStore(String gosEndpoint, String workspace, String coverageStoreName, String fileName){
		String baseURL = geoServerBridgeConfig.getGeoServerBridgeUrl();
		String gsUser = geoServerBridgeConfig.getGeoServerBridgeUser();
		String gsPass = geoServerBridgeConfig.getGeoServerBridgePassword();

		String xmlBody = "<coverageStore><name>" + coverageStoreName +
				"</name><workspace>" + workspace + "</workspace><enabled>true</enabled><type>NetCDF</type>" +
				"<url>file:" + coverageStoreName + "\\/sresa1b_ncar_ccsm3-example.nc</url></coverageStore>";

		StringBuilder sbUrl = new StringBuilder();
		sbUrl.append(baseURL).append("/rest/workspaces/").append(workspace).append("/coveragestores?configure=all");


		String sendResult = HTTPUtils.postXml(sbUrl.toString(), xmlBody, gsUser, gsPass);
