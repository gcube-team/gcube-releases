...
private static final Logger log = LoggerFactory.getLogger(ProjectManager.class);
	...
	@Inject
	public void setLayerDao(LayerManager layerManager) {
	...
	@Inject
	public void setProjectTermDao(ProjectLayerDao projectLayerDao) {
		this.projectLayerDao = projectLayerDao;
	}
	...
	public void removeProjectAttribute(Project project, Principal principal, AttributeInfo attributeInfo) throws Exception {
		...
		Layer attrT = projectLayerDao.findByProjectAndGeocodeSystem(project, taxonomy);
		
		if(attrT == null) {
			log.error("Could not find layer for attribute " + attributeInfo.getName() + " for taxonomy "+ attributeInfo.getTaxonomy() +" in project: "+project.getId());
			throw new Exception("Could not find attribute");
		}
		
		ProjectLayer projectLayer = projectLayerDao.find(project, attrT);
		if(projectLayer == null) {
	...
	public void updateProjectAttribute(Project project, Principal principal, AttributeInfo attributeInfo) throws Exception {
		...
		String locale = LocaleContextHolder.getLocale().getLanguage();
		Layer layerByProjTax = projectLayerDao.findByProjectAndGeocodeSystem(project, taxonomy);
		
		boolean shouldCreate = false;
		if(layerByProjTax == null) {//for updates, allow term auto-creation only for configured taxonomies
		...
		if(layerByProjTax != null){
			attributeValue = LayerUtils.retrieveLocalizedValue(layerByProjTax, locale, documentBuilder);
		}
		
		if(!attributeInfo.getValue().equals(attributeValue)) {
			ProjectLayer oldProjectTerm = projectLayerDao.find(project, layerByProjTax);
			if(!shouldCreate && oldProjectTerm == null) {
				log.error("Unexpected error: could not find project term for project " + project.getId() + " and term " + layerByProjTax.getId());
				throw new Exception("Could not find project term");
			}
			
			projectLayerDao.delete(oldProjectTerm);
...
//			}
			...
			newLayer.setName(normalizedValue);
			layerManager.updateLayer(newLayer);
