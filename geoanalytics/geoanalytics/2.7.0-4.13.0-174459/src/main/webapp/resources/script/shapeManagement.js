function showShapeManagement()
{
    ...
    $('#searchButton')[0].addEventListener('click', function(ev) {searchButtonListener(ev, container);});
    //createEditForm();
    //createModalAddForm();
    createAdminMenu(data);
	setPrimaryMenuButton("btnShapeManagement");
        ...
        success : function(instances) 
        		  {
        			...
        			{
        				option = document.createElement("option");
        				option.value = instances[i].importId;
        				option.text = instances[i].importId + (isPresent(instances[i].termTaxonomy) ? " for " + instances[i].termTaxonomy + ":" + instances[i].term : "") +
						" on " + timestampToDateTimeString(instances[i].timestamp);
        				select.appendChild(option);
        			}
        		  },
	  error: 	function(jqXHR, textStatus, errorThrown) 
