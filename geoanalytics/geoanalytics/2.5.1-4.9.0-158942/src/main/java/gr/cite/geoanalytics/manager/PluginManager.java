	...
	public List<ExecutionDetailsInfo> buildResponseFromExecutionDetails(Map<String, ExecutionDetails> execDet, String tenantName, String principalID) {
		return execDet.entrySet().stream()
				.filter(execDetMap -> {
					...
					Layer layer = null;
					try {
						project = projectManager.getProjectById(UUID.fromString(execDetMap.getValue().getProjectID()));
						if(execDetMap.getValue().getLayerID() != null)
							layer = layerManager.findLayerById(UUID.fromString(execDetMap.getValue().getLayerID()));
					} catch(Exception e){
						e.printStackTrace();
						return false;
					}
					
					if((execDetMap.getValue().getLayerID() != null && layer == null) || project == null) {
						return false;
					}
