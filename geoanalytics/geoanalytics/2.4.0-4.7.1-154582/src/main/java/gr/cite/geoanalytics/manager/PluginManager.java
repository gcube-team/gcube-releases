...
//         Getting a method from the loaded class and invoke it
        ...
        ep.setSystemicSparkParams(getPropertiesFromFile(SPARK_ENV_PROPERTIES));
	...
	public Map<String,String> getPropertiesFromFile(String propertiesFilename){
		...
		try {
			...
			fs.searchDirectory(new File(path), propertiesFilename);
			if(fs.getResult().size()>0){
				Properties p = new Properties();
				p.load(new FileInputStream(fs.getResult().get(0)));
			    Enumeration e = p.propertyNames();
			    while (e.hasMoreElements()) {
			      String key = (String) e.nextElement();
			      fetchedProperties.put(key, p.getProperty(key));
			    }
			}
			else{
				log.info("Could not find any systemic properties for Spark by the filename: "+propertiesFilename);
			}
		} catch (IOException e1) {
			log.info("Error loading the systemic properties for Spark within file: "+propertiesFilename);
			e1.printStackTrace();
		}
