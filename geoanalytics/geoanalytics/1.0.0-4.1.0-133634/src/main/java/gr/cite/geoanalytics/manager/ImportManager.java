...
import gr.cite.gaap.datatransferobjects.TsvImportMetadata;
...
public class ImportManager {
	...
	public ImportManager(GeospatialBackend shapeManager, ShapeImportManager shapeImportManager, TaxonomyManager taxonomyManager,
	 ...
	 * @throws Exception
	 */
	@Transactional(propagation=Propagation.REQUIRES_NEW, rollbackFor={GeoServerBridgeException.class,Exception.class}) 
	public TaxonomyTerm createNewLayerForTaxonomyTermOfTsv(
			UUID templateLayerTaxonomyTermId, String newLayerNameForTaxonomyTerm, UUID taxonomyOfLayerTaxonomyTerm, String tsv)
			throws Exception {

		Taxonomy taxonomy = this.taxonomyManager.findTaxonomyById(taxonomyOfLayerTaxonomyTerm.toString(), false);
		TaxonomyTerm templateLayerTaxonomyTerm = this.taxonomyManager.findTermById(templateLayerTaxonomyTermId.toString(), false);
		...
		System.out.println("Execution Time in  tsvparsing = " + (((System.currentTimeMillis() - startTime)/1000)));
	 ...
	 */
	@Transactional(propagation=Propagation.REQUIRES_NEW, rollbackFor={GeoServerBridgeException.class,Exception.class}) 
	public void createLayerInDataBaseAndGeoserver(
			UUID templateLayerTaxonomyTermId, TaxonomyTerm taxonomyTerm)
	...
	public void publishLayerMetadataToGeonetwork(TsvImportMetadata tsvImportMetadata){
		MetaDataForm meta = new MetaDataForm(tsvImportMetadata.getUser(), tsvImportMetadata.getTitle(), new Date());		
		meta.setAbstractField(tsvImportMetadata.getAbstractField());
		meta.setPurpose(tsvImportMetadata.getPurpose());
		meta.setKeywords(tsvImportMetadata.getKeywords());			
		meta.setUserLimitation(tsvImportMetadata.getLimitation());
		meta.setGraphicOverviewExternal(tsvImportMetadata.getGraphicOverview());		
		meta.setDistributorOrganisationName(tsvImportMetadata.getDistributorOrganisationName());
		meta.setDistributorIndividualName(tsvImportMetadata.getDistributorIndividualName());
		meta.setDistributorSite(tsvImportMetadata.getDistributorOnlineResource());		
		meta.setProviderIndividualName(tsvImportMetadata.getProviderIndividualName());
		meta.setProviderSite(tsvImportMetadata.getProviderOnlineResource());			
		meta.setProviderOrganisationName(tsvImportMetadata.getProviderOrganisationName());		
		
		try {			
			Principal principal =  securityContextAccessor.getPrincipal();
			Tenant tenant = principal.getTenant();
			
			GeoNetworkBridge geo = new GSManagerGeoNetworkBridge();
			geo.publishGeonetwork(tenant.getName(), meta);
			log.info("Error while publishing layer metadata to geonetwork");
	...
	public void tsvParsing(TaxonomyTerm templateLayerTaxonomyTerm, TaxonomyTerm taxonomyTermLayer, String tsv) throws Exception {
		...
		long startTime = System.currentTimeMillis();
		Taxonomy geoTaxonomy = shapeManager.getTermFromLayerTermAndShape(templateLayerTaxonomyTerm, this.shapeManager.getShapesOfLayer(templateLayerTaxonomyTerm).get(0)).getTaxonomy();
		
		List<String> taxonomyHierarchy = this.shapeManager.getGeographyHierarchy(geoTaxonomy).getMainHierarchy().stream().map(t -> t.getName()).collect(Collectors.toList());
		...
		System.out.println("Execution Time for taxonomyHierarchy = " + (((System.currentTimeMillis() - startTime)/1000)));
...
/*		startTime = System.currentTimeMillis();
		...
		for (String taxonomyName : taxonomyHierarchy){
			Taxonomy taxonomy =  this.taxonomyManager.findTaxonomyByName(taxonomyName, false);
			if (taxonomy == null){
		...
		Map<String, TaxonomyTerm> taxonomyTermForShapes = this.taxonomyManager.findTermByNameAndTaxonomies(taxonomies);
		
		System.out.println("Execution Time for List<Taxonomy> taxonomies = " + (((System.currentTimeMillis() - startTime))));*/
		
		startTime = System.currentTimeMillis();
		
		Map<String, Shape> sourceShapes = shapeManager.getShapesFromLayerTerm(templateLayerTaxonomyTerm);
		
		System.out.println("Execution Time for getShapeFromLayerTermAndShapeTerm = " + (((System.currentTimeMillis() - startTime))));
		...
		for(CSVRecord record : csvParser) {
			if (recordIndex != 0){
				...
				TaxonomyTerm taxonomyTermForShape = null;
				...
				for(String value : record){	
					if (valueIndex == 0){
						...
						//taxonomyTermForShape = this.taxonomyManager.findTermByNameAndTaxonomies(geoAttributeValue, taxonomyHierarchy, false);
						//taxonomyTermForShape = this.taxonomyManager.findTermByNameAndTaxonomies(geoAttributeValue, taxonomies);
						//taxonomyTermForShape = taxonomyTermForShapes.get(geoAttributeValue);
						
						//if (taxonomyTermForShape == null){
							//break;
						//}
						
						//sourceShape = shapeManager.getShapeFromLayerTermAndShapeTerm(templateLayerTaxonomyTerm, taxonomyTermForShape);
						...
						sourceShape = checkIfThisShapeExistInThisShapeListAndReturnShape(shapes, sourceShape);	
						...
						this.shapeManager.addShapeAttribute(targetShape, attributeTag, value, null);
						//System.out.println("Execution Time for attributes = " + (System.currentTimeMillis() - startTime));
					}
					valueIndex++;				
				}
			}else{
				...
				unitPos = firstPartOfHeaderListCommaSeparated.indexOf("unit");
			}
			recordIndex++;
		}
		

		this.shapeManager.createShapeAssociationsWithLayerTerm(taxonomyTermLayer, shapes);
	...
	public ShapeImportInfo importLayerFromShapeFile(String filename, TaxonomyTerm tt, TaxonomyTerm boundaryTerm,
			int srid, String charset, boolean forceLonLat, boolean newLayer, List<AttributeInfo> attrInfo, Principal principal, boolean overwriteMappings, 
			String style, GeographyHierarchy hierarchy) throws Exception {
		
		ShapeImportInfo info =  importLayerFromShapeFileToDataBase(filename, tt, boundaryTerm,srid, charset, forceLonLat, newLayer, attrInfo, principal, hierarchy);
		
		try{
			this.setUpNewOrUpdatedLayer(tt, boundaryTerm, newLayer, new LayerConfig(), null, info.getBoundingBox());
			securityContextAccessor.updateLayers(); //update accessible layers (only for current user, which is acceptable because layers are added by a single user with administrative rights)
		}catch(Exception e){
	...
	private ShapeImportInfo importLayerFromShapeFileToDataBase(String filename,
			TaxonomyTerm tt, TaxonomyTerm boundaryTerm, int srid,
			String charset, boolean forceLonLat, boolean newLayer,
			List<AttributeInfo> attrInfo, Principal principal,
			GeographyHierarchy hierarchy) throws Exception {
		...
		try {
			...
			info = newLayerFromShapeFile(filename, tt, boundaryTerm,
					srid, charset, forceLonLat, newLayer, attrInfo, principal, true, layerConfig, layerDefaultStyle, hierarchy); //TODO expose overwriteMappings
		...
		if(newLayer == false && removed == true) {
			...
			try {
				if((l = geoServerBridge.getLayer(tt.getName())) == null) {
					...
					geoServerBridge.addLayer(l, ft, layerStyles, 
	...
	@Transactional(rollbackFor={Exception.class})
	public ShapeImportInfo newLayerFromShapeFile(String filename, TaxonomyTerm tt, TaxonomyTerm boundaryTerm,
			int srid, String charset, boolean forceLonLat, boolean newLayer, List<AttributeInfo> attrInfo, Principal principal, boolean overwriteMappings, 
			LayerConfig layerConfig, String style, GeographyHierarchy hierarchy) throws Exception {
		
		Map<String, Map<String, AttributeInfo>> attrInfoM = createAttributeInfoMap(attrInfo);
		ShapeImportInfo info = shapeImportManager.fromShapefile(filename, tt.getId().toString(), srid, charset, forceLonLat, attrInfoM, principal, overwriteMappings);
		shapeManager.generateShapesOfImport(tt, attrInfoM, info.getValueMappingValues(), info.getImportId(), tt.getId().toString(), hierarchy, principal);
		if(boundaryTerm != null) shapeManager.generateShapeBoundary(tt, boundaryTerm, principal);
		
		String layerName = newLayer ? tt.getName() : layerConfig.getName();
		info.setLayerName(layerName);
		
		if(!newLayer) {
			info.setBoundingBox(new Bounds(layerConfig.getBoundingBox().getMinX(), layerConfig.getBoundingBox().getMinY(),
	...
	public void setUpNewOrUpdatedLayer(TaxonomyTerm tt, TaxonomyTerm boundaryTerm, boolean newLayer,
		...
		if(newLayer){
			...
			if(boundaryTerm != null) layerConfig.setBoundaryTermId(boundaryTerm.getId().toString());
			
			LayerBounds b = new LayerBounds(); b.setMinX(boundingBox.getMinx()); b.setMinY(boundingBox.getMiny());
			b.setMaxX(boundingBox.getMaxx()); b.setMaxY(boundingBox.getMaxy());
			...
			this.dataManager.createDataBaseView(tt.getId().toString(),  tt.getName());/*re-create view as new attributes could have been merged into the dataset*/
			...
			if(boundaryTerm != null) lcfg.setBoundaryTermId(boundaryTerm.getId().toString());
	...
	private Map<String, Map<String, AttributeInfo>> createAttributeInfoMap(List<AttributeInfo> attrInfo) {
		...
		for(AttributeInfo ai : attrInfo) {
			if(attrInfoM.get(ai.getName()) == null) attrInfoM.put(ai.getName(), new HashMap<String, AttributeInfo>());
			if(ai.getValue() == null) attrInfoM.get(ai.getName()).put("", ai);
			else attrInfoM.get(ai.getName()).put(ai.getValue(), ai);
		}
		return attrInfoM;
	}
	...
	public List<String> getCapabilities(WfsRequestMessenger reqM) throws Exception {
...
//		parameters.put("version", reqM.getVersion());
		...
		parseGetCapabilitiesForService(body);
		
		return parseGetFeatureTypes(body);
	}
	
	public void parseGetCapabilitiesForService(String body) throws Exception {
	...
	public List<String> parseGetFeatureTypes(String body) throws Exception {
		 ...
		 * 
		 * <FeatureType><Name>topp:tasmania_cities</Name>
	...
	public List<String> doCapabilities(WfsRequestMessenger reqM) throws Exception {
		return getCapabilities(reqM);
	}
...
//				parameters.put("version", reqM.getVersion());
		...
		try {
			...
			File f = File.createTempFile(featureType, ".zip");
...
//			ZipInputStream z1 = new ZipInputStream(inputStream);
//			ZipEntry zipEntry;
//		//			ZipFile z1 = new ZipFile("C://Users//sganoti//geoanalyticsDataRepository//stash//23d2a2b5-74d9-41fb-8407-9fe7d7bf85e2.zip");
//					while ((zipEntry=z1.getNextEntry())!=null) {
//
//						String name = zipEntry.getName();
//						long size = zipEntry.getSize();
//						long compressedSize = zipEntry.getCompressedSize();
...
//						}
//
//						InputStream is = z1.getInputStream(zipEntry);
//						
//						
//						
...
//					z1.close();				
	...
	public void updateLayerStyle(String name, String origName, String style) throws Exception {
		...
		if(configurationManager.getLayerStyle(origName) != null) {
			...
			for(LayerStyleMessenger cfg : refs) {
				...
				if(addUnique) uniqueStyles.add(cfg);
				geoServerBridge.removeLayerStyle(cfg.getLayerName(), origName, 
						cfg.getMinScale(), cfg.getMaxScale());
			}
			...
			for(LayerStyleMessenger ref : defaultRefs) {
				geoServerBridge.setDefaultLayerStyle(ref.getLayerName(), name, style, 
						ref.getMinScale(), ref.getMaxScale());
				if(!name.equals(origName))
					configurationManager.addDefaultTermStyle(ref.getTermId(), name);
			}
			for(LayerStyleMessenger ref : refs) {
				geoServerBridge.addLayerStyle(ref.getLayerName(), name, style, 
						ref.getMinScale(), ref.getMaxScale());
				if(!name.equals(origName)) 
				...
				if(ts != null) {
					if(!ts.equals(configurationManager.getDefaultTermStyle(cfg.getTermId()))) {
						try {
							...
							log.warn("Could not remove style for theme " + t + " from layer " + cfg.getName() + ", style=" + ts, e);
						}
					}
				}
			}
	...
	@Transactional(rollbackFor={Exception.class})
	public void configureLayer(TaxonomyTerm tt, String theme, String style, Integer minScale, Integer maxScale) throws Exception {
	
		LayerConfig cfg = configurationManager.getLayerConfig(tt);
		if(cfg == null) throw new Exception("Layer configuration not found for term " + tt.getId());
		String ts = theme != null && !theme.equals(SystemPresentationConfig.DEFAULT_THEME) ? 
				configurationManager.getTermStyle(theme, tt.getId().toString()) : configurationManager.getDefaultTermStyle(tt.getId().toString());
		/*
		 * optimization not enforced because actual styles need to be updated on the remote side so that their proper update can be ensured
		 * if(ts != null && ts.equals(style))
		{
			if((minScale == null && cfg.getMinScale() == null) || (minScale != null && cfg.getMinScale() != null && minScale.equals(cfg.getMinScale())))
			{
				if((maxScale == null && cfg.getMaxScale() == null) || (maxScale != null && cfg.getMaxScale() != null && maxScale.equals(cfg.getMaxScale())))
					return; //no change
			}
		}*/
		String sld = configurationManager.getLayerStyle(style);
		if(sld == null) throw new Exception("Style " + style + " not found");
		...
		if(ts != null) {
			...
			for(LayerStyleMessenger ref : refs) {
				if(!ref.getTermId().equals(tt.getId().toString()) || 
						((ref.getMinScale() == null && minScale != null) || (ref.getMinScale() != null && minScale == null) || ref.getMinScale() != null && minScale != null && !ref.getMinScale().equals(minScale)) ||
						((ref.getMaxScale() == null && maxScale != null) || (ref.getMaxScale() == null && maxScale != null) || ref.getMaxScale() != null && maxScale != null && !ref.getMaxScale().equals(maxScale))) {
					found = true;
					break;
				}
			}	
		...
		if(theme != null && !theme.equals(SystemPresentationConfig.DEFAULT_THEME)) {
			if(ts != null && !ts.equals(SystemPresentationConfig.DEFAULT_STYLE) && !found)  //no other layers are referencing this particular style instance, so sync with remote server
				geoServerBridge.removeLayerStyle(cfg.getName(), ts, cfg.getMinScale(), cfg.getMaxScale());
	...
	public void removeLayer(TaxonomyTerm tt) throws Exception {
		...
		try {
			geoServerBridge.deleteLayer(lc != null ? lc.getName() : tt.getName()); //fallback to name of term if for some exceptional reason lc does not exist
		}catch(GeoServerBridgeException e) {
		...
		for(TaxonomyTermShape tts : termMappings) {
			if(tts.getTerm().getExtraData() != null && tts.getTerm().getExtraData().toLowerCase().contains("auto")) {
				...
				if(ttst == null) continue; //tts and ttst already deleted (applies only if terms map to multiple shapes)
	...
	@Transactional(rollbackFor={GeoServerBridgeException.class, Exception.class})
	public String newLayerFromImportedData(String identity, LayerConfig layerConfig, String crs, String style) throws Exception {
		...
		Bounds b = new Bounds(layerConfig.getBoundingBox().getMinY(), layerConfig.getBoundingBox().getMinY(), 
				layerConfig.getBoundingBox().getMaxX(), layerConfig.getBoundingBox().getMaxY(),
				crs);
		...
		geoServerBridge.addLayer(l, featureType, configurationManager.getLayerStyles(), 
				layerConfig.getMinScale(), layerConfig.getMaxScale());
		return l.getName();
	}
}
