...
private static final Logger log = LoggerFactory.getLogger(ProjectManager.class);
	...
	@Transactional(readOnly = true)
	public ProjectSummary fetchProjectSummariesForProjectName(String projectName) throws Exception {
		...
		projectSummary.setNumberOfLayers(projectTermDao.findByProject(project).size());
		projectSummary.setStartDate(project.getCreationDate().getTime());
		projectSummary.setClient(project.getClient());
		
		List<String> usersNames = this.fetchProjectParticipants(projectName);
		projectSummary.setUsersNames(usersNames);
		
		List<String> retrieveProjectLayers = this.retrieveProjectLayers(projectName);
	...
	public List<ProjectSummary> fetchProjectSummariesForPrincipal(Principal principal) throws Exception {
		...
		for(Project project2 : projects2) {
			...
			ProjectSummary projectSummary = new ProjectSummary();
			projectSummary.setClient(project2.getClient());
			projectSummary.setId(project2.getId().toString());
			projectSummary.setName(project2.getName());
			projectSummary.setDescription(project2.getDescription());
			projectSummary.setCreator(project2.getCreator().getName());
			projectSummary.setNumberOfUsers(project2.getPrincipalProject().size());
			projectSummary.setNumberOfLayers(projectTermDao.findByProject(project2).size());
			projectSummary.setStartDate(project2.getCreationDate().getTime());
			projectSummary.setClient(project2.getClient());
			
			if(project2.getShape() != null){
				projectSummary.setShape(shapeManager.getGeometry(project2.getShape()));
			}
...
//		}
	...
	@Transactional(rollbackFor={Exception.class})
	public void createNewProject(Project project, NewProjectData npd) throws Exception {
		
		project.setStatus(ProjectStatus.ACTIVE);
		project.setClient(npd.getCoords().getExtentString());
		if(project.getCreator() == null){ 
			throw new Exception("No creator assigned to project");
		}
...
//		}
		...
		if(npd.getUsers() != null && npd.getUsers().length != 0){
			for(int i=0;i<npd.getUsers().length;i++){
				if(npd.getUsers()[i] != null){
					Principal principal = principalManager.getActivePrincipalByName(npd.getUsers()[i]);
					if(principal != null){
		...
		if(npd.getLayers() != null && npd.getLayers().getJstreeLayers() != null){
			for(int i=0;i<npd.getLayers().getJstreeLayers().length;i++){
				if(npd.getLayers().getJstreeLayers()[i] != null){
					TaxonomyTerm term = taxonomyManager.findTermByName(npd.getLayers().getJstreeLayers()[i], false);
					if(term != null){
						ProjectTerm projectTerm = new ProjectTerm();
						projectTerm.setProject(project);
						projectTerm.setTerm(term);
						projectTerm.setCreator(project.getCreator());
						projectTermDao.create(projectTerm);
					}
				}
			}
	...
	@Transactional(readOnly=true)
	public List<String> retrieveProjectLayers(String projectName) throws Exception{
		Project project = this.findByName(projectName);
		
		List<TaxonomyTerm> layerTerms = this.projectTermDao.findByProject(project);
		List<String> layers = new ArrayList<String>();
		if(layerTerms != null){
			for(TaxonomyTerm t : layerTerms){
				layers.add(t.getName());
			}
		}
	...
	public GenericResponse createNewProject(NewProjectData npd, Tenant tenant) throws Exception {
		...
		if(!currentPrincipal.getName().equals(npd.getUserinfoObject().getFullname())){
			creator = principalManager.createPrincipalIfNotExists(npd.getUserinfoObject(), tenant);
		} else {
			creator = currentPrincipal;
		}
		
//		Principal creator = principalManager.createPrincipalIfNotExists(npd.getUserinfoObject(), tenant);
		...
		Project oldPorject = this.findByNameAndCreator(npd.getNameAndDescriptionObject().getName(), creator, false);
		if(oldPorject != null) {
...
//		project.setLastUpdate(Calendar.getInstance().getTime());
		project.setShape(null);
		project.setTenant(creator.getTenant());
		
		this.createNewProject(project, npd);
		return new GenericResponse(Status.Success, creator, "Ok");
	}
	...
	@Transactional(rollbackFor=Exception.class)
	public void updateCurrentProject(NewProjectData npd) throws Exception {
		...
		Project oldProject = this.findByName(npd.getNameAndDescriptionObject().getOldName());
		...
		if(npd.getCoords() != null && npd.getCoords().getExtent() != null) {
			oldProject.setClient(npd.getCoords().getExtentString());
		}
...
//		}
		...
		if(npd.getUsers() != null && npd.getUsers().length != 0){
			principalProjectDao.deleteByProjectId(oldProject.getId());
			...
			for(int i=0;i<npd.getUsers().length;i++){
				if(npd.getUsers()[i] != null){
					...
					if(principal != null){
						PrincipalProject pp = new PrincipalProject();
						pp.setParticipant(principal);
						pp.setProject(oldProject);
						oldProject.getPrincipalProject().add(pp);
					}
				}
			}
		}
		...
		if(npd.getLayers() != null && npd.getLayers().getJstreeLayers() != null){
			...
			for(int i=0;i<npd.getLayers().getJstreeLayers().length;i++){
				if(npd.getLayers().getJstreeLayers()[i] != null){
					...
					if(term != null){
						ProjectTerm projectTerm = new ProjectTerm();
						projectTerm.setProject(oldProject);
						projectTerm.setTerm(term);
						projectTerm.setCreator(oldProject.getCreator());
						projectTermDao.create(projectTerm);
					}
				}
			}
...
//			}
	...
	@Transactional(readOnly = true)
	public List<String> fetchProjectParticipants(String projectName){
		...
		try {
			...
			if(participants != null){
				for(PrincipalProject pp : participants){
					participantsNames.add(pp.getParticipant().getName());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
