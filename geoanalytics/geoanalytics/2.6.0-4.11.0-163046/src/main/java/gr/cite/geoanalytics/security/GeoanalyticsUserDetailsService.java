...
public class GeoanalyticsUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException {
		...
		try {
			Principal principal = principalManager.getPrincipalByNameAndActivity(userName, null);
			if (principal == null || principal.getPrincipalData() == null || principal.getPrincipalData().getCredential() == null){
				principal = principalManager.getPrincipalById(UUIDGenerator.systemUserUUID());
			}
//				throw new UsernameNotFoundException("User " + userName + " not found");
			...
			return new org.springframework.security.core.userdetails.User(principal.getName(), principal.getPrincipalData().getCredential(),
					principal.getIsActive() == ActiveStatus.ACTIVE.code(), new Date().getTime() < principal.getPrincipalData().getExpirationDate().getTime(), true, ActiveStatus.fromCode(principal.getIsActive()) != ActiveStatus.LOCKED,
					grantedAuthorities);
    ...
    public UserDetails loadUserByVreUsrIdAndTenantOrCreateOrUpdate(UUID vreId, String tenantName, String principalName, String principalEmail, String principalInitials) {
    	try {
			Principal principal = principalManager.findByVreIdAndTenantOrByNameAndTenantOrCreateOrUpdate(vreId, tenantName, principalName, principalEmail, principalInitials);
			if (principal == null || principal.getPrincipalData() == null || principal.getPrincipalData().getCredential() == null){
				principal = principalManager.getPrincipalById(UUIDGenerator.systemUserUUID());
			}
			...
			return new GeoanalyticsAuthenticatedUser(principal.getName(), principal.getPrincipalData().getCredential(),
					ActiveStatus.fromCode(principal.getIsActive()) == ActiveStatus.ACTIVE, new Date().getTime() < principal.getPrincipalData().getExpirationDate().getTime(), true, ActiveStatus.fromCode(principal.getIsActive()) != ActiveStatus.LOCKED,
					grantedAuthorities, vreId, principal.getTenant().getId());

		} catch (UsernameNotFoundException ue) {
			log.error("Exception while authenticating user " + principalName, ue);
			System.out.println("Exception while authenticating user " + principalName);
			throw ue;
		} catch (Exception e) {
			log.error("Exception while authenticating user " + principalName, e);
			System.out.println("Exception while authenticating user " + principalName);
		}
