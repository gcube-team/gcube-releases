...
public class GeoanalyticsPortalPreAuthenticationProvider extends PreAuthenticatedAuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		 ...
		 try
		  {
			 GeoanalyticsPreauthenticationToken gpt = (GeoanalyticsPreauthenticationToken)authentication.getPrincipal();
			 tenantId = gpt.getTenantId();
			 vreUsrId = gpt.getVreUsrUUID();
			 String principalName = authentication.getName();
			 principal = principalManager.findByVreIdAndTenantOrByNameAndTenant(vreUsrId, tenantId, principalName);
...
//				layerTaxonomies = configurationManager.retrieveTaxonomyConfig(TaxonomyConfig.Type.LAYERTAXONOMY, false);
	        ...
	        return new GeoanalyticsPreauthenticationToken(auth.getPrincipal(), auth.getCredentials(), auth.getAuthorities(), layers, allLayersIDs, vreUsrId, tenantId);
