...
public class PrincipalManager {
	...
	@Transactional(readOnly=true)
	public List<String> listPrincipalNamesOfProjectGroupNameAndTenant(String tenantname, String groupName) {
		return this.principalMembershipDao.listPrincipalNamesOfProjectGroupByNameAndTenant(tenantname, groupName);
	}
	...
	@Transactional(rollbackFor=Exception.class)
	public List<PrincipalProjectInfoDao> collectInfoOfActivePrincipals(String creatorName, String tenantName) throws Exception {
		List<PrincipalProjectInfoDao> principalsInfo = new ArrayList<PrincipalProjectInfoDao>();
		
		principalsInfo = principalDao.findByClassReturnsPrincipalProjectInfo(PrincipalClass.ITEM, ActiveStatus.ACTIVE, tenantName);
	...
	public List<ProjectGroupInfo> collectInfoOfActiveProjectGroups(Tenant tenant, Principal principal) throws Exception {
		...
		projectGroups = principalDao.findByClassAndCreatorName(
				PrincipalClass.PROJECT_GROUP, ActiveStatus.ACTIVE, tenant.getName(), principal);
		
		List<ProjectGroupInfo> projectGroupsNames = new ArrayList<ProjectGroupInfo>();
		projectGroups.forEach(p -> {
			projectGroupsNames.add(new ProjectGroupInfo(p.getName(), p.getCreator().getName(), p.getGroupsPrincipal().size()));
		});
	...
	@Transactional(rollbackFor=Exception.class)
	public void assignPrincipalsToProjectGroup(List<String> principalNames, Tenant tenant, String projectGroupName){
		Principal projectGroup = principalDao.findByClassTenantAndName(PrincipalClass.PROJECT_GROUP, ActiveStatus.ACTIVE, tenant.getName(), projectGroupName);
		
		if(projectGroup == null){
			throw new RuntimeException("No project group with this name exists in the database");
		}
		
		principalMembershipDao.deletePrincipalGroupMembers(projectGroup);
		
		for(String memberName : principalNames){
			PrincipalMembership pm = new PrincipalMembership();
			pm.setGroup(projectGroup);
			Principal member = this.findActivePrincipalByNameAndTenant(memberName, tenant.getName());
			pm.setMember(member);
			principalMembershipDao.create(pm);
			projectGroup.getGroupsPrincipal().add(pm);
		}
	...
	@Transactional(rollbackFor=Exception.class)
	public List<PrincipalProjectInfoDao> collectInfoOfActivePrincipalsAndNumOfMembersByTenant(String tenantName, Principal principal) throws Exception {
		List<PrincipalProjectInfoDao> principalsInfo = new ArrayList<PrincipalProjectInfoDao>();
		principalsInfo = principalDao.retrieveProjectGroupByTenant(PrincipalClass.PROJECT_GROUP, ActiveStatus.ACTIVE, tenantName, principal);
