...
private static final Logger log = LoggerFactory.getLogger(ProjectManager.class);
	...
	@Transactional(readOnly = true)
	public ProjectSummary fetchProjectSummariesForProjectNameAndTenant(String projectName, String tenant) throws Exception {
		List<Project> projects = projectDao.findByNameAndTenant(projectName, tenant);
		...
		List<ProjectParticipantInfo> ppi = this.fetchProjectParticipants(projectName, tenant);
		ppi.forEach(p -> {
		...
		List<String> retrieveProjectLayers = this.retrieveProjectLayers(projectName, tenant);
	...
	@Transactional(readOnly = true)
	public List<ProjectSummary> fetchProjectSummariesForPrincipal(Principal principal, String tenantName) throws Exception {
		
		List<ProjectSummary> summaries = new ArrayList<ProjectSummary>();
		List<Project> projects = projectDao.findActiveByCreatorAndTenant(principal, tenantName);
		for(Project project : projects) {
			summaries.add(buildProjectSummaryFromProject(project));
		}
		
		List<Project> projects2 = principalProjectDao.selectProjectByParticipant(principal, tenantName);
		for(Project project2 : projects2) {
			summaries.add(buildProjectSummaryFromProject(project2));
		}
		
		List<Project> projects3 = principalProjectDao.selectProjectByParticipantInGroup(principal, tenantName);
...
//		}
		...
		if(npd.getUsers() != null && npd.getUsers().length != 0){
			for(int i=0;i<npd.getUsers().length;i++){
				if(npd.getUsers()[i] != null){
					Principal principal = principalManager.findActivePrincipalByNameAndTenant(npd.getUsers()[i], npd.getUserinfoObject().getTenant());
					if(principal != null){
		...
		projectAccessControl.setDelete((short) 1);
		projectAccessControl.setEdit((short) 1);
		projectAccessControl.setRead((short) 1);
			...
			for(int i=0;i<npd.getUsers().length;i++){
				if(npd.getUsers()[i] != null){
					...
					if(principal != null){
						...
						projectAC.setDelete((short) 1);
						projectAC.setEdit((short) 1);
						projectAC.setRead((short) 1);
	...
	@Transactional(readOnly=true)
	public List<String> retrieveProjectLayers(String projectName, String tenant) throws Exception{
		Project project = this.findByNameAndTenant(projectName, tenant);
...
//		project.setLastUpdate(Calendar.getInstance().getTime());
	...
	public void updateCurrentProject(NewProjectData npd, Project oldProject) throws Exception {
		oldProject = this.findByName(npd.getNameAndDescriptionObject().getOldName());
...
//			}
	...
	@Transactional(readOnly = true)
	public List<ProjectParticipantInfo> fetchProjectParticipants(String projectName, String tenant){
