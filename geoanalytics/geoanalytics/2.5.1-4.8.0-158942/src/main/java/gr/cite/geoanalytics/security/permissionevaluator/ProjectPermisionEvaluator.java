...
public class ProjectPermisionEvaluator {
	...
	public Boolean hasPermissionReadProject(Object projectId){
		Boolean hasPermission = false;
		try{
			Principal principal = securityContextAccessor.getPrincipal();
			hasPermission = this.accessControlDao.hasPrincipalReadPermissionForEntity(principal, UUID.fromString(projectId.toString()));
		}catch (Exception e) {
			log.error("Error during project permission check", e);
			hasPermission = false;
		}
	...
	public Boolean hasPermissionEditProject(Object projectId){
		Boolean hasPermission = false;
		try{
			Principal principal = securityContextAccessor.getPrincipal();
			hasPermission = this.accessControlDao.hasPrincipalEditPermissionForEntity(principal, UUID.fromString(projectId.toString()));
		}catch (Exception e) {
			log.error("Error during project permission check", e);
			hasPermission = false;
		}
	...
	public Boolean hasPermissionDeleteProject(Object projectId){
		Boolean hasPermission = false;
		try{
			Principal principal = securityContextAccessor.getPrincipal();
			hasPermission = this.accessControlDao.hasPrincipalDeletePermissionForEntity(principal, UUID.fromString(projectId.toString()));
		}catch (Exception e) {
			log.error("Error during project permission check", e);
			hasPermission = false;
		}
