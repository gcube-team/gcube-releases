...
function showCreateProject(ev, defs, postProcessingFuncs)
{
	...
	$('#addProjectFormSaveButton').off('click').click(ev, function(ev) 
			 { 
				updateProject(null, true, [{func: retrieveProjects, args: [null, defs, [{func: function(){ $('#addProjectPopup').modal('hide'); window.mapLayers.editableVector.removeAllFeatures(); disableDrawingControls();}, args:[]}]]}]); 
			 });
}
...
function updateProject(id, create, postProcessingFuncs)
{
	...
	//req.project.shape = wkt.write(window.mapLayers.editableVector.features[0].geometry);
...
/*	$(document).ajaxSuccess(function(event,request,settings){
	...
	$('#tools-info-coords-gwm').click(function() {
	...
	$('#user-widget #logout input[type=submit], #user-widget p a').val($('body').hasClass('anonymous') ?  defs.i18n.nav.login : defs.i18n.nav.logout);
...
function show(defs, layers, bounds)
{
	init(defs);
	if(enhanceScrollbars())
		$('.scrollbar-test').jScrollPane({mouseWheelSpeed: 40});

	OpenLayers.DOTS_PER_INCH = 25.4 / 0.28;
	var projWGS84 = new OpenLayers.Projection("EPSG:4326");
	var projWebMercator = new OpenLayers.Projection("EPSG:900913");
	if(isPresent(bounds) && isPresent(bounds.minx) && isPresent(bounds.miny) && isPresent(bounds.maxx) && isPresent(bounds.maxy))
		bounds = new OpenLayers.Bounds(bounds.minx, bounds.miny, bounds.maxx, bounds.maxy);
	else
		bounds = new OpenLayers.Bounds(19.2734, 34.2271, 28.8975, 42.0493);
	bounds.transform(projWGS84, projWebMercator);
	
	var options = {
					maxExtent: bounds,
					/*scales: [50000000, 20000000, 10000000, 5000000, 2000000, 1000000, 500000, 200000, 100000, 50000, 30000, 20000, 10000,
       ...
       //map.addLayer(wms);
		...
		for(var i=0; i<layerNames.length; i++)
		{
			...
			var wms_layer = new OpenLayers.Layer.WMS(
				...
				"wms",
				{
					layers: defs.layerDatastore + ":" + layerNames[i],
					transparent: "true",
					format: "image/png",
					bgcolor: "0xcccccc"
				},
		...
		map.addLayers(wms_layers);
		map.addLayer(window.mapLayers.editableVector);
		map.addLayer(window.mapLayers.vector);
		
		markers = new OpenLayers.Layer.Markers("geopolis-markers");
        map.addLayer(markers);
        window.mapLayers.markers = markers;
...
*/		                box: true
        ...
        map.events.register('click', map, function (e) {
        	...
        	for(var i=0; i<layerNames.length; i++)
        	{
        		lNames += defs.layerDatastore + ":" + layerNames[i];
