		...
		try {
			...
			while (en.hasMoreElements()) {
			    ...
			    if(je.isDirectory() || !je.getName().endsWith(".class") || je.getName().contains("SimpleApp")){
			    	System.out.println("Continuing for:" + je.getName());
			        continue;
			    }
			    // -6 because of .class
...
//							this.layerNameUtil(execDetailsMap.getValue().getLayerID()),
							this.layerNameUtil(execDetailsMap.getValue().getLayerName()),
	...
	@Transactional(readOnly=true)
	public String layerNameUtil(String encodedLayerName) {		
		try {
			encodedLayerName = URLDecoder.decode( encodedLayerName, "UTF-8" );
		} catch (UnsupportedEncodingException e) {
			log.error("Error while decoding layerName: " + encodedLayerName);
		}
		return encodedLayerName;
	}
}