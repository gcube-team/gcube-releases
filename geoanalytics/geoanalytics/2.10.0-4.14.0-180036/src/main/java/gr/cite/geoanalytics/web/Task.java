	...
	public RequestResult resultReportStatus(String aURL, String layerID, HttpServletRequest request, HttpServletResponse response) throws MalformedURLException {
		...
		try {
			...
			while (headerNames.hasMoreElements()) {
                ...
                if (headerName != null && !headerName.equalsIgnoreCase("Transfer-Encoding") && !headerName.equalsIgnoreCase("accept-encoding"))
			...
			for (Map.Entry<String, List<String>> header : connection.getHeaderFields().entrySet()) {
				...
				if (header.getKey() != null && !header.getKey().equalsIgnoreCase("Transfer-Encoding") && !header.getKey().equalsIgnoreCase("accept-encoding"))
					response.setHeader(header.getKey(), vals.get(0));
				vals.remove(0);
				for (String val : new HashSet<String>(vals))
					if (!header.getKey().equalsIgnoreCase("Transfer-Encoding") && !header.getKey().equalsIgnoreCase("accept-encoding"))
