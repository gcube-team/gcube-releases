...
public class WcsClient extends OwsClient {
	...
	public byte[] downloadCoverage(String url, String workspace, String name, String outputFormat) throws CustomException {
...
//		String outputFormat = "geotiff";
		...
		try {
			...
			logger.error("Could not open connection to " + url);
			logger.error("Failed to download coverage");
		} finally {
			if(is != null){
				try {
					is.close();
				} catch (IOException e) {
					logger.error("Could not close downloaded coverage inputstream of " + url);
				}
			}
		}
