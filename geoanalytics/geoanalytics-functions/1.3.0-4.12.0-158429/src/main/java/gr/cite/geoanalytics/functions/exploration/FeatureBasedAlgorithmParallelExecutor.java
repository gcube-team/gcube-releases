...
public class FeatureBasedAlgorithmParallelExecutor {
	...
	public List<ShapeMessenger> executeForPartition(double minimumX, double minimumY, double maximumX, double maximumY,
			Iterable<CoordinateFilter> filters, Function function, GeometryFactory gFactory, String crs,
			FeatureSource<SimpleFeatureType, SimpleFeature>... layerSources) throws Exception {

		double distance = JTS.orthodromicDistance(new Coordinate(minimumX, maximumY),
				new Coordinate(maximumX, maximumY), CRS.decode(crs));
		...
		Iterable<SimpleFeature> paraktiesFeatures = FeatureRetrievalHelper.getShapes(layerSources[0], "SHP_Geography",
				minimumX, minimumY, maximumX, maximumY, crs);

//		Iterable<SimpleFeature> natura2000Features = FeatureRetrievalHelper.getShapes(layerSources[1], "SHP_Geography",
//				minimumX, minimumY, maximumX, maximumY, crs);
...
//		Iterable<SimpleFeature> difference = LayerOperations.difference(paraktiesFeatures, natura2000Features);
		...
		for (double x = minimumX; x < maximumX; x += sampleDistance) {
			for (double y = minimumY; y < maximumY; y += sampleDistance) {
				...
				Iterator<SimpleFeature> iterator = paraktiesFeatures.iterator();
				...
				if (contained == true) {
					try {
						...
						System.out.println("Could either not compute the function for the point "+x+" , "+y+" or package it in a shape object. Details: "+ex.getMessage());
						logger.debug("Could either not compute the function for the point "+x+" , "+y+" or package it in a shape object. Details: "+ex.getMessage());
					}
				}
