...
@Component
public class IrrSeaBreamParallel {

	@Autowired
	private AquacultureAlgorithm irrSeaBreamAlgorithm;

	private ExecutionNotifier executionNotifier;

	private static final Logger logger = LoggerFactory.getLogger(IrrSeaBreamParallel.class);
			...
			new Thread() {
				public void run() {
					...
					SparkConf conf = new SparkConf().setMaster(executionParameters.getSparkEndpoint())
							...
							.setJars(jars)
							.setAppName("IrrSeaBreamParallel minX:" + minimumX + " maxX:" + maximumX + " minY:" + minimumY + " maxY:" + maximumY);
					...
					try {
						irrSeaBreamAlgorithm.execute(executionID, sparkContext, layerName, styleName, projectID, tenantName, tenantID, creatorID, DISTANCE_METERS,
	...
	private void prepareClassloader(String appContextFilename, ClassLoader loader) {
		...
		irrSeaBreamAlgorithm = context.getBean(IrrSeaBreamAlgorithmParallel.class);
