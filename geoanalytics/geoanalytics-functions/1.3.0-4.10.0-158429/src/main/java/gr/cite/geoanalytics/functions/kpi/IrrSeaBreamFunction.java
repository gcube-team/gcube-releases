...
public class IrrSeaBreamFunction implements Function, Serializable {

	private static final long serialVersionUID = -6915800403768631565L;

	private static final Long speciesId = 2L;

	private String tenant;

	private EconomicsEvaluator economicsEvaluator;

	public IrrSeaBreamFunction(String tenant, Map<String, String> credentials) throws Exception {
		this.tenant = tenant;
	...
	public List<Attribute> execute(double x, double y) throws Exception {
		try {
			...
			throw new Exception("Could not calculate Sea Bream IRR for long lat [" + x + ", " + y + "]", e);
		}
	}
