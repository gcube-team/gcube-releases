...
public class IrrSeaBreamAlgorithmParallel implements AquacultureAlgorithm, Serializable {

	private static final long serialVersionUID = 3033238757748897716L;

	private static final Logger logger = LoggerFactory.getLogger(IrrSeaBreamAlgorithmParallel.class);
	...
	public IrrSeaBreamAlgorithmParallel(int hPartitions, int vPartitions) {
	...
	public void execute(String execID, JavaSparkContext sparkContext, String layerName, String styleName, String projectID, String tenantName, String tenantID, String creatorID,
			int scanStepMeters, FunctionLayerConfigI functionLayerConfig, String crsCode, double minimumX, double minimumY, double maximumX, double maximumY,
		...
		try {
			gosDefinition = trafficShaper.getGosForNewLayer();
		} catch (NoAvailableGos nag) {
		}
		...
		if (gosDefinition == null || gosDefinition.getGosEndpoint() == null || gosDefinition.getGosEndpoint().isEmpty()) // will
																															// not
																															// be
																															// able
																															// to
		...
		for (int i = 0; i < hPartitions; i++) {
			for (int j = 0; j < vPartitions; j++) {
				list.add(new Tuple5<Double, Double, Double, Double, Integer>(minimumX + i * xDistance, minimumY + j * yDistance, minimumX + (i + 1) * xDistance,
						minimumY + (j + 1) * yDistance, cnt++));
			}
		}
		...
		JavaRDD<List<ShapeMessenger>> simpleFeaturesListRDD = partitions.map(s -> {
			...
			Function function = new IrrSeaBreamFunction(tenantName, credentials);
			...
			try {
				return new AquacultureAlgorithmExecutor(scanStepMeters).executeForPartition(s._1(), s._2(), s._3(), s._4(), filters, function, crsCode,
						paraktiesSource);
			} catch (Exception ex) {
				ex.printStackTrace();
