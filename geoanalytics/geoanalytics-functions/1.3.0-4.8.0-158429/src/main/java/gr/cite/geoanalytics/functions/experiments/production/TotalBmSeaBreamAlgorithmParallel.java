...
import gr.cite.geoanalytics.functions.production.kpis.TotalBmSeaBreamFunction;
import scala.Tuple5;

@Configurable
//@PropertySource("classpath*:**/runtime.properties")
public class TotalBmSeaBreamAlgorithmParallel implements AquacultureAlgorithm, Serializable{

	private static final long serialVersionUID = 3033238757748897716L;

	private static final Logger logger = LoggerFactory.getLogger(TotalBmSeaBreamAlgorithmParallel.class);
	...
	public TotalBmSeaBreamAlgorithmParallel(int hPartitions, int vPartitions) {
	...
	public void execute(
		...
		JavaRDD<List<ShapeMessenger>> simpleFeaturesListRDD = partitions.map(s -> {
			...
			Function function = new TotalBmSeaBreamFunction(tenantName, credentials);
