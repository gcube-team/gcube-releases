...
import gr.cite.geoanalytics.functions.production.kpis.TotalBmSeaBassFunction;
import scala.Tuple5;

@Configurable
//@PropertySource("classpath*:**/runtime.properties")
public class TotalBmSeaBassAlgorithmParallel implements AquacultureAlgorithm, Serializable{

	private static final long serialVersionUID = 3033238757748897716L;

	private static final Logger logger = LoggerFactory.getLogger(TotalBmSeaBassAlgorithmParallel.class);
	...
	public TotalBmSeaBassAlgorithmParallel(int hPartitions, int vPartitions) {
	...
	public void execute(
		...
		JavaRDD<List<ShapeMessenger>> simpleFeaturesListRDD = partitions.map(s -> {
			...
			Function function = new TotalBmSeaBassFunction(tenantName, credentials);
