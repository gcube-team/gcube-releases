...
//	@Autowired private FeatureBasedAlgorithmParallelSimAn featureBasedAlgorithmParallelSimAn;
	...
	//		String tenantName = executionParameters.getTenantName();
			...
			logger.info("Style for the produced layer: "+ styleName);
			...
			logger.info("Executed notifyAbout");
			...
			new Thread(){
				...
				public void run(){
					...
					try{
						...
						featureBasedAlgorithmParallel.execute(executionID, sparkContext, layerName, styleName, projectID, tenantName, tenantID, creatorID, DISTANCE_METERS, executionParameters.getFunctionLayerConfig(), sst, FEATURE_TYPE_CRS, minimumX, minimumY, maximumX, maximumY, credentials);
					}
					catch(Exception ex){
						ex.printStackTrace();
