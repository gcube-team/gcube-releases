...
@Component
public class FoodConsumptionSeaBassParallel {

	@Autowired private AquacultureAlgorithm foodConsumptionSeaBassAlgorithm;
	
	private ExecutionNotifier executionNotifier;
	
	private static final Logger logger = LoggerFactory.getLogger(FoodConsumptionSeaBassParallel.class);
	...
	public FunctionResponse initialize(Map<String, Object> UIParameters) {
		...
		try{
			...
			String styleName = executionParameters.getResultingLayerName();
			...
			new Thread(){
				public void run(){
					...
					SparkConf conf = new SparkConf()
							...
							.setJars(jars)
							.setAppName("FoodConsumptionSeaBassParallel minX:"+minimumX+" maxX:"+maximumX+" minY:"+minimumY+" maxY:"+maximumY);
					...
					try{
						foodConsumptionSeaBassAlgorithm.execute(executionID, sparkContext, layerName, styleName, projectID, tenantName, tenantID, creatorID, DISTANCE_METERS, executionParameters.getFunctionLayerConfig(), FEATURE_TYPE_CRS, minimumX, minimumY, maximumX, maximumY, credentials);
					}
					catch(Exception ex){
						ex.printStackTrace();
	...
	private void prepareClassloader(String appContextFilename, ClassLoader loader) {
		...
		foodConsumptionSeaBassAlgorithm = context.getBean(FoodConsumptionSeaBassAlgorithmParallel.class);
