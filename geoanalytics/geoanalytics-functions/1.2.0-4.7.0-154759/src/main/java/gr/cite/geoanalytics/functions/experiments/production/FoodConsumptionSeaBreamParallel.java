...
@Component
public class FoodConsumptionSeaBreamParallel {

	@Autowired private AquacultureAlgorithm foodConsumptionSeaBreamAlgorithm;
	
	private ExecutionNotifier executionNotifier;
	
	private static final Logger logger = LoggerFactory.getLogger(FoodConsumptionSeaBreamParallel.class);
			...
			new Thread(){
				public void run(){
					...
					SparkConf conf = new SparkConf()
							...
							.setJars(jars)
							.setAppName("FoodConsumptionSeaBreamParallel minX:"+minimumX+" maxX:"+maximumX+" minY:"+minimumY+" maxY:"+maximumY);
					...
					try{
						foodConsumptionSeaBreamAlgorithm.execute(executionID, sparkContext, layerName, stylename, projectID, tenantName, tenantID, creatorID, DISTANCE_METERS, executionParameters.getFunctionLayerConfig(), FEATURE_TYPE_CRS, minimumX, minimumY, maximumX, maximumY, credentials);
					}
					catch(Exception ex){
						ex.printStackTrace();
	...
	private void prepareClassloader(String appContextFilename, ClassLoader loader) {
		...
		foodConsumptionSeaBreamAlgorithm = context.getBean(FoodConsumptionSeaBreamAlgorithmParallel.class);
