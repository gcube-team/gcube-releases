...
public class FeatureBasedAlgorithmParallelSimAn implements FeatureBasedAlgorithmParallelI, Serializable{

	private static final long serialVersionUID = 3033238757748897716L;

	private static final Logger logger = LoggerFactory.getLogger(FeatureBasedAlgorithmParallelSimAn.class);
	...
	public FeatureBasedAlgorithmParallelSimAn(int hPartitions, int vPartitions) {
...
//		final String natura2000LayerID = functionLayerConfig.getLayerConfigByObjectID("2").getLayerID();
		...
		Function function = 
				new RandomNPV();
//				new NPVFunction(sst);
//				new NPVFunction().setTenant(tenantName);
...
//			filters.add(cityDistanceFilter);
			...
			try {
				return new FeatureBasedAlgorithmParallelExecutorSimAn(scanStepMeters).executeForPartition(
					s._1(),	s._2(), s._3(), s._4(), 
					function,
		...
		geoanalyticsStore.storeToGeoanalytics(execID, layerName, styleName, tenantID, creatorID, projectID, gosDefinition, simpleFeaturesListRDD, crsCode, function.getResultsSchema());
