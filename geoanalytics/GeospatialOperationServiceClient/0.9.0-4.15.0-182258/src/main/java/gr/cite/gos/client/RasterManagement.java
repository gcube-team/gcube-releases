...
//		errorHandling(response, CREATED, "Could not insert GeoTIFF image in database. GOS is unavailable");
	...
	public void
	createCoverage(String gosEndpoint, Coverage coverage, InputStream coverageStream) throws Exception {
...
//		final StreamDataBodyPart filePart = new StreamDataBodyPart("file", coverageStream);
	...
	//				.field("file", coverageStream, MediaType.APPLICATION_OCTET_STREAM_TYPE);
			...
			System.out.println("Sending coverage to gos");
			...
			errorHandling(response, CREATED, "Could not insert coverage image in database. GOS is unavailable");
		}
		finally {
			bodyPart.cleanup();
