...
public class EnvironmentInitializer {
			...
			if (layerNotExists) {
				try {
					...
					if (DataSource.isGeoTIFF(layerConfig.getDataSource())) {
						//TODO uncomment when Blob postgres fixed
					...
					log.error("Could not initialize layer " + layerConfig.getName() + ":" + layerConfig.getLayerId() + " in GeoServer", e);
				}
			}
		}
	...
	public void initializeGeoserverEnvironment(List<LayerConfig> layerConfigs, SystemPresentationConfig systemPresentationConfig, List<StyleMessenger> styles) throws Exception {
		log.info("Inside initializeGeoserverEnvironment...");
		assertWorkspace();
		assertDataStores();
