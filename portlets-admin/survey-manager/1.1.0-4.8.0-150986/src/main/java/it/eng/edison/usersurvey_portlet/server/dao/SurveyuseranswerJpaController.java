...
public class SurveyuseranswerJpaController implements Serializable {
     ...
     * @return the list
     */
    public List<Surveyuseranswer> findAnswersSurveyById(int idSurvey){
    	Query query = null;
    	List<Surveyuseranswer> SurveyuseranswerList = null;
        EntityManager em = getEntityManager();
        try {
       		em.getTransaction().begin();
       		query = em.createQuery("SELECT s FROM Surveyuseranswer s WHERE s.idSurvey.id = :idSurvey");
       		query.setParameter("idSurvey", idSurvey);
       		SurveyuseranswerList = query.getResultList();
    	} catch (Exception e) {
    		e.printStackTrace();
        } finally {
    		em.getTransaction().commit();
    ...
    public int deleteQuestions(int idSurveyQuestion){
    	...
    	try {
    		em.getTransaction().begin();
    		query = em.createQuery("DELETE FROM Surveyuseranswer where id_survey = :idSurveyQuestion");
