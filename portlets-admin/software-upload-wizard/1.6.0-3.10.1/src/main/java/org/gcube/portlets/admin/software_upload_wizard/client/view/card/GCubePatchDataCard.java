...
public class GCubePatchDataCard extends WizardCard {
	...
	private GenericComponentInfoFieldSet softwareFieldSet = new GenericComponentInfoFieldSet("Software");
	
	private String packageId;

	public GCubePatchDataCard(String packageId) {
		super("Edit Patch data");
		this.packageId = packageId;
		
		InfoPanel stepInfo = new InfoPanel();
		stepInfo.setText(Resources.INSTANCE.stepInfo_GCubePatchData().getText());
	...
	private void loadData() {
		...
		dispatchAsync.execute(new GetPackageData(packageId), new AsyncCallback<GetPackageDataResult>() {
			...
			public void onSuccess(GetPackageDataResult result) {
				...
				softwareFieldSet.setDescription(packageData.getDescription());
				softwareFieldSet.setVersion(packageData.getVersion());
				
				GCubePatchDataCard.this.unmask();
	...
	private <T extends GwtEvent<H>, H extends EventHandler> void saveDataAndFireEvent(
		...
		packageData.setDescription(softwareFieldSet
				.getDescription());
		packageData.setVersion(softwareFieldSet.getVersion());
		
		Log.debug("Saving data...");
		dispatchAsync.execute(new BatchAction(OnException.ROLLBACK,new SetServiceData(serviceData),new SetPackageData(packageId, packageData)), new AsyncCallback<BatchResult>() {
	...
	public String getHelpContent() {
		return Resources.INSTANCE.stepHelp_GCubePatchData().getText();
	}
