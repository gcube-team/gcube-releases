...
public class TokenWidget extends Composite {
	...
	private void requireUsernameAndPort(){
		...
		//		tokenServices.getCurrentUser(new AsyncCallback<String>() {
		//
		//			@Override
		...
		//				if(result != null){
		//
		//					currentUsername.setText(result);
		//
		//
		//
		//				}else{
	...
	public void onGenerateClick(ClickEvent ce) {
		...
		if(nodeAddress.getText() == null || nodeAddress.getText().trim().isEmpty()){
			...
			tokenServices.createNodeToken(nodeAddress.getText().trim(), portNumber, 
