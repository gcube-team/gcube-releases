...
public class FhnManagerController {
	...
	static{
		...
		eventBus.addHandler(RefreshGridEvent.TYPE, new RefreshGridEventHandler() {
			...
			public void onRefreshGrid(RefreshGridEvent event) {
				refreshGrid(event.getOptions(),event.getCascade());
			}
		});
		...
		eventBus.addHandler(RetrievedGridElementsEvent.TYPE, new RetrievedGridElementsEventHandler() {
			
			@Override
			public void onRetrievedElements(RetrievedGridElementsEvent event) {
				
				showLoading("Refresh Grid", "Populating grid..");
				Set<Storable> theResult=((SerializableStorableSet)event.getTheResult()).getTheSet();
				logger.log(Level.FINE,"Received data "+theResult.size());
				event.getTheDataContainer().setData(castStorableList(event.getStorableType(), theResult));
	...
	private static void refreshGrid(final RefreshGridOptions options,FutureEvent cascade){
		logger.log(Level.FINE,"Refresh Grid Event, type : "+options.getType());
		FhnManagerEntryPoint.managerService.listResources(options.getType(), options.getFilters(),new AsyncLoader("Refresh Grid", "Accessing cache.. ", cascade,false)); 
	}

	private static void removeElement(ObjectType type,String id, Map<String,Boolean> flags,FutureEvent cascadeEvent){		
	...
	private static void create(ObjectType type,Map<String,String> fields, FutureEvent cascadeEvent){	
	...
	private static void startNode(String nodeId,FutureEvent cascadeEvent){		
		FhnManagerEntryPoint.managerService.startNode(nodeId, new AsyncLoader("Start Node", "Sending request", cascadeEvent));
	}

	private static void stopNode(String nodeId,FutureEvent cascadeEvent){		
