...
public class RemoteServiceImpl implements VMManagerServiceInterface {
	...
	public List<VMTemplate> getVMTemplates(String serviceProfileId,
		...
		try{
			...
			Collection<ResourceTemplate> result=client.findResourceTemplate(vmProviderId);
			return ModelTranslation.toVMTemplates(result);
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	public RemoteNode createNode(String serviceProfileId, String vmTemplateId,
		...
		try{
			...
			Node toConvert=client.createNode(vmProviderId,serviceProfileId,vmTemplateId);
			if(toConvert==null) throw new Exception("Selected provider didn't provide information on created node. Please refresh Remote Node grid to retrieve them.");
			else return ModelTranslation.toClient(toConvert);
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
