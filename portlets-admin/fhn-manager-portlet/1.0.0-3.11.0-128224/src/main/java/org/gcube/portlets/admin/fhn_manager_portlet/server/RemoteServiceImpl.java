...
public class RemoteServiceImpl implements VMManagerServiceInterface {
	...
	private static FHNManagerClient getClient() throws IllegalArgumentException, MalformedURLException{
		...
		else return FHNManagerProxy.getService(new URL(toUseEndpoint)).build();
	}
	...
	ServiceException {		
		try {
			return ModelTranslation.toServiceProfiles(getClient().allServiceProfiles());
		} catch (Exception e) {
	...
	public List<VMTemplate> getVMTemplates(String serviceProfileId,
		...
		try{
			return ModelTranslation.toVMTemplates(getClient().findResourceTemplate(vmProviderId));
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	public List<VMProvider> getVMProviders(String serviceProfileId,
		...
		try{
			return ModelTranslation.toVMProviders(getClient().findVMProviders(serviceProfileId));
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	public List<RemoteNode> getNodes(String serviceProfileId,
			String vmProviderId) throws RemoteException, ServiceException {
		try{
			return ModelTranslation.toRemoteNodes(getClient().findNodes(serviceProfileId, vmProviderId));
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	public RemoteNode createNode(String serviceProfileId, String vmTemplateId,
			String vmProviderId) throws RemoteException, ServiceException {
		try{
			return ModelTranslation.toClient(getClient().createNode(vmProviderId,serviceProfileId,vmTemplateId));
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	ServiceException {
		try{
			getClient().startNode(remoteNodeId);
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
	...
	ServiceException {
		try{
			getClient().stopNode(remoteNodeId);
		} catch (Exception e) {
			logger.error("Unexpected error from server ",e);
			throw new ServiceException("Unexpected error from server : "+e.getMessage());
		}
