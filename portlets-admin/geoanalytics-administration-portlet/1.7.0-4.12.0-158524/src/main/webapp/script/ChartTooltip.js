...
window.Chart = function(context, options){
	...
	var Line = function(data,config,ctx){
		...
		function drawLines(animPc){
			var zeroX = yAxisPosX;
			var zeroY = xAxisPosY - animPc*(calculateOffset(0,calculatedScale,scaleHop));
			for (var i=0; i<data.datasets.length; i++){
				ctx.strokeStyle = data.datasets[i].strokeColor;
				ctx.lineWidth = config.datasetStrokeWidth;
				ctx.beginPath();
				...
				if (config.datasetFill){
					...
					var positive = true;
					var varying = false;
					positive = data.datasets[i].data[cnt] > 0;
					for(cnt=0; cnt<data.datasets[i].data.length; cnt++) {
						if((positive && data.datasets[i].data[cnt] <= 0) || (!positive && data.datasets[i].data[cnt] >= 0)) {
							varying = true;
							break;
						}
					}
					if(!config.fillAroundZeroAxis) {
						...
						var lastDataCnt = -1;
						cnt = 0;
						while(cnt < data.datasets[i].data.length) {
							ctx.beginPath();
							positive = data.datasets[i].data[cnt] > 0;
							if(!lastDataPoint.x)
								ctx.moveTo(yAxisPosX, xAxisPosY - animPc*(calculateOffset(data.datasets[i].data[cnt], calculatedScale, scaleHop)));
							else
								ctx.moveTo(lastDataPoint.x, lastDataPoint.y);
							while(cnt < data.datasets[i].data.length) {
								var pt = data.datasets[i].data[cnt+1];
								if((positive && pt && pt <= 0) || (!positive && pt && pt >= 0)) {
									var intersectionX = -data.datasets[i].data[cnt]/(data.datasets[i].data[cnt+1]-data.datasets[i].data[cnt])+cnt;
									tmpDataPoint.x = xPos(intersectionX);
									tmpDataPoint.y = zeroY;
									lastDataCnt = cnt;
									ctx.lineTo(xPos(cnt), yPos(i, cnt));
									cnt++;
									break;
								}else {
									ctx.lineTo(xPos(cnt), yPos(i, cnt));
							...
							if(!lastDataPoint.x || cnt === data.datasets[i].data.length) {
								if((!varying && cnt === data.datasets[i].data.length) || (varying && cnt === data.datasets[i].data.length)) {
									var nonVaryingY;
									if(positive)
										nonVaryingY = valueBounds.minValue < 0 ? zeroY : yPos(i, valueBounds.minIndex);
									else
										nonVaryingY = valueBounds.maxValue > 0 ? zeroY : yPos(i, valueBounds.maxIndex);
									//ctx.lineTo(xPos(data.datasets[i].data.length), nonVaryingY);
										ctx.lineTo(yAxisPosX + (valueHop*(data.datasets[i].data.length-1)),nonVaryingY);
										ctx.lineTo(xPos(0), nonVaryingY);
									//ctx.lineTo(xPos(0), )
								}else {
									prevIntersection = {};
									prevIntersection.x = lastDataPoint.x;
									prevIntersection.y = lastDataPoint.y;
									lastDataPoint.x = tmpDataPoint.x;
									lastDataPoint.y = tmpDataPoint.y;
									ctx.lineTo(lastDataPoint.x, lastDataPoint.y);
									
									//ctx.lineTo(xPos(intersectionX), zeroY);
									ctx.lineTo(zeroX, zeroY);
									ctx.lineTo(xPos(0), yPos(i,0));
								}
							}else {
								ctx.lineTo(tmpDataPoint.x, zeroY);
								ctx.lineTo(prevIntersection.x, prevIntersection.y);
								prevIntersection = {};
								prevIntersection.x = lastDataPoint.x;
								prevIntersection.y = lastDataPoint.y;
								lastDataPoint.x = tmpDataPoint.x;
		...
		function getValueBounds() {
			...
			for (var i=0; i<data.datasets.length; i++){
				for (var j=0; j<data.datasets[i].data.length; j++){
					if ( data.datasets[i].data[j] > upperValue) { upperValue = data.datasets[i].data[j]; upperIndex = j; };
					if ( data.datasets[i].data[j] < lowerValue) { lowerValue = data.datasets[i].data[j]; lowerIndex = j; };
				}
			};
