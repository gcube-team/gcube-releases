...
function tabsEvents(resourceURL) {
	$('.importTab').one('click', function() {
		...
		$('#importOptions .dropDownSelection').append('<option id="netcdfImport">NetCDF Import</option>');
		$('#importOptions .dropDownSelection').append('<option id="geojsonImport">GeoJson Import</option>');
		...
		$('#importOptions .dropDownSelection').change(function(){
			...
			} else if ($(this).children(":selected").attr('id') == 'geojsonImport'){
                var stylesPath = window.config.createResourceURL("styles/getAllStyles");
                var importGeojsonPath = window.config.createResourceURL("import/geojson");

                $('#tab3').geojsonImporter({
                    mode				: 	"div" 	,					// or "button"
                    importGeojsonURL	:	importGeojsonPath,
                    stylesURL			:   stylesPath,
                    headerDiv			: 	"#importOptions",
                    content				: 	"#contentOfImporter"
                })
                .geojsonImporter("destroy")
                .geojsonImporter("createImporter");
            } else if ($(this).children(":selected").attr('id') == 'netcdfImport'){
                var stylesPath = window.config.createResourceURL("styles/getAllStyles");
                var importNetcdfPath = window.config.createResourceURL("import/netcdf");

                $('#tab3').netcdfImporter({
                    mode				: 	"div" 	,					// or "button"
                    importNetcdfURL: importNetcdfPath,
                    importNetcdfPath	:	importNetcdfPath,
                    stylesURL			:   stylesPath,
                    headerDiv			: 	"#importOptions",
                    content				: 	"#contentOfImporter"
                }).netcdfImporter("createImporter");
            }
