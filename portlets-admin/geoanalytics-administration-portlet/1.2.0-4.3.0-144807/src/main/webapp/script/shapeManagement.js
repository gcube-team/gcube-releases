...
function populateTermSelector(){
	...
	var url = createLink(pageState.rURL, 'admin/shapes/layers');
	...
	$.ajax({ 
        ...
        success : function(terms) {
        			for(var i=0; i<terms.length; i++) {
        				...
        				option.value = terms[i].layerID;
        				option.text = terms[i].geocodeSystem+":"+terms[i].layerName;
        				select.appendChild(option);
        			}
        			        			
        			$('#geoadmin-layers').on('change', function() {
        				pageState.taxonomyTermSelected = this.value;
        			});
        		  },
        		  error	: 	function(jqXHR, textStatus, errorThrown){
//      			  alert("The following error occured: " + jqXHR.responseText, jqXHR.status);
