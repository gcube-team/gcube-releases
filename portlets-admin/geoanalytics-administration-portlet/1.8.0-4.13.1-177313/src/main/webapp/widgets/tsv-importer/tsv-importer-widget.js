		...
		$(document).ready(function() {
			...
			$('#tsv-importer-import-button').on('click',function (e){
				...
				if(validData && ((publishOnGeoNetwork && validMetadata) || !publishOnGeoNetwork)){
					...
					let callback = function ( tsvFileData, fileName ) {
...
//							tags.push(spans[i]);
						...
						importFormData.append( "tsvFileData", encodeURI( tsvFileData ) );
//						importFormData.append("tsvImportFile", file);
						importFormData.append("tsvImportProperties", new Object([JSON.stringify({
							layerName		: 	encodeURI( $('#tsv-importer-layername').val() ),
							description		: 	encodeURI( $('#tsv-importer-abstract').val().trim() ),
							geocodeSystem	:	encodeURI( $('#tsv-importer-geocodesystem').val() ),
							style			:   encodeURI( $('#tsv-importer-style option:selected').text() ),
							tags			: 	tags,
						...
						if(publishOnGeoNetwork){
							let title = encodeURI( $('#tsv-importer-layername').val() );
							let description = encodeURI( $('#tsv-importer-abstract').val().trim() );
						...
						let theContentType = false;//"multipart/form-data";
	 					...
	 					$.ajax({
	 					    ...
	 					    processData : false,
	 					    cache : false,
	 		                contentType : theContentType,//;charset=utf-8",
	 					    data: importFormData,
	 					    beforeSend : function() {
					...
					self.getFileDataAsText( file, callback );
	...
	getFileDataAsText : function( file, callback ) {
