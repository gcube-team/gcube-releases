...
public class GeoanalyticsAdministrationHome extends GenericPortlet {
	...
	public Integer doRequest(String endpoint, ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException {
		...
		try {
			...
			logger.info("Status inside dorequest:" + status);
...
//						resourceResponse.setProperty(entry.getKey(), entry.getValue() != null ? entry.getValue().get(0) : null);
//					});
//				}
				
				
				logger.info("Connection contentType: " + connection.getContentType());
				if (connection.getContentType() != null) {
					resourceResponse.setContentType(connection.getContentType());
				}
				logger.info("Connection Disposition: " + connection.getHeaderField("Content-Disposition"));
