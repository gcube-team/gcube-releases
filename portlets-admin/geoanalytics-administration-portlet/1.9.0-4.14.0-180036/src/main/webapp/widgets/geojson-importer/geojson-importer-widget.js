$.widget('cite.geojsonImporter', {	
	isActive : false,
	notificator : $("#geojson-importer-notificator"),
	options:{
		mode: "div"	,
		importGeojsonURL: "",
		stylesURL: "",
	...
	_createShowButton : function() {
		var geojsonimporter = $('#geojsonimporter');	
		$(geojsonimporter).css("display", "inline-block");

		var showButton = 	'<button id="geojson-importer-show-button"  ' +
									'type="button" '+
									'class="btn btn-large" '+
									'data-toggle="modal" '+
									'data-target="#geojson-importer-container">	'	+								
								'<i class="fa fa-upload" ></i> GeoJson Importer	'+
							'</button>';
		geojsonimporter.append($(showButton));
	},	
	_createNonModal : function() {
		var container = '<div id="geojson-importer-container"> ';
		this._container = $(container);		
		this._container.appendTo(this.options.content);
	},	
	_createModal : function() {	
		var container = '<div id="geojson-importer-container" ' +
							'class="modal fade in geojson-importer-container-button"   ' +
							'tabindex="-1" ' +
							'role="dialog"	' +
							'aria-labelledby="geojsonimporter"  ' +
							'aria-hidden="true " ' +
							'style="display:none;"> ';			
		this._container = $(container);		
	...
	_createUI : function() {
		...
		if(this.options.mode === "button"){
			closeButton = '<button id="geojson-importer-close-button" type="button" data-dismiss="modal" aria-hidden="true"></button>';
			header = '<div id="geojson-importer-modal-header" class="modal-header">' +
				closeButton +
				'<h5 id="geojson-importer-label-modal">GeoJson Importer</h5> ' +
			'</div>';	
			body = '<div id="geojson-importer-modal-body" class="modal-body scrollable" >';
			scrollableRow = " scrollable";
		}else{
			body = '<div id="geojson-importer-modal-body" >';
		}

		var importerBody = header + body + 	'<input type="button" id="geojson-importer-toggle-geonetwork-metadata" />' +
		'<div class="row">' +
			'<div class="span6" id="geojson-importer-general-form-container">' +
				'<div class="spinner" style="display: none"></div>' +
				'<span class="headerDescription">GENERAL</span>	' +
				'<div id="geojson-importer-notificator"></div>	' +													
				'<hr>	' +
				'<form class="form-horizontal" id="geojson-importer-general-form">	' +
					'<div class="control-group row">		' +		
						...
						'<div class="span6">		' +													
							'<input id="geojson-importer-selected-file" class="span11"  type="text" placeholder="No geojson selected with json extension" readonly>' +
							'<br>' +
							'<span class="help-inline"></span>' +
						'</div>				' +			
						'<div class="controls span2" id="geojson-importer-patch-browsefiles">	' +														
							'<button class="span12">Browse</button>' +
							'<input id="geojson-importer-browsefiles-button" name="browseFiles" type="file" >	' +
						'</div>	' +
					'</div>		' +
					
					'<div class="control-group row">' +
						'<div class="span4">' +
							'<label for="geojson-importer-dbf-check">Set DBF charset</label>' +
						'</div>' +
						'<div class="span8">' +
							'<input type="checkbox" id="geojson-importer-dbf-check">' +
							'<label for="geojson-importer-dbf-check"></label>' +
						'</div>' +
					'</div>' +		
					...
					'<div class="control-group row">' +
						'<div class="span4">' +
							'<label for="geojson-importer-layername">Layer Name<span class="makeMeOrange">*</span></label>' +
						'</div>' +
						'<div class="span8">' +
							'<textarea  style="resize: none;" id="geojson-importer-layername"  class="span12"  name="layerName"  placeholder="Please fill in your Layer Name" rows="1"></textarea>' +
							'<span class="help-inline"></span>' +
						'</div>' +
					'</div>' +
					...
					'<div class="control-group row">' +
                        '<div class="span4">' +
                            '<label for=""geojson-importer-crs"">Choose CRS (if not included in file)<span class="makeMeOrange"></span></label>' +
                        '</div>' +
                        '<div class="span8">' +
                            '<select id="geojson-importer-crs" class="span12"  name="crs">' +
                                '<option  value="" disabled selected>Choose a CRS</option>' +
                                '<option  value="4326" >EPSG:4326 (WGS84)</option>' +
                                '<option  value="28992">EPSG:28992 (DUTCH RD)</option>' +
                                '<option  value="3785">EPSG:3785</option>' +
                                '<option  value="4055">EPSG:4055</option>' +
                                '<option  value="27700">EPSG:27700 (GB)</option>' +
                            '</select>' +
                            '<span class="help-inline"></span>' +
                        '</div>' +
                    '</div>	' +
					...
					'<div class="control-group row">' +
						'<div class="span4">' +
							'<label for=""geojson-importer-style"">Style<span class="makeMeOrange">*</span></label>' +
						'</div>' +
						'<div class="span8">' +
							'<select id="geojson-importer-style" class="span12"  name="style">' +
					...
					'<div class="control-group row">' +
						'<div class="span4">' +
							'<label for=""geojson-importer-template-layer"">Is Template Layer</label>' +
						'</div>' +
						'<div class="span8">' +
							'<input type="checkbox" id="geojson-importer-template-layer">' +
							'<label for="geojson-importer-template-layer"></label>' +
						'</div>' +
					'</div> ' +
					
					'<div class="control-group row" style="display:none; margin-top:20px;">' +
						'<div class="span4">' +
							'<label for="geojson-importer-template-layer" style="padding-left:20px;">' +
								'Template Layer Geocode System<span class="makeMeOrange">*</span>' +
							'</label>' +
						'</div> ' +
						'<div class="span8">' +
							'<textarea style="resize: none;" ' +
								'id="geojson-importer-template-layer-geocode-system" ' +
								'class="span12" ' +
								'rows="1" ' +
					...
					'<div class="control-group row" style="display:none;">'	+
						...
						'<div class="span8">'	+
							'<textarea style="resize: none;" '	+
								'id="geojson-importer-template-layer-geocode-mapping" '	+
								'class="span12" '	+
								'rows="2" '	+
								'placeholder="Please give the geojson attribute containing the Geocode" '	+							
								'name="geocodeMapping"></textarea>'	+
							'<span class="help-inline"></span>'	+
						'</div>'	+
					'</div>'	+	
					
					'<div class="control-group row" id="geojson-importer-general-description">' +
						'<div class="span4">' +
							'<label for="geojson-importer-abstract">Layer description<span class="makeMeOrange">*</span></label>' +
						'</div>' +
						'<div class="span8">' +
							'<textarea name="abstractDescription" id="geojson-importer-abstract" class="span12" placeholder="Please give a brief description of the layer" rows="5"></textarea>' +
						'</div>' +
					'</div>' +
									
					'<div class="control-group row" id="generalKeywords">' +
						'<div class="span4">' +
							'<label for="geojson-importer-tagsinput">Layer keywords<span class="makeMeOrange">*</span></label>' +
						'</div>' +
						'<div class="span8">' +
							'<textarea  style="resize: none;" id="geojson-importer-tagsinput" name="tagsInput" class="span12" placeholder="Please fill in the layer keywords" rows="1"></textarea>' +
						'</div>' +
					'</div>' +
				'</form>' +
			'</div>	' +
			...
			'<div class="span6">' +
				'<div id="geojson-importer-geonetwork-metadata"></div>' +
			'</div>' +
		'</div>'	;
		...
		this._container.append('<div class="modal-footer">' +
									'<div id="geojson-importer-div-submit" class="control-group pull-right">' +
										'<button type="button" data-dismiss="modal" id="geojson-importer-cancel" class="btn" aria-hidden="true">Cancel</button>' +
										'<button type="button" id="geojson-importer-import-button"  class="btn" >Import</button>' +
									'</div>' +				
								'</div>');	
	},
	_initializeFunctionality : function (){		
		var importGeojsonURL = this.options.importGeojsonURL;
		var stylesURL = this.options.stylesURL;
		var notificator = this.options.notificator;
		var toggleGeoNetworkMetadata = $("#geojson-importer-toggle-geonetwork-metadata");
		var geoNetworkModule = window.config.geoNetworkModule.createInstance("#geojson-importer-geonetwork-metadata");
		geoNetworkModule.createGeoNetworkForm();
		
		$(document).ready(function(){
			$('#geojson-importer-tagsinput').tagsInput({
			...
			$('#geojson-importer-browsefiles-button').bind("change", function() {
				var fileName = $(this).val().split('\\').pop();
				$('#geojson-importer-selected-file').val(fileName);
				var valid = $('#geojson-importer-browsefiles-button').valid();
				
				if (valid) {
					fileName += " has been selected";
					window.noty.showNoty($("#geojson-importer-notificator"), fileName, "success", false);
				} else {
			...
			$('#geojson-importer-general-form').validate({
				rules: {
					layerName: {
						minlength: 2,
					    required : true
				...
				highlight: function (element) {
					...
					$(element).closest('.control-group').find('.help-inline').addClass('geojson-importer-color-red');
					
					if($(element).attr('id') === 'geojson-importer-browsefiles-button'){
						$(element).closest('.control-group').find('#geojson-importer-patch-browsefiles').css("border-color","red");		
					}					
				},
				success: function (label, element) {
					$(element).closest('.control-group').removeClass('error');
					if($(element).attr('id') === 'geojson-importer-browsefiles-button'){
						$(element).closest('.control-group').find('#geojson-importer-patch-browsefiles').css("border-color","#DDD");		
					}
					label.remove();
				},		
				errorPlacement: function(error, element) {	
					if(element.attr('id') === 'geojson-importer-browsefiles-button'){
						...
						element.closest('.control-group').find('#geojson-importer-patch-browsefiles').addClass('error');			
					}else{
			...
			jQuery.extend(jQuery.validator.messages, {
				extension: "Please upload a file with .json or .geojson extension."
			});
			
			$("#geojson-importer-template-layer").change(function() {
				$("#geojson-importer-template-layer-geocode-system").closest(".control-group").toggle();
				$("#geojson-importer-template-layer-geocode-mapping").closest(".control-group").toggle();
			});
			
			$('#geojson-importer-dbf-check').change(function() {
		...
		$(document).ready(function() {
			$('#geojson-importer-cancel , #geojson-importer-close-button').on('click',function (e){ 
				$('#geojson-importer-general-form')[0].reset();
				geoNetworkModule.resetGeoNetworkForm();
				
				$("#geojson-importer-container").find("*").removeClass("geojson-importer-color-red error success");
				$('.help-inline > label').remove();
				
				window.noty.closeAllNotys();
				
				$('#geojson-importer-patch-browsefiles').css("border-color", "gray");
				$('#geojson-importer-tagsinput_tagsinput').find(".tag").remove();
			});
		});	
		...
		$(document).ready(function() {			
			$.ajax({
				...
				success: function(response) {
					$.each(response, function(i,v){
						...
						$('#geojson-importer-style').append($option);
					});
				},
			...
			var notificator = $("#geojson-importer-notificator");			
			
			$('#geojson-importer-import-button').on('click',function (e){
				var validData = $('#geojson-importer-general-form').valid();
				...
				if(validData && ((publishOnGeoNetwork && validMetadata) || !publishOnGeoNetwork)){				
					var spans = $('#geojson-importer-container .tagsinput').find(".tag > span");
					var tags = [];
...
//						tags.push(spans[i]);
					...
					let dbfUserInput = $('#geojson-importer-dbf-check').is(":checked");
					
					var file = document.getElementById('geojson-importer-browsefiles-button').files[0];					

					var importFormData = new FormData();
					importFormData.append("geojsonImportFile", file);
					importFormData.append("geojsonImportProperties", new Object([JSON.stringify({
						newLayerName	: 	encodeURI( $('#geojson-importer-layername').val() ),
						geocodeSystem	:	$("#geojson-importer-template-layer-geocode-system").val(),
						geocodeMapping	:	$("#geojson-importer-template-layer-geocode-mapping").val(),
						description		: 	encodeURI( $('#geojson-importer-abstract').val().trim() ),

						isTemplate		:	$("#geojson-importer-template-layer").is(":checked"),
						srid			    :   $('#geojson-importer-crs option:selected').val(),
						style			:   $('#geojson-importer-style option:selected').text(),
						tags			: 	tags,
					...
					if(publishOnGeoNetwork){
						let title = encodeURI( $('#geojson-importer-layername').val() );
						let description = encodeURI( $('#geojson-importer-abstract').val().trim() );
						...
						importFormData.append("geojsonImportMetadata", new Object([JSON.stringify({
							title			:	title,
 					...
 					$.ajax({
 					    type : "POST", 					    
 					    url : importGeojsonURL,
 					    processData : false, 					   
 		                contentType : false, 
 					    data: importFormData, 	
 					    beforeSend : function() {
 					    	$('#geojson-importer-container .spinner').show();
 					    },
					    ...
					    complete: function() {
					    	$('#geojson-importer-container .spinner').hide();
					    }
 					});				    
				}				
			});

//			$('#geojson-importer-add-legend-btn').on('click',function (e){
//                 importLegend.init();
//			});
		});
	},
