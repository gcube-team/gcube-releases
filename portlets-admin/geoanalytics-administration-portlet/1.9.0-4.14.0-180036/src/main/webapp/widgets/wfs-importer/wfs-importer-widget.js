$.widget("cite.WFSImport", {
	...
	_create : function() { 
		this.wfsImportLayoutHead ='	<div class="span4" id="urlInput">															\
										<input type="text" id="url" class="span12" placeholder="http://mydomain/geoserver/ows"> 				\
...
//		$(this.options.headerDiv).append(this.wfsImportLayoutHead);
	...
	_fetchLayers : function(pageState) {
		...
		var wfsRequestMessenger = {
			url: self._getURLUpToOws( pageState.url ),
		...
		$('#importWfsLayers').on('click', function(e) {
			...
			for(i=0; i < selectedLayers.length; i++){
				...
				var wfsReaquestLayer = {
					layerName : encodeURI( $('#modalLayerName'+i+'').val().trim() ),
					layerDescription :encodeURI(  $('#modalLayerDescription'+i+'').val().trim() ),
			...
			var wfsRequestMessenger = {
		    	url: widget._getURLUpToOws( $('#url').val() ),
