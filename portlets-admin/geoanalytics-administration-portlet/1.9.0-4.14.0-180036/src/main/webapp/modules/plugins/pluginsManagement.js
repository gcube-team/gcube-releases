			    ...
			    $('#geoadmin-plugins-datatable').PortletDataTable({
			    	ajax :	{
				        ...
				        dataSrc : function(data) {
				        	if(data !== '' && data !== null) {
				        		var locale = config.locale.replace("_","-");
				        		var options = { 
				        				year: 'numeric',
				        				month: 'short',
				        				day: 'numeric',
					        	...
					        	$.each(data, function(index, value) {
									...
									data[index]["updateDateObject"] = new Date(value.updateDate);
									data[index]["creationDateObject"] = new Date(value.creationDate)
									
					        		var updateDateString = new Date(value.updateDate).toLocaleString(locale, options);
					        		
					        		data[index].updateDate = self.formatDate(updateDateString);
//					        		data[index].updateDate = new Date(value.updateDate).toLocaleDateString(locale);
					        		
					        		var creationDateString = new Date(value.creationDate).toLocaleString(locale, options);
					        		data[index].creationDate = self.formatDate(creationDateString);
//					        		data[index].creationDate = new Date(value.creationDate).toLocaleDateString(locale);
