pageState = {};
var rURL;
function showTaxonomyManagement(resourceURL, data, notificator) {
	rURL = resourceURL;
...
//					$('#taxonomyConfigButton').prop('disabled', false);
	...
	$('table#taxonomyTbl button[value="Deactivate"]').css('visibility', 'collapse');
}
...
function createAddTaxonomyButton()
{
	...
	input.addEventListener('click', function() {
		$('#addTermModal').css('display', 'none');
		$('#addTaxonomyModal').css('display', 'block');
...
function createAddTermButton()
{
	...
	input.addEventListener('click', function() {
		$('#addTaxonomyModal').css('display', 'none');
		$('#addTermModal').css('display', 'block');
...
//    tr.appendChild(th);
    ...
    button.className = "btn btn-default noVisible";
    button.type = "button";
    button.value = "Delete";
    ...
    for (var i=0; i<values.length; i++) {
    	if (values[i].name === "taxonomyNames")
    		taxonomyNames.push(values[i].value);
    	else if (values[i].name === "termNames")
    		termNames.push(values[i].value);
    	else if (values[i].name === "activeTaxonomies") {
    		if (values[i].value === "on")
    			activeTaxonomies = true;
    		else activeTaxonomies = false;
    	} else if (values[i].name === "activeTerms") {
    		if (values[i].value === "on")
    			activeTerms = true;
    		else activeTerms = false;
    	}    	
    }
    
    
	var taxonomySearchSelection = {
	...
	postDataToServer(taxonomySearchSelection,  createLink(rURL, 'admin/taxonomies/search'), function(response) {
		if (response.status) {
...
//	var url = createLink(rURL, "admin/taxonomies/search");
//    $.ajax({ 
//        url : url,
//        type : "post", 
//        data :  JSON.stringify(taxonomySearchSelection),
...
//    tr.appendChild(th);
    ...
    if (reqTaxonomy == undefined) return;
    
    var url = createLink(rURL, "admin/taxonomies/retrieveTerms");
    ...
    if(isPresent($('#termTable')[0].term))
    {
    	...
    	if($('#termTable')[0].descendants == true)
    	{
    		...
    		url = createLink(rURL, "/taxonomies/retrieveTermDescendants");
    	}
    	if($('#termTable')[0].classDescendants == true) 
    	{
    		$('#termTableHeading > p').html("Class descendants of " + reqTaxonomy + ":" + reqTerm);
    		url = createLink(rURL, "/taxonomies/retrieveTermClassDescendants");
    	}
    }
    
    var taxonomyTransfer = {
    	taxonomyName: reqTaxonomy
	};
    ...
    $.ajax({ 
        ...
        dataType: "json",
        data : JSON.stringify(taxonomyTransfer),
        success : function(info) 
        		  { 
		        	var count = 0;
	        		...
	        		for(var i=0; i<info.length; i++)
	        		{
	        			...
	        			input.type = "button";
	        			input.className = "btn btn-sm btn-default noVisible";
	        			input.value = info[i].active ? "Deactivate" : "Activate";
	        			input.innerHTML = info[i].active ? "Deactivate" : "Activate";
	        			input.taxonomy = info[i].taxonomy;
	        			input.term = info[i].name;
	        			...
	        			input.type = "button";
	        			input.className = "btn btn-sm btn-primary noVisible";
...
function createTaxonomyConfigForm(postProcessingFuncs) {
	...
	postDataToServer(null,  createLink(rURL, 'admin/taxonomies/listTaxonomyConfig'), function(response) {
		if(response.status) {
			var taxonomyConfig = response.data;
			...
			pageState.notificator.notification("error", "The following error occured: " + response.message);
        	$("#notification").addClass("in");
		}
	});
...
function retrieveTaxonomyConfig(postProcessingFuncs) {
	var url = createLink(rURL, 'admin/taxonomies/retrieveTaxonomyConfig');
	$.ajax({ 
        url : url,
        type : "post", 
...
function populateTerms(ev)
{
	...
	var url = createLink(rURL, "admin/taxonomies/listTerms");
	...
	postDataToServer(taxonomyTransfer,  url, function(response) {
		if (response.status) {
...
function listTaxonomies(postProcessingFuncs, active)
{
	...
	var url = createLink(rURL, 'admin/taxonomies/listTaxonomies');
	...
	$.ajax({
		...
		success: function(response) {
			applyFuncs(postProcessingFuncs, response.data);
		},
		error : function(jqXHR, textStatus, errorThrown) {
        	pageState.notificator.notification("error", errorThrown);
        	$("#notification").addClass("in");
...
function taxonomyConfigSaveButtonListener(ev)
{
	...
	var url = createLink(rURL, 'admin/taxonomies/updateTaxonomyConfig');
	...
	$.ajax({
        ...
        success : function(response) {
        	...
        	pageState.notificator.notification("success", "TaxonomyConfig updated");
        	$("#notification").addClass("in");
...
function updateTaxonomy(ev, form, create, postProcessingFuncs)
{
    ...
    for (var i=0; i<values.length; i++) {
    	if (values[i].name === "name")
    		name = values[i].value;
    	else if (values[i].name === "originalName")
    		originalName = values[i].value;
    	else if (values[i].name === "taxonomyClass")
    		taxonomyClass = values[i].value;
    	else if (values[i].name === "extraData")
    		extraData = values[i].value;
    	else if (values[i].name === "userTaxonomy")
    		userTaxonomy = values[i].value;
    	else if (values[i].name === "active")
    		active = values[i].value;
    }
    
	var taxonomyMessenger = {
			name: name,
	...
	var url = createLink(rURL, create ? "admin/taxonomies/addTaxonomy" : "admin/taxonomies/updateTaxonomy");
    ...
    $.ajax({ 
	    ...
	    contentType: "application/json",
	    dataType: "json",
		data: JSON.stringify(taxonomyMessenger),
        success : function(response) 
        		  {
        			if(response.status == false) {
        			...
        			pageState.notificator.notification("success", "Taxonomy created/updated");
                	$("#notification").addClass("in");
        			
					createTaxonomyTable(response.data, false);
...
function deleteTaxonomies(ev, postProcessingFuncs)
{
	...
	var url = createLink(rURL, "admin/taxonomies/deleteTaxonomies");
	$.ajax({ 
        url : url,
        type : "post", 
        data : req,
        ...
        success : function(response) 
        		  {
        			...
        			pageState.notificator.notification("success", "Taxonomy deleted");
                	$("#notification").addClass("in");
...
function deleteTerms(ev, postProcessingFuncs)
{
	...
	var url = createLink(rURL, "admin/taxonomies/deleteTerms");
        ...
        success : function(response) 
        		  {
        			...
        			pageState.notificator.notification("success", "Term deleted");
                	$("#notification").addClass("in");
                	
	//				createTermTable(response.data, false);
	//				applyFuncs(postProcessingFuncs, response);
		          }
        		  ,
        error : function(jqXHR, textStatus, errorThrown) {
...
function updateTerm(ev, form, create, postProcessingFuncs)
{
	...
	var url = createLink(rURL, create ? "admin/taxonomies/addTerm" : "admin/taxonomies/updateTerm");
	...
	var mappedValue;
	var originalName;
	var originalTaxonomyName;
    
    for (var i=0; i<values.length; i++) {
    	if (values[i].name === "id")
    		id = values[i].value;
    	else if (values[i].name === "name")
    		name = values[i].value;
    	else if (values[i].name === "order")
    		order = values[i].value;
    	else if (values[i].name === "parentTaxonomy")
    		parentTaxonomy = values[i].value;
    	else if (values[i].name === "parentTerm")
    		parentTerm = values[i].value;
    	else if (values[i].name === "taxonomy")
    		taxonomy = values[i].value;
    	else if (values[i].name === "classTaxonomy")
    		classTaxonomy = values[i].value;
    	else if (values[i].name === "classTerm")
    		classTerm = values[i].value;
    	else if (values[i].name === "configuration")
    		configuration = values[i].value;
    	else if (values[i].name === "active")
    		active = values[i].value;
    	else if (values[i].name === "mappedValue")
    		mappedValue = values[i].value;
    	else if (values[i].name === "originalName")
    		originalName = values[i].value;
    	else if (values[i].name === "originalTaxonomyName")
    		originalTaxonomyName = values[i].value;
    }
    
	var taxonomyTermMessenger = {
			id: id,
			name: name,
			order: order,
			...
			originalName: originalName,
			originalTaxonomyName: originalTaxonomyName
	};
    ...
    $.ajax({
	    ...
	    contentType: "application/json",
	    dataType: "json",
		data: JSON.stringify(taxonomyTermMessenger),
        success : function(response)
        		  {
        			if(response.status == false) {
        				pageState.notificator.notification("error", response.message);
        			...
        			pageState.notificator.notification("success", "Term created/updated");
                	$("#notification").addClass("in");
