...
public class StorageChartContextPanel extends SimpleContainer {
	...
	public StorageChartContextPanel(EventBus eventBus,
	...
	public static native void onDownloadPNG(String id) /*-{
		...
		var chart = $wnd
				.jQuery('#' + id)
				.highcharts(
						this.@org.gcube.portlets.admin.accountingmanager.client.maindata.charts.storage.StorageChartContextPanel::options);
		console.log(chart);
		chart.exportChart();
	...
	private void createChart() {
		...
		if (!(seriesStorage.getSeriesStorageDefinition() instanceof SeriesStorageContext)) {
			Log.debug("Invalid SeriesStorageContext!");
			return;
		}
		SeriesStorageContext seriesStorageContext = (SeriesStorageContext) seriesStorage
		...
		options.exporting().filename("AccountingStorageContext");
		options.title().text("Accounting Storage Context");
		...
		/*if (seriesStorageContext.getShowOthers()) {
			createSeriesColumnShowOthers(seriesStorageContext, highchartsFactory,
					colors, seriesColumn);
		} else {*/
		createSeriesColumnSimple(seriesStorageContext, highchartsFactory,
					colors, seriesColumn);
		/*}*/
	...
	private void createSeriesColumnShowOthers(
			SeriesStorageContext seriesStorageContext,
		...
		if (accountingStateData.getSeriesRequest().getAccountingPeriod()
			...
			int i = 0;
			for (; i < seriesStorageContext.getSeriesStorageDataContextList().size()
					&& i < MAX_NUMBER_OF_CONTEXT; i++) {
				SeriesColumn seriesColumnData = highchartsFactory
						.createSeriesColumn();

				seriesColumnData.name(seriesStorageContext
						.getSeriesStorageDataContextList().get(i).getContext());
				seriesColumnData.color(colors.get(i));
				seriesColumnData.type("column");

				ArrayNumber data = seriesColumnData.dataAsArrayNumber();

				for (SeriesStorageData seriesStorageData : seriesStorageContext
						.getSeriesStorageDataContextList().get(i).getSeries()) {
					switch (measure) {
					case DataVolume:
			...
			if (i < seriesStorageContext.getSeriesStorageDataContextList().size()) {
				SeriesColumn seriesColumnData = highchartsFactory
						.createSeriesColumn();
				...
				for (int j = i; j < seriesStorageContext
						.getSeriesStorageDataContextList().size(); j++) {
					Long value;
					for (int k = 0; k < seriesStorageContext
							.getSeriesStorageDataContextList().get(j).getSeries()
							.size(); k++) {
						SeriesStorageData seriesStorageData = seriesStorageContext
								.getSeriesStorageDataContextList().get(j)
								.getSeries().get(k);
						if (j == i) {
							switch (measure) {
							case DataVolume:
	...
	private void createSeriesColumnSimple(SeriesStorageContext seriesStorageContext,
			HighchartsOptionFactory highchartsFactory, ArrayString colors,
			ArrayList<SeriesColumn> seriesColumn) {
		...
		if (accountingStateData.getSeriesRequest().getAccountingPeriod()
			...
			for (int i = 0; i < seriesStorageContext.getSeriesStorageDataContextList()
					.size(); i++) {
				SeriesColumn seriesColumnData = highchartsFactory
						.createSeriesColumn();
