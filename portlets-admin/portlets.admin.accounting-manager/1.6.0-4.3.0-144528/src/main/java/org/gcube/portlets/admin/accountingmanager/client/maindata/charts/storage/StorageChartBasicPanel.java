...
public class StorageChartBasicPanel extends SimpleContainer {
	...
	private void createChart() {
		...
		seriesDataVolume
		.point()
		.addClickHandler(new com.github.highcharts4gwt.model.highcharts.option.api.seriesarea.point.ClickHandler() {
			
			@Override
			public void onClick(
					com.github.highcharts4gwt.model.highcharts.option.api.seriesarea.point.ClickEvent clickEvent) {
				Log.debug("Series name: "
						+ clickEvent.point().series().name());
				Log.debug("Point Selected: ["
						+ clickEvent.point().x() + ", "
		...
		if (accountingStateData.getSeriesRequest().getAccountingPeriod()
			...
			Array<Data> arrayDataOperationCount = seriesOperationCount
					.dataAsArrayObject();
			Array<com.github.highcharts4gwt.model.highcharts.option.api.seriesarea.Data> arrayDataVolume = seriesDataVolume
					.dataAsArrayObject();
			...
			for (SeriesStorageData seriesStorageData : seriesStorageBasic
				...
				dataOperationCount.y(seriesStorageData.getOperationCount());
				arrayDataOperationCount.addToEnd(dataOperationCount);

				com.github.highcharts4gwt.model.highcharts.option.api.seriesarea.Data dataVolume = highchartsFactory
						.createSeriesAreaData();
				dataVolume.x(dateFrom1970);
				dataVolume.y(seriesStorageData.getDataVolume() / unitMeasure);
				arrayDataVolume.addToEnd(dataVolume);
