	...
	public static void getAccountingReportForRelease(int releaseInternalId){
		...
		try{
			...
			try{
				...
				List<String> packageIds = (List<String>) query.setParameter("release", daoRelease).getResultList();
				...
				List<Integer> listAccountingIds = (List<Integer>) query.setParameter("packages", packageIds).getResultList();
				
				//RETRIEVE LIST ACCOUNTING IDS
				queryString = "select sum(p.downloadNmb) as totaldownloadNmb, sum(p.javadocNmb) as totaljavadocNmb,  sum(p.sourcecodeNmb) as totalsourcecodeNmb, sum(p.wikiNmb) as totalwikiNmb"
