	...
	public AccountingReport getAccountingReportForRelease(String releaseInternalId) throws Exception{
		...
		try{
			...
			try{
				...
				//RETRIEVE LIST ACCOUNTING IDS
				queryString = "select sum(p.downloadNmb) as totaldownloadNmb, sum(p.javadocNmb) as totaljavadocNmb,  sum(p.mavenRepoNmb) as totalmavenrepoNmb, sum(p.wikiNmb) as totalwikiNmb"
