...
public class RsFromFileGeneratorOp {
	...
	/**
	 * Creates a new {@link RsFromFileGeneratorOp}
	 * 
	 * @param locator The input ResultSet
	 */
	public RsFromFileGeneratorOp(/*,StatsContainer stats*/){
		//this.stats=stats;
	}
	 ...
	 */
	public URI compute(String inFile, boolean onlyFinalEvent, int id, ProxyType proxyType, long timeout, TimeUnit timeUnit, Float threshold) throws Exception{
		try{	
			...
			RSFromFileGeneratorWorker worker = new RSFromFileGeneratorWorker(inFile, onlyFinalEvent, id, proxyType, timeout, timeUnit, threshold, synchWriter);
			worker.start();
			URI outLocator = null;
