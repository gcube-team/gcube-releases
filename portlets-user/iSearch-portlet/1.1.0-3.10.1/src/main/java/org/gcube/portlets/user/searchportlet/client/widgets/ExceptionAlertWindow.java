...
public class ExceptionAlertWindow extends GCubeDialog implements ClickHandler {
	...
	public void addDock(final Throwable caught)
	{
		...
		String msg;
		if (caught.getMessage() != null && !caught.getMessage().isEmpty())
...
//			cause = "";
		...
		add(createDockForErrorMessage(msg, emailBtn));
		setWidth("450px");
		...
		emailBtn.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				};SearchPortlet.searchService.sendEmailWithErrorToSupport(caught, sendEmailToSupportTeamCallback);
				showLoading();
	 ...
	 */
	private FlexTable createDockForErrorMessage(String errorMsg, Button b) 
	{
...
//		infoTable.getFlexCellFormatter().setAlignment(1, 1, HasHorizontalAlignment.ALIGN_LEFT, HasVerticalAlignment.ALIGN_MIDDLE);
		infoTable.setWidget(1, 0, new HTML("<br>"));
		infoTable.setWidget(2, 3, b);
		infoTable.getFlexCellFormatter().setAlignment(2, 3, HasHorizontalAlignment.ALIGN_RIGHT, HasVerticalAlignment.ALIGN_BOTTOM);
	...
	private void showLoading() {
		...
		int left = ExceptionAlertWindow.this.getAbsoluteLeft() + ExceptionAlertWindow.this.getOffsetWidth()/2;
		int top = ExceptionAlertWindow.this.getAbsoluteTop() + ExceptionAlertWindow.this.getOffsetHeight()/2;
