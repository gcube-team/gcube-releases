function initializeProjectSelectionTable(){
	...
	$('table#ProjectSelectionTable').off().on( 'init.dt', function(){
		        	...
		        	if(serverResponse.status === "Success"){
		        		...
		        		$.each(serverResponse.response, function(index, value) {
		        			tileElements.push(buildProjectTile(value.name, value.startDate, value.description, value.id,
		        					value.creator, value.numberOfUsers, 0, value.numberOfLayers, value.extent, value.rights));//[0].outerHTML);
		        		});
								...
								if(projectGroupNames[i].id === value.id){
									...
									$.each(trsData, function(ind,val){
										if(val[0] === value.projectGroupName){
											val[3] = createAssignRightsToUserCheckboxes(ind, rightsObject);
	...
	$.ajax({ 
        ...
        success: function(serverResponse){
        	if(serverResponse.status === "Success"){
        		...
        		var deleteThis = 0;
        		$.each(serverResponse.response, function(index, value) {
        			tileElements.push(buildProjectTile(value.name, value.startDate, value.description, value.creator, deleteThis, deleteThis, deleteThis, value.rights));//[0].outerHTML);
        			deleteThis++;
        		});
        		tileElementsGlobal = tileElements;
        		if(tileElements.length > 0) {
...
function buildProjectTile(projectName, projectDate, projDescription, projectId,
		projCreatorName, numOfUsers, numOfFunctions, numOfLayers, extent, rights){
	var div = '<div></div>';
	var span = '<span></span>';
