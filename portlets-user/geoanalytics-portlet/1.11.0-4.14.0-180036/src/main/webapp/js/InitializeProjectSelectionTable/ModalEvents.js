...
function allModalsEvents(){
	...
	$('#BBOXModal').on('shown', function() {
		...
		for(let ll in coords){
			previousCoords[ll] = coords[ll];
		}
		coords = {};
		mapBBOX.updateSize();
...
//			AJAX_Call_POST(bboxURL, callback, userinfoObject);
	...
	$('#ChooseLayersModal').on('shown', function(){
		...
		if(EDITMODE){
			...
			if(typeof coords.extent !== 'undefined')
				theExtent = coords.extent;
			else theExtent = previousCoords.extent;
			layersMap.getView().fit(theExtent, layersMap.getSize());
		}
...
//					showSpinner();
					//spinners next to nodes
					let $i = $('<i></i>', {
						class : 'fa fa-spinner fa-spin fa-x fa-fw'
					});
					let $spinnersArray = [];
					
					$('#treeviewLayers ul:first li > a.jstree-anchor').each(function() {
						let $spinner = $i.clone();
						$(this).append( $spinner );
						$spinnersArray.push($spinner);
					});
					//disable nodes
					let treeData = $('#treeviewLayers').jstree(true).get_json('#', {flat:true});
					$.each(treeData, function(i,v) {
						$('#treeviewLayers').jstree().disable_node(v.id);
					});
...
//						hideSpinner();
						//remove spinners
						$.each($spinnersArray, function(i, v) {
							$spinnersArray[i].remove();
						});
						//enable nodes
						$.each(treeData, function(i,v) {
							$('#treeviewLayers').jstree().enable_node(v.id);
						});
					};
...
function initializeMapForWizardModal() {
	...
	var geometryFunction = function(coordinates, geometry) {
        ...
        $('#coord0').text( restrictToFiveDigits(transformTo4326(coords.coord0)) );
        
        coords.coord1 = [extent[2], extent[1]];
        $('#coord1').text( restrictToFiveDigits( transformTo4326(coords.coord1)) );
        
        coords.coord2 = [extent[0], extent[3]];
        $('#coord2').text( restrictToFiveDigits( transformTo4326(coords.coord2)) );
        
        coords.coord3 = [extent[2], extent[3]];
        $('#coord3').text( restrictToFiveDigits( transformTo4326(coords.coord3)) );
...
function restrictToFiveDigits(coordsArray) {
	for(let i in coordsArray) {
		coordsArray[i] = Number(coordsArray[i]).toFixed(5);
	}
	return coordsArray;
}
...
//		$('#treeviewLayers').jstree().deselect_all(true);
		try {
			...
			console.log('jstree was not initialized');
		}
...
//			$( 'table#relateUsersGroupsToProjectsTable' ).DataTable().ajax.reload();
		}
	...
	$('.groupOrIndividual').on('click',function(){
		$('.groupOrIndividual').each(function() {
