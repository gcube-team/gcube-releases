(function(){
	...
	var functionExecutionMonitor = {
			...
			deleteExcecutionInfo: function(ExecutionInfoIDs) {
		    	...
		    	var selectedRowsData = functionExecutionMonitor.dataTable.getSelectedRowsData();
		    	
		    	var returnClause = false;
		    	
		    	$.each(selectedRowsData, function(index, value) {
		    		if($(value.status).text()==='INPROGRESS') {
		    			var obj = {};
		    			obj.responseText = "Function executions with status INPROGRESS cannot be deleted";
		    			
		    			functionExecutionMonitor.errorHandling(obj, null);
		    			returnClause = true;
		    			return false;
		    		}
		    	});
		    	...
		    	var parameters = "ExecutionInfoIDs=" + encodeURIComponent(ExecutionInfoIDs);
//		    	var url = window.config.createResourceURL(window.resourceURL, encodeURIComponent('plugin/deleteExecutionDetailsInfo'), parameters);
		    	var url = window.config.createResourceURL('plugin/deleteExecutionDetailsInfo');
...
//			    });
		    ...
		    reload : function() {
		    	...
		    	functionExecutionMonitor.clearMessage();
		    },
		    ...
		    showMessage : function(text, type) {
			    window.notificator.setText($("#geoanalytics-functions-execution-monitor-notificator"), text, type);
		    },
