...
//				selector : '',
				...
				content : function(){
    				...
    				$legend.find('i').attr('class', findCheckedControlELement($zoomSlider.attr('class')));
...
//				delay : ''
	...
	dockeGISControlButtonsPressed.legend = true;
...
function addClearMapControl() {
	...
	var clearMapControl = function(opt_options) {
        ...
        $button.click(clearTheMap);

        var $element = $('<div></div>',{
        	class:'clear-map ol-unselectable ol-control',
        	css : {
        		bottom : '2.3em',
        		right : '.5em'
        	}
  ...
  activeControls.clearMap = clearMapControl;
  
  return new clearMapControl();
}