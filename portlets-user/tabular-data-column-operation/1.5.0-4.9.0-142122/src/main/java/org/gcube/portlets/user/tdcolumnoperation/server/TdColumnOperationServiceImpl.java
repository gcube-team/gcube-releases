...
/**
 * The Class TdColumnOperationServiceImpl.
 *
...
public class TdColumnOperationServiceImpl extends RemoteServiceServlet implements TdColumnOperationService {
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#loadOperatorForOperationId(org.gcube.portlets.user.tdcolumnoperation.shared.OperationID)
	 */
	@Override
...
//							"Split as string with leading and trailing whitespace omitted"));
	 ...
	 *
	 * @param operationColumnSession the operation column session
	 * @return the string
...
//					SplitColumnSession splitSession = new SplitColumnSession(operationColumnSession.getFirstColumnData(), label1, label2, expressions);
					...
					taskId=gwtService.startSplitColumn(splitSession, this.getThreadLocalRequest());
...
//					MergeColumnSession mergeSession = new MergeColumnSession(operationColumnSession.getFirstColumnData(), label1, mergeExpression);
					...
					//returning task id
					taskId = gwtService.startMergeColumn(mergeSession,this.getThreadLocalRequest());
					logger.trace("Operation return task id: "+taskId, " returning");
				}
...
//				logger.trace("Converted expression 2 is :"+exp2);
	...
	/**
	 * Resolve merger operation.
	 *
	 * @param operationColumnSession the operation column session
	 * @param firstRef the first ref
	 * @param secondRef the second ref
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#getListAggregationFunctionIds()
	 */
	@Override
...
//		getAslSession(session);
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#getListTimeTypes()
	 */
	@Override
...
//		getAslSession(session);
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#startGroupByOperation(org.gcube.portlets.user.tdcolumnoperation.shared.AggregationColumnSession)
	 */
	@Override
		...
		try{
			if(aggregationSession.getGroupColumns()!=null && !aggregationSession.getGroupColumns().isEmpty()){
				...
				if(aggregationSession.getAggregateFunctionPairs()!=null && !aggregationSession.getAggregateFunctionPairs().isEmpty()){
					...
					TDGWTServiceImpl gwtService = new TDGWTServiceImpl();
					String taskId = gwtService.startGroupBy(new GroupBySession(aggregationSession.getTrId(), map), this.getThreadLocalRequest());
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#startAggregateByTimeOperation(org.gcube.portlets.user.tdcolumnoperation.shared.AggregationColumnSession, org.gcube.portlets.user.tdcolumnoperation.shared.TdPeriodType, java.util.List)
	 */
	@Override
		...
		try{
			if(aggregationSession.getGroupColumns()!=null && !aggregationSession.getGroupColumns().isEmpty()){
				...
				if(aggregationSession.getAggregateFunctionPairs()!=null && !aggregationSession.getAggregateFunctionPairs().isEmpty()){
					...
					String taskId = gwtService.startTimeAggregation(new TimeAggregationSession(aggregationSession.getTrId(),timeDimensionsColumns.get(0), map), this.getThreadLocalRequest());
					logger.info("Returning task id: "+taskId +" generated by GroupByOperation");
					return taskId;
				}else
	 ...
	 *
	 * @param aggregationSession the aggregation session
	 ...
	 *
	 * @return the td tabular data service
	 */
	protected TdTabularDataService columnOperationService(){

		try {
			ServiceCredentials sc =  SessionUtil.getServiceCredentials(this.getThreadLocalRequest());
			return new TdTabularDataService(sc.getScope(), sc.getUserName());
		} catch (Exception e) {
	...
	/* (non-Javadoc)
	 * @see org.gcube.portlets.user.tdcolumnoperation.client.rpc.TdColumnOperationService#getSuperiorPeriodType(java.lang.String)
	 */
	@Override
