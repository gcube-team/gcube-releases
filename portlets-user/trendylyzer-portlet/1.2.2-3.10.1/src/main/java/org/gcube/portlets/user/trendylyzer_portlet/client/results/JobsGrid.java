...
public class JobsGrid extends ContentPanel {
	...
	private static final ImageResource ICON_CANCEL = TrendyLyzer_portlet.resources.cancel();
	private static final ImageResource ICON_RESUBMIT = TrendyLyzer_portlet.resources.resubmit();
	boolean dirty=true;
...
//						updateJobItemStatus(jobItem);
	...
	private void updateJobItemStatus(final JobItem jobItem) {
		TrendyLyzer_portlet.getService().getComputationStatus(jobItem.getId(), new AsyncCallback<ComputationStatus>() {
			@Override
	...
	private void loadGrid() {
		...
		RpcProxy<ListLoadResult<JobItem>> proxy = new RpcProxy<ListLoadResult<JobItem>>() {
			...
			protected void load(Object loadConfig, AsyncCallback<ListLoadResult<JobItem>> callback) {
				TrendyLyzer_portlet.getService().getListJobs(callback);
			}
		};
		...
		GridCellRenderer<JobItem> renderer = new GridCellRenderer<JobItem>() {
			...
			public String render(JobItem jobItem, String property, ColumnData config, int rowIndex, int colIndex, ListStore<JobItem> store, Grid<JobItem> grid) {
				if (property.contentEquals("categoryImage")) {
					AlgorithmCategory category = jobItem.getOperatorCategory();
					return "< category/>";
...
//							+(category.hasImage() ? category.getId() : "DEFAULT_IMAGE")+".png' width='25' height='25' border='1' alt='"+category.getName()+"' />";
					...
					Algorithm op = jobItem.getOperator();
...
//		configs.add(column);
		...
		// creation date
		column = new ColumnConfig("creationDate", "Start Date", 100);
		...
		// end date
		column = new ColumnConfig("endDate", "End Date", 100);
		column.setDateTimeFormat(DateTimeFormat.getFormat("MM/dd/yyyy'<br/>'hh:mm:ss"));
		column.setAlignment(HorizontalAlignment.CENTER);		
		column.setResizable(false);
		configs.add(column);
		...
		// status
		column = new ColumnConfig("status", "Status", 170);
...
//						logger.log(Level.SEVERE, "click remove job");
...
//						MessageBox.confirm("Confirm removing", message, new Listener<MessageBoxEvent>() {
//							@Override
//							public void handleEvent(MessageBoxEvent be) {
//								if (be.getButtonClicked().getTitle().contentEquals("Yes"))
//									logger.log(Level.SEVERE, "confirm remove");
//									removecomputation(j);
//							}
//						});
//					}
...
//		configs.add(columnButton);  
	...
	private void removecomputation(final JobItem jobItem) {
		logger.log(Level.SEVERE, "inside remove computations");
