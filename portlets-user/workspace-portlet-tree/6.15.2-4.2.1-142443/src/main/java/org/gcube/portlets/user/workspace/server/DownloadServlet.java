...
public class DownloadServlet extends HttpServlet{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			try{
				if(userID==null || Long.valueOf(userID)==-1){
					logger.error("User ID passed is null or -1, throwing session expired exception");
					throw new SessionExpiredException();
				}
			}catch(NumberFormatException e){
				logger.error("User ID passed is not a number, throwing session expired exception");
				throw new SessionExpiredException();
			}
