...
public class AppControllerExplorer implements EventHandler, TreeAppControllerInterface{
	...
	private int waitTiming = 3000; //TO WAIT REPOSITORY SYNC
						...
						public void handleEvent(BaseEvent be) {
							...
							if(finalDialog.isValidForm(true)){
								...
								rpcWorkspaceService.shareFolder(fileModel, finalDialog.getSharedListUsers(), isNewFolder, finalDialog.getSelectedACL(), new AsyncCallback<Boolean>() {
									...
									public void onSuccess(Boolean result) {
										if(result){
											FileModel parentToRefresh= isNewFolder?parentModel:parentFileModel;
											final FileModel toRefresh = parentToRefresh;
											...
											Timer t = new Timer() {
										      ...
										      public void run() {
										    	GWT.log("Runing refresh after wait: "+waitTiming);
												//TODO UPDATED ID
												RefreshFolderEvent refEvent = new RefreshFolderEvent(toRefresh, true, true, false);
		...
		eventBus.addHandler(FileUploadEvent.TYPE, new FileUploadEventHandler() {
			...
			private void doFileUploadEvent(final FileUploadEvent fileUploadEvent) {
				...
				WorskpaceUploadNotificationListener listener = new WorskpaceUploadNotificationListener() {
					...
					@Override
					public void onUploadCompleted(final String parentId, final String itemId) {
					...
					@Override
					public void onOverwriteCompleted(final String parentId, final String itemId) {
