...
public class PanelVRE extends Composite {
	...
	private Grid getHeaderPanel(VRE vre) {
		...
		String name = (vre.getName().length() > 23) ? vre.getName().substring(0, 23) : vre.getName();
		HTML vrelabel = new HTML(name);
	...
	private void Init() {
		...
		if (this.vre.getUserBelonging() == UserBelonging.BELONGING) {
			...
			if (!vre.isUponRequest()) 
					enter_Button = new ActionButton(vre, ButtonType.FREE);
				else
	...
	public ActionButton getEnter_Button() {
		return enter_Button;
	}
