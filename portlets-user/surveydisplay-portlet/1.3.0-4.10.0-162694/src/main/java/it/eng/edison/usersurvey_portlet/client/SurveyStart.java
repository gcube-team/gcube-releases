...
public class SurveyStart extends Composite {
	...
	private void getUserFromSession() {
		Image loadingImage = new Image(resources.loadingImage());
		RootPanel.get("displaysurvey-div").add(new Paragraph("Loading questionnaire, please wait ..."));
		RootPanel.get("displaysurvey-div").add(new Image(resources.loadingImage()));
		...
		greetingService.getUser(new AsyncCallback<UserDTO>() {
			...
			public void onSuccess(UserDTO result) {
				...
				if((userDTO != null) && (isSurveyManager()) && (UUIDExtractFromUrl == EMPTY_TOKEN)){
				...
				} else if((userDTO != null) && UUIDExtractFromUrl.equalsIgnoreCase(EMPTY_TOKEN)) {
					RootPanel.get("displaysurvey-div").clear();
					UserAnswerNotAnonymousSurveyView userAnswerNotAnonymousSurveyView = new UserAnswerNotAnonymousSurveyView(userDTO);
					RootPanel.get("displaysurvey-div").add(userAnswerNotAnonymousSurveyView);
				} else if(!UUIDExtractFromUrl.equalsIgnoreCase(EMPTY_TOKEN) && userDTO.getUserId() == 0){ //user has to compile the survey and is not logged in
					...
					//user has to compile the survey and is logged in
			...
			public void onFailure(Throwable caught) {
				RootPanel.get("displaysurvey-div").clear();
				RootPanel.get("displaysurvey-div").add(new Paragraph("An error occurred."));
			}
		});		
	}
