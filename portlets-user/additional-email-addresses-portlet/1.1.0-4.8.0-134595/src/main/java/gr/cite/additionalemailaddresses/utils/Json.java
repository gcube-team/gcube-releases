...
public class Json {
	 ...
	 * @param fieldName the name of the liferay Object in the json
	 */
	 ...
	 * @param object The liferay Object tha needs to be serialized and returned to the FrontEnd
	 */
	public static void returnDeepJson(ResourceResponse response, Object object){
		
		JSONSerializer jsonSerializer = JSONFactoryUtil.createJSONSerializer();
		
		try {
			PrintWriter writer = response.getWriter();
			writer.write(jsonSerializer.exclude("*.class").serializeDeep(object).toString());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
