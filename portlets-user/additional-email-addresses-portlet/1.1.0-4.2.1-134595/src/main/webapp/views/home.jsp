...
// String infoMessage = LanguageUtil.get(pageContext, "Add additional email addresses to your account to enable logging in with additional authentication providers under the same account");
%>
...
<p class="subtitle" id="ptitle" data-toggle="popover" data-trigger="hover" data-placement="top" data-original-title="Add additional email addresses to your account to enable logging in with different authentication providers under the same account"><%= LanguageUtil.get(pageContext, "associate-more-emails-to-your-account (?)") %></p>
...
<%-- 			var selectPrimaryEmailAddress = '<%= selectPrimaryEmailAddress %>'; --%>
			...
			additionalEmailAddressesNS.init(sendEmailVerification, listAdditionalEmailAddresses, removeAdditionalEmail, resendVerificationEmail, maxNumEmails, isEmailAddressAlreadyUsed);
