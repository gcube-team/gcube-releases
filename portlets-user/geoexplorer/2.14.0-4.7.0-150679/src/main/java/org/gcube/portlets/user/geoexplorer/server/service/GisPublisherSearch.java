...
public class GisPublisherSearch {
	 ...
	 * @param isSearchForVreContext the is search for vre context
	 * @param httpSession the http session
	 * @throws Exception the exception
	 */
	public void search(boolean sortByTitle, String textToSearch, Boolean isSearchForVreContext, HttpSession httpSession) throws Exception{
		this.lastTextToSearch = textToSearch;
		GNSearchRequest req=getRequest(sortByTitle, textToSearch);

		logger.info("Text to search: "+textToSearch +", Search for VRE Context: "+isSearchForVreContext);

		LoginLevel loginLev = LoginLevel.DEFAULT;
		if(isSearchForVreContext!=null && isSearchForVreContext){
			loginLev = LoginLevel.CKAN;
		}

		logger.info("Using Login Level: "+loginLev);
		geonetworkReader.login(loginLev);
	...
	/**
	 * Gets the layer by uuid.
	 *
	 * @param uuid the uuid
	 * @return the layer by uuid
	 * @throws Exception the exception
	 */
	public LayerItem getLayerByUUID(String uuid) throws Exception {

		if(uuid==null || uuid.isEmpty())
			throw new Exception("Invalid UUID");

		return MetadataConverter.getLayerItemFromMetadataUUID(geonetworkReader, uuid);
	}
