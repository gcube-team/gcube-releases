...
import org.gcube.portlets.user.simulfishgrowth.model.verify.EntityVerify.VerifyException;
...
public class ModelerVerify extends EntityVerify<Modeler> {
	...
	public void verify() throws EntityVerify.VerifyException {
		...
		if (entity.getUploadFilenameData() == null) {
			toThrow = new VerifyException(String.format("No samples uploaded. Data needed in order to proceed."),toThrow);
		}
		if (entity.getUploadFilenameWeights() == null) {
			toThrow = new VerifyException(String.format("No weight limit indicators uploaded. Data needed in order to proceed."),toThrow);
		}
