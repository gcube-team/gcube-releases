...
<form action="<%=addUrl%>" id="<portlet:namespace />fm"
	...
	<fieldset class="fieldset i2s  i2s-large"
		...
		<div class="column">
			...
			<div class="column-content">
				<div class="control-group form-inline input-text-wrapper">
					<label class="control-label">Oxygen Rating :</label> <select
						class="form-control" name="<portlet:namespace />oxygenRatingId"
						id="<portlet:namespace />oxygenRatingId">
						<%
							for (OxygenRating item : oxygenRatingList) {
						%>
						<option
							<%=(item.getId() == (entity == null ? 0L : entity.getOxygenRatingId())) ? "selected" : ""%>
		...
		<div class="column">
			<div class="column-content">
				<div class="control-group form-inline input-text-wrapper">
					<label class="control-label">Region :</label> <select width="40px"
						class="form-control" name="<portlet:namespace />regionId"
						id="<portlet:namespace />regionId">
						<%
							for (Region item : regionList) {
						%>
						<option
							<%=(item.getId() == (entity == null ? 0L : entity.getRegionId())) ? "selected" : ""%>
							value="<%=item.getId()%>"><%=item.getDesignation()%>
						</option>
						<%
							}
			...
			<div class="column-content">
				<div class="control-group form-inline input-text-wrapper">
					<label class="control-label">Current Rating :</label> <select
						class="form-control" name="<portlet:namespace />currentRatingId"
						id="<portlet:namespace />currentRatingId">
						<%
							for (CurrentRating item : currentRatingList) {
						%>
						<option
							<%=(item.getId() == (entity == null ? 0L : entity.getCurrentRatingId())) ? "selected" : ""%>
							value="<%=item.getId()%>"><%=item.getDesignation()%>
						</option>
						<%
							}
