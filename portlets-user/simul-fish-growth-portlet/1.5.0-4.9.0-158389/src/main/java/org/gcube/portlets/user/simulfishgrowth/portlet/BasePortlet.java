...
public class BasePortlet extends MVCPortlet {
	...
	protected String getSocialNetworkingEndpoint(String scope) throws Exception {
		String endpoint = null;
		String endpointName = getPortletContext().getInitParameter(BasePortlet.CTX_PARAM_SOCIAL_NETWORKING_ENDPOINT);
		try {
			ScopeProvider.instance.set(scope);
			if (logger.isTraceEnabled()) {
				logger.trace(String.format("Query IS for [%s]", endpointName));
