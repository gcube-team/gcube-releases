...
import org.gcube.portlets.user.simulfishgrowth.model.verify.EntityVerify;
import org.gcube.portlets.user.simulfishgrowth.model.verify.ScenarioVerify;
...
public class SimulPortlet extends BasePortlet {
	...
	public void save(ActionRequest request, ActionResponse response) throws Exception {
		...
		try {
			...
			scenario.setStatusId(ModelerUtil.STATUS_PENDING_KPI);
			...
			// if I use ScenarioFull, jersey will compalin for the addition
			...
			if (doRun && (scenario.getModelerStatusId() == ModelerUtil.STATUS_READY)) {
