...
public class SiteFullUtil extends SiteUtil {
	...
	public List<Long> getSiteFullSimilar(Long id, Integer dtemp, Integer doxygen) throws Exception {
		URI uri = UriBuilder.fromPath(ConnectionUtils.endpoint).path(ENTITY).path(ConnectionUtils.SIMILAR)
				.path(String.valueOf(id)).path(String.valueOf(dtemp)).path(String.valueOf(doxygen)).build();
		String json = getData(uri);
		if (logger.isTraceEnabled()) {
			logger.trace(String.format("similars retrieved ~%s~", json));
		}
		List<Long> toRet = new ObjectMapper().readValue(json, new TypeReference<List<Long>>() {
		});
