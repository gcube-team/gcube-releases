...
import org.gcube.portlets.user.simulfishgrowth.model.verify.ModelerVerify;
import org.gcube.portlets.user.simulfishgrowth.util.AddGCubeHeaders;
import org.gcube.portlets.user.simulfishgrowth.util.CalcKPIAlgorithmExecutor;
...
public class ModelerPortlet extends BasePortlet {
	...
	public void save(ActionRequest request, ActionResponse response) throws Exception {
		...
		try {
			...
			String scope = getScope(httpSession);
			String token = getToken(httpSession);
			...
			Modeler modeler;
			if (id > 0) {
				modeler = new ModelerUtil(addGCubeHeaders).getModeler(id);
			} else {
				modeler = new Modeler();
			...
			if (id > 0) {
				new ModelerUtil(addGCubeHeaders).update(modeler);
			} else {
				new ModelerUtil(addGCubeHeaders).add(modeler);
			}
			...
			if (modeler.getStatusId() != ModelerUtil.STATUS_READY) {
				new ModelerUtil(addGCubeHeaders).cleanKPIs(id);
				try {
					...
					logger.warn(
							String.format("Problem on KPI calculation for simulid [%s]. I will clean its existing KPIs",
									modeler.getId()),
							e);
					// no leftovers
					new ModelerUtil(addGCubeHeaders).cleanKPIs(id);
					throw new UserFriendlyException(
							String.format("Problem on KPI calculation for (%s). Existing KPIs cleaned.",
									modeler.getDesignation()),
							e);
				}
			}
		} catch (Exception e) {
	...
	private void calcKPIs(String scope, String token, AccessPoint accessPoint, Modeler modeler) throws Exception {
		...
		try {
			if (Boolean.valueOf(getPortletContext().getInitParameter(DEBUG_BYPASS_KPI_CALCULATION))) {
				...
				String id = getPortletContext().getInitParameter(CTX_PARAM_CALC_KPI_ALGORITHM_ID);
				...
				new CalcKPIAlgorithmExecutor(endpointName, id, token)
						...
						.setModelId(String.valueOf(modeler.getId()))
						.setCalcKind(CalcKPIAlgorithmExecutor.CALC_KIND_MODEL).execute();

				// TODO restore for Global Model
				/*-
				new CalcKPIAlgorithmExecutor(endpointName, id, token)
						.setConnectionInfo(accessPoint.name(), accessPoint.username(),
								StringEncrypter.getEncrypter().decrypt(accessPoint.password()), accessPoint.address())
						.setModelId(String.valueOf(modeler.getId()))
						.setCalcKind(CalcKPIAlgorithmExecutor.CALC_KIND_GLOBAL).execute();
				*/
