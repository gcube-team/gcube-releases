...
public class ModelerPortlet extends BasePortlet {
	...
	public void save(ActionRequest request, ActionResponse response) throws Exception {
		...
		try {
			...
			boolean isNew = (id <= 0); 
			final Modeler modeler = isNew ? new Modeler() : new ModelerUtil(addGCubeHeaders).getModeler(id);
			if (id < 0) {
				modeler.setId(id);
			...
			com.liferay.portal.theme.ThemeDisplay td  = (com.liferay.portal.theme.ThemeDisplay) request.getAttribute(com.liferay.portal.kernel.util.WebKeys.THEME_DISPLAY);
		    javax.servlet.http.HttpServletRequest httpRequest = com.liferay.portal.util.PortalUtil.getHttpServletRequest(request);
		    httpRequest = PortalUtil.getOriginalServletRequest(httpRequest);

		    //Retrieve layout id of another portlet 
		    String portletIdWhatIf = "simul_WAR_simulfishgrowthportlet";
		    PortletURL portletURLWhatIf = com.liferay.portlet.PortletURLFactoryUtil.create(httpRequest ,portletIdWhatIf, PortalUtil.getPlidFromPortletId(td.getScopeGroupId(), portletIdWhatIf), javax.portlet.PortletRequest.RENDER_PHASE);
		    String portletIdTechno = "TechnoEconomicAnalysisPortlet_WAR_technoeconomicanalysisportlet";
		    PortletURL portletURLTechno = com.liferay.portlet.PortletURLFactoryUtil.create(httpRequest ,portletIdTechno, PortalUtil.getPlidFromPortletId(td.getScopeGroupId(), portletIdTechno), javax.portlet.PortletRequest.RENDER_PHASE);
		    String whatIdDid = "I have just " + (isNew ? "set-up a new model named" : "updated the model named") + " " + modeler.getDesignation() + ".";
		    final String message4Success = whatIdDid + " You can make use of it on a What-If Analysis at " + portletURLWhatIf.toString() + " or on a Techno Economic Investment Analysis at " + portletURLTechno.toString() + "."; 
			
			final ModelerUtil mutil = new ModelerUtil(addGCubeHeaders);
			spawnSaveThread(scope, token, username, modeler, mutil, message4Success);
		} catch (Exception e) {
			if (logger.isTraceEnabled()) {
	...
	protected void spawnSaveThread(final String scope, final String token, final String username, final Modeler modeler,
			final ModelerUtil mutil, final String message4Success) throws Exception, UserFriendlyException {
