...
public class SubmittedFormValidation extends MVCPortlet {
	...
	public void serveResource(ResourceRequest resourceRequest, ResourceResponse resourceResponse) throws IOException, PortletException {
		...
		String template = getTemplateName(phase, fileName);
	...
	private ValidationResult doWPSCallToDataMiner(String inputFileLink, String context, String token, String template) {
		...
		try {
			dms = getDataMinerInstance(context);
	 ...
	 * @return
	 */
	private String getTemplateName(String phase, String fileName) {
		switch (phase) {
		case PFISHConstants.SHOW_PRE_ONGROWING:
			if (fileName.toLowerCase().contains("close"))
				return TEMPLATE_PRE_ONGROWING_CLOSED;
			else
				return TEMPLATE_PRE_ONGROWING;
		case PFISHConstants.SHOW_HATCHERY: {
			if (fileName.toLowerCase().contains("aggregated")) {
				if (fileName.toLowerCase().contains("close"))
					return TEMPLATE_HATCHERY_AGGREGATED_CLOSED;
				else
					return TEMPLATE_HATCHERY_AGGREGATED;
			}
			else {
				if (fileName.toLowerCase().contains("close"))
					return TEMPLATE_HATCHERY_INDIVIDUAL_CLOSED;
				else
					return TEMPLATE_HATCHERY_INDIVIDUAL;
			}
		}
		case PFISHConstants.SHOW_GROW_OUT: {
			if (fileName.toLowerCase().contains("aggregated")){
				if (fileName.toLowerCase().contains("close"))
					return TEMPLATE_GROWOUT_AGGREGATED_CLOSED;
				else
					return TEMPLATE_GROWOUT_AGGREGATED;
			}
			else {
				if (fileName.toLowerCase().contains("close"))
					return TEMPLATE_GROWOUT_INDIVIDUAL_CLOSED;
				else
					return TEMPLATE_GROWOUT_INDIVIDUAL;
			}
		}
		default:
			return null;
