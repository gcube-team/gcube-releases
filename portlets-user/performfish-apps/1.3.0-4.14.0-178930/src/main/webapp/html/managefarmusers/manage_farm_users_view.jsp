...
<%
	...
	long currentGroupId = PortalUtil.getScopeGroupId(request);
...
<c:choose>
	...
	<c:otherwise>
		<a class="btn btn-link btn-large no-padding" href="${normalState}"><i
			...
			<% 
			String encodedFarmId = Utils.maskId(farm.getTeamId());
			pageContext.setAttribute("encodedFarmId", encodedFarmId);%>
		<a class="btn btn-link btn-large" href="manage-farm-users?<%=Utils.ENCODED_FARM_PARAM %>=${encodedFarmId}"><i
