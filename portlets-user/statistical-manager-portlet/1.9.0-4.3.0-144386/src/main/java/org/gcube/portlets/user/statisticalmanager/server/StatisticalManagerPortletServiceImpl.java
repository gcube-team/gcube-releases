...
import org.gcube.portlets.user.statisticalmanager.client.rpc.StatisticalManagerPortletService;
...
public class StatisticalManagerPortletServiceImpl extends RemoteServiceServlet
	...
	private StatisticalManagerDataSpace getDataSpaceService() throws Exception {
		HttpSession httpSession = this.getThreadLocalRequest().getSession();
	...
	private String getScope() throws Exception {
		HttpSession httpSession = this.getThreadLocalRequest().getSession();
			...
			if (computationStatus.isTerminated()) {
				try {
					...
					if (computationStatus.isFailed()) {
						...
						logger.error("Computation is failed");
						...
						Resource resource = ObjectConverter
								.convertSmResourceToResource(smResource,
										getUsername());
						logger.error("Error on computation: " + resource);
	...
	public List<ImportStatus> getImportsStatus(Date todayDate) {
		...
		try {
			...
			logger.error("Error retrieving user: "+user);
			e.printStackTrace();
			return null;
		}
		...
		try {
			dataSpace = getDataSpaceService();
		} catch (Exception e) {
			logger.error("Error retrieving dataspace: "+dataSpace);
			e.printStackTrace();
			return null;
		}
	...
	public ImportStatus getImportStatusById(String importId) {
		...
		if (smImport.abstractResource() != null) {
			if (smImport.abstractResource().resource() != null) {
				Resource res = ObjectConverter.convertSmResourceToResource(
						smImport.abstractResource().resource(), user);
				importStatus.setResource(res);
