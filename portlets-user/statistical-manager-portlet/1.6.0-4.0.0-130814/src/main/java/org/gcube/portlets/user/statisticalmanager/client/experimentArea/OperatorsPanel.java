...
//	private ArrayList<OperatorCategoryPanel> categoryPanels;
	...
	private void setToolbar() {
	    ...
	    filterField.addListener(Events.TriggerClick, new Listener<BaseEvent>(){
			@Override
	...
	private void showFilteredList(String searchText) {
		List<Operator> operators = StatisticalManager.getOperatorsClassificationByName(currentClassificationName).getOperators();
	...
	private void showCategoriesList(boolean force) {
		if (force || view != View.CATEGORIES) {
			...
			if (categoryPanels==null) {
				...
				for (OperatorCategory cat : StatisticalManager.getOperatorsClassificationByName(currentClassificationName).getOperatorCategories())
