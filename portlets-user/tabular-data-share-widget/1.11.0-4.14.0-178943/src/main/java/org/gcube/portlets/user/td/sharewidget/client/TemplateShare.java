...
public class TemplateShare {
	...
	public TemplateShare(UserInfo userInfo, TemplateData templateData, EventBus eventBus) {
	...
	protected void shareWindow() {
		...
		FileModel file = new FileModel(String.valueOf(templateData.getId()), templateData.getName(),
				templateData.getDescription(), false);
	...
	protected void shareCall(List<CredentialModel> credentials) {
		...
		TDGWTServiceAsync.INSTANCE.setShareTemplate(shareTemplate, new AsyncCallback<Void>() {
			...
			public void onFailure(Throwable caught) {
				if (caught instanceof TDGWTSessionExpiredException) {
					...
					UtilsGXT3.alert("Error sharing template",
							"Error sharing template: " + caught.getLocalizedMessage());
				}
			}

			public void onSuccess(Void result) {
				Log.debug("Template Shared: " + shareTemplate.getTemplateData().getId());
				UtilsGXT3.info("Shared", "Template is shared");
				/*
				 * ChangeTableWhy why = ChangeTableWhy.TABLEUPDATED;
				 * ChangeTableRequestEvent changeTableRequestEvent = new
				 * ChangeTableRequestEvent( ChangeTableRequestType.SHARE, trId,
				 * why); eventBus.fireEvent(changeTableRequestEvent);
				 */
